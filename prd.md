...
| **Phase-MM Wave 3** | 🟡 ACTIVE 2026-05-20 (Ralph in flight) — TTS reply via Kyutai Pocket-TTS swap (italian_24l INT8 voice giovanni). 5 stories US-MM-A04..A08. PRD: `scripts/ralph/prd.json` | ~1-2 sessions | Running |
| **Phase-ONB** | 🔵 STAGED — LLM-driven self-onboarding. Federated install: each operator clones Aura, runs setup wizard (Telegram token + LLM creds), opens Telegram and /start — Aura interviews them in their language (passive detection) and writes SOUL/AGENT/USER overlays via `workspace_write`. Self-demonstrating. Prompt-overlay swap in agentloop + new `finalize_onboarding` tool + migration v27 + IT/EN fallback templates. 2 stories US-ONB-A01..A02. Research: [docs/onboarding-research/](docs/onboarding-research/) (local + state-of-art 2026). PRD: `scripts/ralph/prd-phase-onb-staged.json`. **Sequenced right after Wave 3 (2026-05-20 user decision): the onboarding conversation IS the first end-to-end test of the full stack (audio IN/OUT + memory + tools + agent loop) — dogfood + regression coverage in one shot.** | ~1 session | After Wave 3 |
| **Phase-RAG** | 🔵 STAGED — Auto-adaptive Tool-RAG eval harness. 4-layer strategy: production-mined seed (`D:/tmp/bench_dataset.json` 188 entries) + autogen-from-examples + distractor stress + external HumanMCP/MCP-Bench import. 3 stories US-RAG-A01..A03. Plan: [docs/tool-rag-eval-plan-2026-05-20.md](docs/tool-rag-eval-plan-2026-05-20.md). PRD: `scripts/ralph/prd-phase-rag-staged.json` | ~1 session | After Phase-ONB |
| **Phase-KV** | 🔵 STAGED — Prompt-cache wiring. ~76% input cost reduction + ~3x TTFT on stable prefix (Anthropic provider). 5 stories US-KV-A01..A05 with AUDIT-FIRST ordering: byte-faithfulness audit → canonical tools[] + literal tool_call preservation → cache_control markers (system + MCP boundary + rolling tail) per-provider gated → observability normalization → 2-call + 8-turn rolling-anchor probe. Research: [docs/kv-cache-research/](docs/kv-cache-research/) (3 artifacts). PRD: `scripts/ralph/prd-phase-kv-staged.json` | ~1.5 sessions | After Phase-RAG |
| **Phase-MCP-UI** | 🟣 SKETCHED — Dashboard config framework for MCP servers: generic JSON-schema → form renderer, secret 3-way wiring, OAuth callback infra, capability grant UI, hot-reload via existing fsnotify reconciler. Gate before bulk MCP integration. ~4 stories | ~2-3 sessions | After Phase-KV |
| **MCP Roundup Wave 1** | 🟣 SKETCHED — Pilot batch of 3-5 self-hosted MCPs from survey ([docs/mcp-survey/](docs/mcp-survey/)): kimai_mcp (time) + vikunja-mcp (tasks) + obsidian-mcp (notes) + gitea-mcp (dev) + motherduck (analytics) | ~1-2 sessions | After Phase-MCP-UI |
