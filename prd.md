...
| **Phase-CONS** | Web ↔ Telegram 1+1 consolidation — CONS-02..13 (Wave A backend 7 stories + Wave B assistant-ui frontend 5 stories). New `internal/agentcore.Builder` + `PerTurnHooks`. Web gains 7 missing features (streaming, voice, ask_user, soft-budget, compaction, archive). CONS-04 feature-flagged for 1 week of live traffic. Backend SSE in CONS-07 speaks Vercel AI SDK data-stream protocol so assistant-ui frontend (Wave B) works out of the box. Reference survey: `docs/research-2026-05-24-cons-tmp-survey.md` (openhuman channels/routes.rs + ttsClient.ts + microcompact.rs + ask_clarification.rs STUB; assistant-ui templates/minimal/app/api/chat/route.ts; hermes voice_mode pattern). | 12 stories, ~5 sessions | net +710 (dedup -810 + parity +720 + Wave B +800) | 🟣 staged — locked after Phase-CTX-CLOSE |
