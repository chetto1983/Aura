...
> **▶ Amendment #108 (2026-08-03 — restore the cockpit memory graph on ArcadeDB and adopt Studio's bounded incremental exploration).** The Neo4j removal intentionally replaced the drawable Graph Explorer with a schema-only `ArcadeGraphView`: every `seed`, `expand`, and `schema_overview` request returns HTTP 200 with empty `nodes`/`edges` and a prose `query` saying traversal is not implemented. That is now a product defect, not an accepted reduction. This amendment supersedes the schema-only posture recorded by the ArcadeDB migration and supersedes the old conversation-footprint promise where it cannot be true against the delivered memory model. ArcadeDB memory stores `Entity` vertices and bitemporal `FACT` edges in one database per Aura identity; it does not store `Conversation`/`Message` vertices, and `source_run_id`/`source_memory_ids` are not a guaranteed thread-to-graph join. The Graph surface therefore represents **the authenticated identity's memory graph**, not a fabricated graph “linked to this thread.” The existing `session` wire field remains accepted for bundle compatibility but grants no scope and is not advertised as a conversation filter.
>
> **Read contract and isolation.** Aura reuses ArcadeDB 26.7.3 Studio's documented HTTP graph envelope: a read-only database query with `serializer: "studio"` yields compact vertices `{r,t,p,i,o}` and edges `{r,t,p,i,o}`. Aura calls only `/api/v1/query/{database}` through the tenant-specific database/user/password derived for the authenticated identity; the browser never receives an ArcadeDB credential, database name, arbitrary-query endpoint, or raw server response. `seed` and `schema_overview` compile to a bounded edge-first overview over schema-validated edge types, then use schema-validated vertex types only to fill remaining capacity with isolated vertices. `expand` accepts only an ArcadeDB RID matching `^#[0-9]+:[0-9]+$` and loads that vertex's direct incoming/outgoing edges, optionally narrowed to schema-validated relationship types. User values remain bind parameters; the only interpolated tokens are a validated RID, live schema type names escaped as identifiers, and server-owned numeric limits. Caller caps default to 75 nodes / 200 edges and are clamped to those maxima. The projection maps edge `o→source` and `i→target`, vertex `i+o→degree`, stable RID→ID, type→label, and human properties (`name`, then `title`, then type)→caption; vector/embedding payloads never cross into cockpit JSON. `FACT.predicate` is the visible edge caption while `FACT` remains the filterable relationship type. Output capping drops dangling edges rather than emitting a renderer-crashing partial reference.
>
...
>
> **Machine-checkable acceptance.** (1) transport tests pin the `serializer:"studio"` payload, the query endpoint, compact envelope decoding, server-error detail, and omission of credentials from errors; (2) compiler/projector tests pin default/max caps, identifier escaping, strict RID rejection before I/O, directionality, degree, captions, embedding omission, de-duplication, isolated vertices, filters, and dangling-edge removal; (3) AG-UI tests prove `seed`/overview return drawable nodes and edges and `expand` stays identity-scoped; (4) jsdom tests prove the ArcadeDB copy, explicit expand action, cumulative merge, and replacement-on-filter behavior; (5) Playwright on desktop and mobile drives the real authenticated `dvdmarchetto@gmail.com` database, observes its non-empty memory graph, expands a real RID without losing prior nodes, records zero serious/critical accessibility violations and zero browser errors, and performs no ArcadeDB write. Focused Go unit/race, web lint/typecheck/Vitest/build, repository vet/build, the live ArcadeDB read smoke, and a rebuilt embedded bundle must all pass before closure.
