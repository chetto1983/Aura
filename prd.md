...
> **▶ Amendment #60 (operator directive 2026-06-07) — modello primario default = `minimax/minimax-m3`, sostituisce DeepSeek-V4.** Directive: "minimax-m3 sarà al posto di deepseek v4". L'architettura lo permette già senza refactor — il modello è **config-driven** (`AURA_LLM_MODEL`, "mai hardcodare", D00/amendment #30); cambia il **default built-in**. Verifica capability live su OpenRouter (2026-06-07): minimax-m3 = **text+image+video**, ctx **1M**, max-out **512K**, **caching presente** (`cache_read $0.06/M`, write $0 — stessa shape auto-cache di DeepSeek → Slice 4 KV cache + cache-invariant audit + cost-model REGGONO), **reasoning + tools** entrambi ✓. Costo $0.30/$1.20 per M (~3-6× deepseek-v4-flash $0.098/$0.197). **Conseguenza architettonica chiave**: minimax-m3 è **multimodale** → da LLM primario copre la vision **nativamente** ⇒ il **tier cloud no-GPU (#59/5b)** non ha bisogno di NESSUN sidecar OCR-VL: la `photo.go` cloud-tier manda l'`image_url` direttamente al modello primario; il tier locale usa `aura-ocr-vl`/GLM-OCR per i media privati. **Gate pre-merge (NON bloccano la decisione, la qualificano)**: (a) **re-baseline cot_eval** (AI-SPEC scorava DeepSeek-V4 — rifare i 7 dimension vs minimax-m3 prima di dichiarare parità); (b) **misurare il cache-hit ratio reale** (target L0 ≥80% era su DeepSeek — riconfermare); (c) **rivedere i default cost-cap** (`AURA_LLM_LOCAL_FALLBACK_COST_USD_DAY` ecc.) dato il costo ~3-6×; (d) seed price-table A3 con `minimax/minimax-m3`. Le righe Slice-1 sotto + l'env `AURA_LLM_MODEL` (§Caps) sono marcate `[#60]` — il default diventa `minimax/minimax-m3`, DeepSeek-V4 resta opzione config-selezionabile e baseline storica.

**Open questions — CHIUSE.**
1. **~~Provider primario~~ → OpenRouter + `minimax/minimax-m3` (#60, era `deepseek/deepseek-v4-flash:exacto`), API key da `.env`.**
   Provider: **OpenRouter** (base URL `https://openrouter.ai/api/v1`, OpenAI-compat nativo).
   Model default: **`minimax/minimax-m3`** (#60 — multimodale, sostituisce DeepSeek-V4; quest'ultimo resta selezionabile via config + è la baseline cot_eval storica).
   ...
   Default built-in: `Provider="openrouter"`, `Model="minimax/minimax-m3"` (#60, era `deepseek/deepseek-v4-flash:exacto`), `BaseURL="https://openrouter.ai/api/v1"`, `APIKey=""` (richiesto, vuoto = error chiaro).
...
| `AURA_LLM_MODEL` | `minimax/minimax-m3` | operative | 1 | Model id (#60: default minimax-m3 multimodale, sostituisce `deepseek/deepseek-v4-flash:exacto` ancora selezionabile). Endpoint+model config-driven = seam portabilità D00 (amendment #30); mai hardcodare un provider. |
