...
>
> **Machine-checkable acceptance.** Tests MUST: reject noncanonical/unknown/missing/duplicate JSON for every registered top-level/nested schema and reproduce rational, draw, stratum/cluster hash, typed-ref, report, evidence-body, and transition-binding content addresses; reject serving-arm empty-feature hashes and endpoint-model nonempty-feature hashes; exhaust tiny Bernoulli assignments to prove conditional complete-randomization equality; cover all four draw vectors plus entropy/template/rollback/crash/retry/commit-unknown/concurrency faults with no redraw or duplicate Assignment; prove all five cutoff/equality/late-fact/predecessor/disposition/unique-artifact rules and exact cumulative alpha; verify hypergeometric tails, strict equality, caps, outward rounding, missing substitutions, singleton/one-arm bounds, and exhaustive compatible binary potential-outcome coverage; pass the deterministic 5,000-member many/tiny/degenerate/rare/all-zero/all-one benchmark within caps with order-independent hashes, retaining the known floating FFT normalization regression as a failing negative oracle; reproduce DR/SNIPS/ESS and bootstrap sampler/rank vectors, including selected-intention eligibility with unknown delivery exposure, and reject policy/reward/model leakage, hashes, support, overlap, ESS, and uncertainty failures; and fault-test the sole writer, exact retry, rollback, distinct endpoint-report bindings, evidence-kind non-substitution, and complete transactional transition revalidation.
>
