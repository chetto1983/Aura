...
>
> **▶ Amendment #93.7 (2026-07-25, exact randomization, effect-bound inference, and OPE contract).** Task 2 of `docs/superpowers/plans/2026-07-24-aura-adaptive-intelligence-implementation.md` exposed an authorization gap: the current `CohortSpec` freezes marginal arm probabilities and block keys, but not the assignment mechanism or its revision; #93.5's exported trusted assignment callback can choose an arbitrary arm; the session/episode cross-product produces singleton strata under the one-focal-request-per-conversation rule; and the generic scalar `Snapshot.Utility` is not a genuine endpoint-specific outcome model. Exact evidence and doubly robust off-policy evaluation were therefore not authorized before this amendment. This amendment supersedes only conflicting #93.4/#93.5 shorthand about caller-owned arm drawing, statistical blocks, generic snapshot utility, evidence artifacts, and OPE; #93.6's bitemporal loader, trusted record-time cutoff, closed-view hash, and late-fact behavior remain unchanged and authoritative.
>
> **Versioned initial randomization.** Cohort artifact v2 MUST contain a canonical, immutable, content-addressed `RandomizationPlan`. Its initial plan has exactly two arms in this order: baseline/control, then challenger/treatment. Each arm has integer weight `1`, the common denominator is `2`, and the design is independent Bernoulli. The mechanism ID is `aura/go-crypto-rand-bit`, mechanism revision is `1`, and the sole entropy source is Go `crypto/rand` backed by the operating-system CSPRNG. Revision 1 reads exactly one fresh byte after the focal claim wins and maps its least-significant bit without rejection: bit `0` selects baseline/control and bit `1` selects challenger/treatment. There is no seed, caller-supplied entropy, deterministic assignment input, or API through which a caller chooses an arm.
>
> Before any entropy read, the caller and store MUST construct and fully validate either one arm-neutral Assignment template with both frozen policy alternatives or two complete arm alternatives. That phase may perform all fallible callback/template work but cannot see entropy, select an arm, mutate durable state outside the transaction, or defer arm-dependent validation until after the draw. Only after the owner-locked transaction has revalidated the cohort and a fresh globally unique focal claim insert has won may store-owned code read the OS CSPRNG, apply the frozen mapping, select one prevalidated alternative, and atomically persist both the schema-`2.0` Assignment and its immutable `RandomizationReceipt`. The receipt is bound to the Assignment and records mechanism ID/revision, entropy-source ID, exact draw byte as lowercase hexadecimal, mapped bit, selected arm role/ID, server-derived analysis-stratum ID, exact rational propensity `1/2`, the ordered arm catalog and policy references, and the lowercase SHA-256 of the canonical `RandomizationPlan`. It proves the declared byte-to-arm mapping and replay identity, not that entropy was honestly fresh; Go `crypto/rand` and the OS CSPRNG are an explicit trusted boundary.
>
> An exact retry reloads and validates the persisted Assignment and receipt and never draws. A uniqueness loser never reads entropy. Entropy failure, pre-draw callback/template failure, Assignment/receipt validation failure, transaction rollback, persistence failure, or unresolved commit status fails closed to static behavior and MUST NOT trigger an arm-dependent retry or redraw. Commit-unknown recovery first reloads durable claim, Assignment, and receipt state under the same owner scope; it returns the exact durable result when complete, reports conflict when partial or inconsistent, and may attempt no fresh draw until absence is authoritative. Fault paths MUST NOT reveal an uncommitted arm to code that can condition retry behavior on it.
>
> **Analysis strata and interference are separate artifacts.** The initial immutable analysis-stratum schema is `aura.adaptive.analysis-stratum/owner-claim-time-block/v1`, revision `1`. Before the draw, store-owned code derives its tuple solely from the pre-treatment owner UUID and the focal claim's trusted server claim-time block under the `RandomizationPlan`'s frozen UTC block width/origin. The canonical stratum ID is the lowercase SHA-256 of the repository-canonical JSON encoding of `{schema_id, revision, owner_id, time_block_id}`; the schema content hash and resulting ID are persisted in the plan, claim, Assignment receipt, and reports. An owner that is constant throughout an owner-scoped cohort is harmless redundant information and MUST NOT create a singleton stratum. No outcome, delivery, assigned arm, caller label, episode ID, session ID, or post-treatment fact may enter the stratum.
>
> Session, episode, conversation/interference-cluster identity, carryover horizon, shared-memory/tool-state policy, and the declared cross-unit interference assumptions instead belong to a separate canonical immutable `InterferencePlan`. They never automatically compose the statistical stratum. Any result-affecting interference not represented and bounded by that plan makes the cohort ineligible. Realized singleton or one-arm strata are retained exactly as randomized: they are never dropped, merged, relabeled, or post-hoc reblocked. Under the initial inference rule they contribute vacuous conservative potential-outcome bounds for their unidentified side; resulting lack of support or power makes evidence ineligible rather than changing the strata.
>
> **Exact binary finite-population effect bounds.** Independent equal-probability Bernoulli draws, conditional on the realized treated count within every frozen analysis stratum, induce the product of uniform complete-randomization distributions over assignments with those stratum counts. The promotion gate uses the finite-population binary average-treatment-effect bounds of Li, Spertus, and Stark, *Exact and Conservative Inference for the Average Treatment Effect in Stratified Experiments with Binary Outcomes* ([arXiv:2508.03834v1](https://arxiv.org/abs/2508.03834)), not a generic exact Bernoulli p-value and not the sharp-null diagnostic. The canonical algorithm ID is `stratified-esi-ate-v1`; its inference plan binds that paper/revision, the one-sided construction, the exact rational look alpha, an internal Bonferroni split of `alpha/2` for the treatment potential-outcome total and `alpha/2` for the control potential-outcome total, and arithmetic revision `exact-integer-rational-outward-v1`. Combinatorial probabilities and comparisons use exact integer/rational arithmetic where representable; every exported decimal lower endpoint rounds down and every upper endpoint rounds up.
>
> The inference plan also freezes positive maximum state-count, memory-byte, and wall-time limits before enrollment. Cancellation, arithmetic-domain failure, or any limit breach returns an explicit ineligible/fail-closed result and cannot fall back to an asymptotic interval, a different algorithm, or a partially computed bound. Exhaustive tiny-case enumeration verifies the production construction but is not the production estimator. An optional conditional hypergeometric sharp-null test may be emitted under its own diagnostic algorithm/revision, but neither it nor any exact sharp-null enumeration may substitute for the ESI ATE effect bounds.
>
> Quality passes only when the one-sided lower ATE bound for challenger minus baseline is **strictly greater than** the frozen minimum quality uplift. Harm passes only when the one-sided upper ATE bound for challenger minus baseline is **at or below** the frozen maximum harm increase. Quality and harm are an intersection-union decision, so both must pass; they may each use the same look alpha without splitting alpha between endpoints. Missing/censored outcomes remain ITT members. For the quality lower bound, a missing challenger/treatment outcome is imputed `0` and a missing baseline/control outcome is imputed `1`; for the harm upper bound the substitutions reverse. These conservative substitutions do not replace the separately frozen maximum total-censoring and differential-censoring gates, both of which must also pass.
>
> **Exactly five immutable looks.** `LookPlan` contains exactly five ordered look records fixed before enrollment. Every record persists a strictly increasing cumulative membership trigger or a strictly increasing UTC cutoff, its positive rational look alpha, cumulative rational alpha, inference algorithm/revision, and the required predecessor artifact behavior. Look 1 has no predecessor; looks 2–5 MUST bind the immediately preceding look artifact ID and hash and may proceed only when that preceding artifact's immutable disposition permits continuation. The five rational look alphas sum to at most `1/20`, and each stored cumulative alpha equals the exact prefix sum. A `(cohort, look)` artifact is append-only and unique: a repeated look cannot reuse an earlier report as a new look, replace an artifact, skip predecessor linkage, or incorporate facts that were late for its frozen cutoff. Only the #93.6 owner-locked, closed, pre-cutoff ledger view may seal a look.
>
> Admission power and ESI simulations MUST use production-shaped repeated-owner/time-block membership with the same analysis-stratum and `InterferencePlan` rules, including many small, one-arm, and singleton realized strata. Insufficient simulated support/power or a degenerate realized configuration fails eligibility; it never authorizes reblocking. Per #93.6, `ExpectedMembers` remains the preregistered ceiling whose excess poisons reconstruction, while `MinimumMembers` remains the floor required to seal a look.
>
> **Genuine endpoint-specific OPE.** An immutable `OPEPlan` separately freezes, for binary quality and binary harm, the explicit target policy `pi(a|x)`, the exact logged behavior/exposure policy `mu(a|x)`, the supported action set, whether the estimand is selected-intention or delivered-exposure value, the endpoint reward encoding/provenance, and distinct report IDs/hashes. Every action with positive target probability must have a known positive behavior/exposure probability under the chosen semantics. Selected and delivered actions are not interchangeable: delivered-exposure `mu` includes every randomized selection and stochastic delivery/fallback factor. Unknown or fallback exposure remains an ITT member, is counted by reason in the OPE report, and is OPE-ineligible.
>
> The generic scalar `Snapshot.Utility` is explicitly not accepted as endpoint-specific `qhat`. Every accepted outcome-model artifact is immutable, canonical, and content-addressed and binds: endpoint/evaluator identity; exact binary reward definition and provenance; a training cutoff and data-ledger hash; action-schema and feature-schema hashes; algorithm/model ID, version, and content hash; prediction clipping to `[0,1]`; the no-support fallback; target-policy hash; and predictions for every supported action. The preferred initial path trains only on eligible history whose trusted cutoff is strictly before cohort creation. If training uses the cohort, the `OPEPlan` MUST preregister assignment-independent folds over whole `InterferencePlan` clusters before outcomes, keep each cluster wholly in one fold, and seal fold membership, per-fold model hashes, and out-of-fold prediction hashes; row-wise or post-assignment folds are forbidden.
>
> For endpoint reward `r_i`, supported logged action `a_i`, context `x_i`, `w_i = pi(a_i|x_i)/mu(a_i|x_i)`, and clipped endpoint model `qhat`, the canonical `dudik-langford-li-dr-v1` estimate is `n^-1 * sum_i[sum_a pi(a|x_i)qhat(x_i,a) + w_i(r_i-qhat(x_i,a_i))]`, following Dudík, Langford, and Li ([arXiv:1103.4601v2](https://arxiv.org/abs/1103.4601)). Canonical `snips-v1` is `sum_i w_i r_i / sum_i w_i`; canonical `importance-ess-v1` is `(sum_i w_i)^2 / sum_i w_i^2`. The OPE plan freezes exact support/overlap thresholds, minimum ESS, `qhat` fallback, primary weight-clipping mode and any cap before enrollment; the initial standard DR/SNIPS estimands use untruncated weights (`none`), while any capped-weight sensitivity result is separately labeled and cannot replace them. It also freezes the uncertainty algorithm ID/revision, confidence level, whole-interference-cluster resampling or analytic unit, deterministic seed if applicable, replicate/iteration limits, arithmetic/rounding revision, and failure behavior. Missing support, insufficient overlap/ESS, model or reward leakage, an incomplete action prediction vector, or uncertainty failure makes that endpoint's OPE ineligible. OPE is diagnostic only and never substitutes for admission evidence or a closed-canary outcome gate.
>
> **Persisted artifacts and transition safety.** Cohort artifact v2 (`aura.adaptive.cohort/v2`) and its `RandomizationPlan`, `InterferencePlan`, `LookPlan`, endpoint-specific `OutcomeModel` references/prediction hashes, `OPEPlan`, and Assignment `RandomizationReceipt` are immutable canonical content-addressed contracts; parent artifacts bind every child schema ID, revision, canonical bytes hash, and policy/snapshot reference. Legacy v1 cohort artifacts remain readable historical records but are ineligible for new randomization, look sealing, exact evidence, OPE, or transition authorization and MUST NOT be rewritten or backfilled. Implementation may add one forward PostgreSQL migration when it lands, using the then-current next free migration slot observed from disk; this amendment does not assign or imply a migration number, and historical rows/migrations remain untouched.
>
> Evidence kinds are distinct typed strings: `canary_admission` and the repository-canonical closed outcome kind `canary_outcome`. A `canary_admission` artifact contains the frozen cohort/plans, snapshot, evaluator calibration, endpoint-specific model references, power simulation, and explicit operator approval, but no canary outcome, reconstructed ledger, or outcome report. It may authorize only the authorized canary-entry transition. A `canary_outcome` artifact can be produced only by the #93.6 sealed loader at or after its cutoff and binds the randomization-plan hash; look number/trigger/cutoff, alpha and predecessor artifact; closed-ledger and endpoint-report hashes; censoring diagnostics; exact owner/domain/provider/model/policy/snapshot/cohort scope; eligibility status; and sealer plus operator capability provenance. It may authorize only the separately authorized active transition and cannot authorize canary entry.
>
> Any future transition operation MUST transactionally reload and revalidate evidence kind, exact target state, owner/domain/provider/model/policy/snapshot/cohort scope, all bound content hashes, look/trigger/cutoff and predecessor, eligible status, and the current operator capability before state mutation and audit append. Kind substitution, stale scope, hash drift, wrong look/cutoff, an admission presented for active, or an outcome presented for canary entry fails closed. This artifact contract does not restore a policy-mutation API or authorize one.
>
> **Machine-checkable acceptance obligations.** Tests MUST: (1) exhaust every tiny independent-Bernoulli assignment vector and prove that conditioning on realized treated counts gives exactly the corresponding product complete-randomization distribution; hand-check ESI endpoints and exhaust compatible binary potential-outcome schedules for conservative coverage, including singleton/one-arm strata and both missing-outcome substitutions; (2) prove all five look triggers/cutoffs, exact cumulative alpha, predecessor linkage, late-fact stability, uniqueness, and tamper rejection; (3) fault-inject entropy reads/bit mapping, template/callback failures, transaction rollback, crash/retry, commit-unknown recovery, and concurrent claim races to prove no duplicate Assignment and no arm-dependent redraw; (4) pass a 5,000-member deterministic benchmark spanning many, tiny, degenerate, rare-event, all-zero, and all-one strata within frozen memory/time limits, with conservative rounding and order-independent report hashes, while retaining the known floating FFT normalization regression as a failing negative oracle and not vendoring research prototypes; (5) match hand-calculated SNIPS, DR, and ESS examples and reject target-policy, reward/model leakage, model/prediction hash, support/overlap, and exposure-semantics failures; and (6) prove strict `canary_admission`/`canary_outcome` non-substitution and complete transactional transition-scope revalidation.
>
> **Design basis and bounded choice.** Conditioning is justified by Pashley, Basse, and Miratrix, [*Conditional As-If Analyses in Randomized Experiments*](https://arxiv.org/abs/2008.01029); the effect-bound construction is Li, Spertus, and Stark's [stratified binary ATE ESI](https://arxiv.org/abs/2508.03834); DR follows [Dudík–Langford–Li](https://arxiv.org/abs/1103.4601); experiment-plan immutability and separation from application logic are informed by [PlanOut](https://arxiv.org/abs/1409.3174); and operational separation of assignment, exposure, metrics, and analysis follows Microsoft's [large-scale experimentation platform](https://www.microsoft.com/en-us/research/publication/the-anatomy-of-a-large-scale-experimentation-platform/). Precomputed complete-randomization schedules were considered but not selected because this bounded online cohort has an unknown final roster and must decide only after a unique live focal claim. Committed HMAC assignment was also considered but not selected because it introduces a long-lived secret, rotation/version and deterministic-input coupling without improving the required atomic unique-claim boundary. Store-owned OS-CSPRNG Bernoulli draws plus immutable receipts fit the online unknown-roster contract while making replay and audit mapping explicit.
>
> No clause above creates an environment variable, CLI, policy-mutation authorization, or learned-serving authorization. Canary and active learned serving remain unavailable except as separately authorized and implemented by later Task 10 work.
>
> The admission plan freezes owner/session/episode/time-block keys, blocking or stratification, and a no-between-unit-interference assumption. Its power simulation uses the same repeated-owner and time-block structure expected in production. The current ESS-weighted Wilson implementation is simulation evidence only and cannot authorize production promotion. Each planned look instead uses the preregistered assignment mechanism and exact randomization inference, stratified by the frozen blocks; the five look-specific alpha allocations sum to at most `0.05`, and effect-bound inversion must produce the required quality lower bound and harm upper bound. A cohort with unmodelled cross-request, cross-session, shared-memory, tool-side-effect, or time carryover is ineligible rather than treated as independent. Bounded-continuous quality requires a separately preregistered, simulation-calibrated estimator and is not silently substituted for the binary endpoint.
>
> The cohort loader reconstructs assignments, deliveries, outcomes, corrections, and membership solely from the immutable PostgreSQL ledger; it uses experiment arm/arm propensity for the intention-to-treat canary gate. Treatment-as-served OPE uses the actual delivery only when the complete marginal exposure probability—including randomized selection and any stochastic fallback—is known, frozen, positive, and recorded; a missing delivery, deterministic fallback with unknown exposure probability, or selected/delivered mismatch is OPE-ineligible but remains in ITT. It rejects invalid focal claims, orphan/mismatched cohort assignments, duplicate assignments/deliveries, unsupported or poisoned arms/actions, invalid probability vectors, support/overlap failure, incompatible policy/snapshot/model identities, claim counts above `ExpectedMembers`, provenance/calibration mismatch, and censoring violations; claim counts below `MinimumMembers` reconstruct deterministically but cannot seal. It persists a sealed evaluation artifact containing cohort, focal predicate hash, experiment/arm catalog, owner, provider/model, policy/snapshot, cutoff, reconstructed-ledger hash, report hash, and eligibility status. Only trusted in-process artifact builders—the admission sealer for a stored plan and the cohort loader for outcomes—may create eligible artifacts; an external caller cannot supply observations or a report body. Policy transitions accept the artifact ID and revalidate its exact scope, hashes, status, and expected transition in the same transaction. OPE reports SNIPS, doubly robust estimate, effective sample size, overlap diagnostics, and uncertainty; it is diagnostic unless based on a supported logging policy and never substitutes for canary admission or closed real-canary outcome evidence.
>
> **Two-stage canary evidence.** Shadow-to-canary admission and canary-to-active promotion use distinct immutable artifacts. A **canary-admission artifact** contains a verified snapshot, held-out evaluator calibration, power analysis, supported randomization/catalog plan, preregistered focal cohort, safety limits, rollback plan, and explicit operator approval; it makes no claim about canary outcomes. It is the only artifact eligible for `shadow -> canary`. A **closed canary outcome artifact** is loader-sealed from completed real randomized traffic and is the only artifact eligible for `canary -> active`. The policy transition function and evidence schema must be amended so migration `0058`'s single-report contract cannot make admission circular or accept the wrong artifact kind.
>
