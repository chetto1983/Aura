...
>
> **▶ Amendment #93.5 (2026-07-25, focal claim/assignment transaction boundary).** A standalone committed `Claim` API is forbidden because callback failure could strand an enrolled request without its assignment. The typed `CohortStore` exposes one owner-scoped operation that receives the cohort/request/conversation/session/episode identity plus a trusted in-process assignment-builder callback. In one `WithIdentityTx` transaction it locks the adaptive owner first, loads and revalidates the immutable cohort, verifies the exact focal predicate and required admission block identities, and attempts the unique claim insert. Only the winning fresh insert invokes the callback; the callback performs the local arm draw and returns the complete schema-`2.0` Assignment. `CohortStore` then revalidates that Assignment against the persisted cohort and claim—owner, request, deterministic assignment ID, domain/point/ordinal, experiment and arm support/propensity, action catalog/support, provider/model, policy epoch/version, snapshot ID/hash, and production-canary environment—and records it through the existing owner-locked ledger primitive before committing. For this focal contract, `eligibility_sha256` is the lowercase SHA-256 of the canonical JSON encoding of the exact ordered `eligible_actions` string array, and `catalog_sha256` is the lowercase SHA-256 of the canonical JSON encoding of the exact ordered `action_probabilities` array; both must match the ordered actions frozen in the cohort. Callback, validation, or assignment-write failure rolls back both claim and assignment. A uniqueness loser never invokes the callback and must serve the static champion. An exact retry returns the already-persisted claim plus its validated Assignment without drawing again; any changed immutable claim field, absent/invalid Assignment, or pre-existing Assignment without the claim is a conflict/fail-closed condition. No database or network work is permitted inside the callback. Lock order is always owner → cohort/claim → assignment event → aggregate sequence, and no adapter may execute or expose the learned path until this transaction commits.
>
