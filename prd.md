...
| **Wave 0 quick-wins (2026-05-21)** | ✅ closed — commit `570696b3`: wiki_page cross-tool read hint + accurate fusion comment + read_source extract.md fallback (Bug 1) + agent all-dup force-finalize (Bug 2). Pre-shipped before Phase-WIKI-B Wave A. | — | — |
| **Phase-WIKI-B** | 🚨 **CRITICAL — NOW BLOCKER FOR ALL STAGED PHASES (user 2026-05-21)**. Wiki bedrock = ingest + retrieval substrate. Until test matrix is green across complex real fixtures for every file type (xlsx, docx, pdf, pptx, csv, json, html, zip, epub, audio, image, url), every downstream phase is built on sand. Plan: [docs/wiki-retrieval-research/phase-wiki-b-plan-2026-05-21.md](docs/wiki-retrieval-research/phase-wiki-b-plan-2026-05-21.md). 4 cross-agent research artifacts converged into 4-layer composed solution (ingest+index+retrieval+quality). | ~3 Ralph sessions | NOW |
| **— Wave A (retrieval engine)** | 🔵 STAGED — 3 stories US-WIKI-B02 (hybrid FTS+vec+exact fusion on wiki search path; wires already-written `mergeHybridResults`) + US-WIKI-B04 (heading-level subnodes H2/H3 → parent_slug + byte-range snippets) + US-WIKI-B01 (`wiki_subgraph(query, depth, budget)` tool with PPR seeding + hub-aware BFS + token-budget render). ~700 LOC. PRD: `scripts/ralph/prd-phase-wiki-b-wave-a-staged.json` | ~1 session | Up next |
| **— Wave B (ingest robustness)** | 🟣 SKETCHED — markitdown plugins per format (xlsx per-row + docx per-heading + pptx per-slide + epub per-chapter + html per-section) + ingest LLM prompt v3 that materializes per-row/per-section sub-pages instead of summary-only. ~900 LOC (mostly Python plugins). Public fixture pool curated in plan §"Fixture pool" (Apache Tika + ISTAT + arXiv + Wikipedia IT + Project Gutenberg IT + Common Voice IT). | ~1 session | After Wave A |
| **— Wave C (quality)** | 🟣 SKETCHED — entity dedup pipeline (MinHash/LSH + Jaro-Winkler + union-find) + confidence-aware rerank + lazy query-time summarization (LazyGraphRAG pattern). ~480 LOC. | ~1 session | After Wave B |
| **Phase-ONB** | 🔵 STAGED → 🟡 **DEFERRED behind Phase-WIKI-B**. LLM-driven self-onboarding needs solid wiki to dogfood-validate. 2 stories US-ONB-A01..A02. PRD: `scripts/ralph/prd-phase-onb-staged.json` | ~1 session | After Phase-WIKI-B |
| **Phase-RAG** | 🔵 STAGED → 🟡 **DEFERRED behind Phase-WIKI-B**. Tool-RAG eval can only be meaningful when retrieval is reliable. 3 stories US-RAG-A01..A03. PRD: `scripts/ralph/prd-phase-rag-staged.json` | ~1 session | After Phase-ONB |
| **Phase-KV** | 🔵 STAGED → 🟡 **DEFERRED behind Phase-WIKI-B**. Prompt cache benefits depend on stable prefix shape from solid wiki. 5 stories US-KV-A01..A05. PRD: `scripts/ralph/prd-phase-kv-staged.json` | ~1.5 sessions | After Phase-RAG |
| **Phase-MCP-UI** | 🟣 SKETCHED → 🟡 **DEFERRED behind Phase-WIKI-B**. Dashboard config framework for MCP servers. ~4 stories | ~2-3 sessions | After Phase-KV |
| **MCP Roundup Wave 1** | 🟣 SKETCHED → 🟡 **DEFERRED behind Phase-WIKI-B**. Pilot batch of 3-5 self-hosted MCPs from survey ([docs/mcp-survey/](docs/mcp-survey/)): kimai_mcp (time) + vikunja-mcp (tasks) + obsidian-mcp (notes) + gitea-mcp (dev) + motherduck (analytics) | ~1-2 sessions | After Phase-MCP-UI |
| **MCP Roundup Wave 2..N** | 🟣 SKETCHED — Bulk expansion across calendar/email/comms/productivity. Each new MCP = 1 story (schema + i18n + capability gate). Auto-extends Phase-RAG eval via Layer-2 autogen | continuous | Additive |
...
The catalog below formalizes 10 new phases derived from convergent evidence (4-source agreement is the bar). Phase-WIKI-FIX was added 2026-05-22 after the substrate bench surfaced 10 substrate bugs; **shipped same day** by Ralph (8/8 commits, FTS hit ratio 0→20/20). Phase-TOOL was added 2026-05-22 evening after a 4-scout research run + live log analysis: user mandate "togli il rag dei tool, è una cagata; alla fine basta il search tool come hai te".
...
| **Phase-TOOL** | Tool surface + RAG cleanup — KILL tool RAG entirely (user 2026-05-22: "togli il rag dei tool è una cagata"; the unified `search` tool stays as single retrieval surface), delete 18 orphan unregistered Tool types (~-1500 LOC), description audit EN-only ≤200char + structural markers, kitchen-sink collapses (`task(mode=)` + `create_document(format=)` + `workspace(action=)`, ~-700 LOC), MCP supervisor with `tools/list_changed` listener + per-server circuit breaker + reconnect (Hermes pattern), dim-mismatch detect for `aura_memory_v1_compact`. Plan + Ralph queue: `.planning/post-drift-2026-05-21/Phase-TOOL/plan.md` + `scripts/ralph/prd-phase-tool-staged.json` | ~3-4 sessions, 10 atomic stories | net **-4000** | 🟡 in flight — Ralph kicked 2026-05-22 evening |
| **Phase-MODERNIZE** | Infrastructure hygiene + Wave-1 god-file splits (user 2026-05-22 option B: "Phase-INFRA + Wave 1 god-files"). 5 INFRA stories — depguard v2 enforcing §9 forbidden deps + golang.org/x/tools/cmd/deadcode in CI + 600-LOC file-size linter with grandfather baseline + opt-in lefthook pre-commit + auto-generated MODULE-HEALTH.md scoreboard. 10 god-file splits — migrations.go 1562, memoryindex/store.go 1143, runs/store.go 812, api/types.go 771, wiki/memory_hygiene.go 751, api/auth/store.go 740, sources/ingest/pipeline.go 733, qdrant.go 725, search.go 653, cron/dispatch.go 643. Plan + Ralph queue: `.planning/post-drift-2026-05-21/Phase-MODERNIZE/plan.md` + `scripts/ralph/prd-phase-modernize-staged.json` | ~3 sessions, 15 atomic stories | +540 (infra) + 0 (splits rebalance) | 🟣 staged — kicks after Phase-TOOL ships |
| **Phase-BUG** | 3 verified-still-present critical bugs (user-locked 2026-05-22 to ship next, BEFORE Phase-MODERNIZE) — US-BUG-01 `/api/chat` overlay loading wires AGENT.md/SOUL.md/USER.md/TOOLS.md into the web channel (cmd/aura/web_chat.go:109 still uses bare RenderSystemPrompt); US-BUG-02 inverts internal/logging → internal/api boundary (590 transitive deps → <50); US-BUG-03 propagates silent failures (health_server.go 3 unchecked json.Encode + backup/export.go 5+ unchecked gzip/tar close paths). US-BUG-04 (appendUniqueSorted) dropped — already cleaned per 2026-05-22 grep. Plan + Ralph queue: `.planning/post-drift-2026-05-21/Phase-BUG/plan.md` + `scripts/ralph/prd-phase-bug-staged.json` | ~1 session, 3 atomic stories | +40 + 3 real bugs fixed | 🔴 **NEXT** — ships after Phase-TOOL completes |
| **Phase-OUT** | Output discipline (truncate / spill / repeated-lookup throttle / tasks_completed / length-recovery / orphan-backfill) + 3 NEW budget enforcement stories (US-OUT-07 per-tool-class budget caps, US-OUT-08 OR-of-four guard, US-OUT-09 force-finalize on cap). Backed by 2 convergent scouts 2026-05-22 on tool-budget patterns (Hermes/nanobot/openhuman + arXiv 2511.17006). User-locked after Phase-MODERNIZE. | ~3 sessions, 9 atomic stories | ~+720 | 🟣 staged — locked after Phase-MODERNIZE |
| **Phase-CTX** | Context engineering substrate — `ContextEngine` ABC (hermes), `ContextCompressor` with `SUMMARY_PREFIX` + scaled budget + tool-result pruning + JSON-arg sanitizer + streaming scrubber, `payload_summarizer` with circuit breaker (openhuman), inline auto-compaction at 70% (codex). | ~3 sessions | ~+900 | 🟣 staged — locked after Phase-OUT |
| **Phase-CONS** | Web ↔ Telegram 1+1 consolidation — CONS-02..08. New `internal/agentcore.Builder` + `PerTurnHooks`. Web gains 7 missing features (streaming, voice, ask_user, soft-budget, compaction, archive). CONS-04 feature-flagged for 1 week of live traffic. | ~3 sessions | net -90 (dedup -810 + parity +720) | 🟣 staged — locked after Phase-CTX |
| **Phase-WIKI-SUBNODES** | Heading-level subnodes (H2/H3 → parent_slug + byte-range snippets) — re-scoped from Phase-WIKI-B Wave A US-WIKI-B04 after Phase-WIKI-FIX closed Wave A B02 and Phase-TOOL "no new tools" direction cancelled Wave A B01. Standalone 1-session story. | ~1 session, 1 story | ~+250 | 🟣 staged — parallel-safe, kicks anytime after Phase-OUT |
| **Phase-BUG** | Critical bug fixes — `/api/chat` overlay loading, `logging→api` boundary inversion, errcheck-hidden real bugs (health JSON, backup gzip), `appendUniqueSorted` dead-code | ~1 session | -30 + 2 bugs | 🔴 ship anytime — concurrent with any phase |
| **Phase-CACHE** | Provider prompt caching (`prompt_cache_key=thread_id`) + server-driven `end_turn` termination + description ≤200 char audit + `lastToolResult` empty-reply fallback + untrusted-content snippet upgrade. Small enough to fold into a feature phase opportunistically. | ~1 session | ~+50 | 🟣 staged — ship anytime |
