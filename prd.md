...
> **D12 — primitiva di isolamento (amendment #32, risolto 2026-06-01; rigore Neo4j/Memgraph, 2 agenti; registro `.planning/DECISIONS.md`).** **Primario = container+seccomp indurito** (questo piano): è l'unica primitiva production-grade *identica* su x86 **e arm64** (DGX), regge 7e multi-lang + 2b + swarm, switch-cost zero. **gVisor = seam di escalation x86-first** (flag runtime `--runtime=runsc` per-deployment, coerente con D00 "config-per-deployment"): si attiva se `SandboxEscapeBench` >5% (Gate-3 DoD, vedi PITFALLS Pitfall 1); **NON default day-1 perché l'arm64 di gVisor è preliminary/non-GA** (240/294 syscall, 4KB-page) → violerebbe l'invariante D00 (DGX = ARM). **microVM (Firecracker/Kata) RIGETTATO**: richiede `/dev/kvm`, bloccato su Hetzner cloud + non confermato su DGX → fallisce la portabilità D00 in partenza. **wazero = fast-path futuro** (in-process, no-Docker, honors `AURA_PRIVACY_MODE=local-only`) solo per snippet pure-compute; NON regge il matrice py/shell/js di 7e (no subprocess/socket/thread) → non è dipendenza di Slice 2.
...
- [ ] **Hardening flags completi (amendment #32 / PITFALLS)**: oltre ai sopra, `cap_drop: ALL`, `no-new-privileges: true`, `pids_limit: 64`, userns-remap (o rootless Docker). **Gate-3 DoD**: `SandboxEscapeBench` escape-rate **<5%** sul profilo di produzione, documentato in PHASE_REPORT.md; se >5% → escalation gVisor (x86) prima che Slice 2b shippi.
