...
> **▶ Amendment #60 (operator directive 2026-06-07) — switch modello+vision deve essere CONFIG-ONLY (zero codice), default INVARIATO.** Chiarimento operatore: *"non serve cambiare modello adesso ma deve essere semplice passare da GLM-OCR con DeepSeek a minimax senza scrivere codice"*. Quindi **NON** è un cambio di default — il default RESTA `deepseek/deepseek-v4-flash:exacto` (text-only) + `aura-ocr-vl`/GLM-OCR per la vision. Il requisito è la **switchabilità a configurazione** fra due modi di deployment:
...
1. **~~Provider primario~~ → OpenRouter + `deepseek/deepseek-v4-flash:exacto`, API key da `.env`.** *(default invariato; #60: switch a `minimax/minimax-m3` multimodale è config-only — vedi amendment #60)*
   Provider: **OpenRouter** (base URL `https://openrouter.ai/api/v1`, OpenAI-compat nativo).
   Model default: `deepseek/deepseek-v4-flash:exacto` (variant `:exacto` instradata da OpenRouter). **[#60: `minimax/minimax-m3` selezionabile via `AURA_LLM_MODEL` senza codice — `photo.go` rileva `SupportsVision` e bypassa il sidecar OCR.]**
   ...
   Default built-in: `Provider="openrouter"`, `Model="deepseek/deepseek-v4-flash:exacto"`, `BaseURL="https://openrouter.ai/api/v1"`, `APIKey=""` (richiesto, vuoto = error chiaro). **[#60: `minimax/minimax-m3` è uno swap config-only.]**
...
| `AURA_LLM_MODEL` | `deepseek/deepseek-v4-flash:exacto` | operative | 1 | Model id. Endpoint+model config-driven = seam portabilità D00 (amendment #30); mai hardcodare un provider. **[#60: settare `minimax/minimax-m3` (multimodale) = switch config-only; `photo.go` rileva `SupportsVision` e salta il sidecar OCR.]** |
