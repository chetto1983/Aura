...
The catalog below formalizes the post-DRIFT phases derived from convergent evidence (4-source agreement is the bar). As of 2026-05-24 (late afternoon), Phase-WIKI-FIX, Phase-TOOL, Phase-BUG, Phase-MODERNIZE, Phase-CACHE, Phase-DEFER, Phase-OUT, Phase-GRAPH-FULL, and Phase-CTX are closed. Phase-CTX-CLOSE is in flight (CTX-06 shipped via parallel Codex; CTX-07 split into 07A offline-shipped + 07B live-pending; CTX-08 queued). Phase-CONS follows Phase-CTX-CLOSE. Phase-RAG (tool-RAG eval) was cancelled 2026-05-24 per user — the unified `search` surface is the single retrieval contract.
