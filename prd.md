...
The catalog below formalizes 9 new phases derived from convergent evidence (4-source agreement is the bar). Phase-WIKI-FIX was added 2026-05-22 after the direct retrieval substrate bench (memory `project_2026-05-22_substrate_bench_diagnosis`) surfaced 10 distinct substrate bugs. Sequencing: Phase-WIKI-FIX is the priority-0 blocker (Phase-WIKI-B Wave A measurements are meaningless until FTS5 mirror is sync'd); Phase-BUG runs concurrent because it fixes a separate live bug.

| Phase | Scope (1-line) | Sessions | LOC delta | Trigger |
| --- | --- | --- | --- | --- |
| **Phase-WIKI-FIX** | Substrate bug sweep — FTS5 mirror auto-sync (the KEY fix), embed cache key includes output_dim, skip-and-continue on rebuild, admin /api/wiki/reindex endpoint + dim-mismatch auto-detect, system-page filter, slug dedup, settings.updated_at default, embed n_ctx smoke. Plan + Ralph queue: `.planning/post-drift-2026-05-21/Phase-WIKI-FIX/plan.md` + `scripts/ralph/prd-phase-wiki-fix-staged.json` | ~1-2 sessions, 8 atomic stories | +450 | **NOW** — blocks Phase-WIKI-B Wave A measurement |
