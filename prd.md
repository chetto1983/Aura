...
> **Il passaggio dall'uno all'altro = cambiare `AURA_LLM_MODEL` (+ eventuale `OPENROUTER_API_KEY`), MAI toccare codice.** Design che lo rende possibile (capability-driven, non hardcoded):
> **(a)** `internal/llm/openai_compat/models.go` ha già i flag capability per modello (riga §Slice-9c "SupportsVision/SupportsAudio") — il registry deve marcare `minimax/minimax-m3` `SupportsVision=true`, `deepseek/deepseek-v4-flash` `false`.
> **(b)** **`photo.go` instrada in base a `Model.SupportsVision` del primario**: true → allega l'`image_url` al turno del loop LLM primario (niente call al sidecar); false → POST a `aura-ocr-vl` `/v1/chat/completions`. Una sola riga di branch sulla capability, decisa a runtime dalla config — non due build.
> **(c)** I 3 sidecar 9c restano nel compose ma **opzionali/disabilitabili** (`AURA_CHANNEL_*` / un flag `AURA_MULTIMODAL_LOCAL_ENABLED`): in modo cloud-multimodale l'operatore non avvia `aura-ocr-vl`. STT/TTS restano locali in entrambi i modi (l'audio cloud è estensione separata).
> **(d)** Verifica capability minimax-m3 (OpenRouter 2026-06-07, per il giorno dello switch): text+image+video, ctx 1M, **caching presente** (`cache_read $0.06/M` — cost-model/Slice-4 reggono), reasoning+tools ✓, costo ~3-6× DeepSeek. **Gate quando/se si fa lo switch** (non ora): re-baseline cot_eval, misurare cache-ratio reale, rivedere cost-cap, seed price-table A3.
> Le righe Slice-1 + env `AURA_LLM_MODEL` restano su DeepSeek-V4; il valore aggiunto di #60 è il **branch capability-driven in `photo.go`** che rende lo switch gratuito.
...
internal/channels/telegram/    # Slice 9c (multimodal)  [#59: engine = aura-ocr-vl + aura-stt + aura-tts, tutti CPU]
  ...
  photo.go                # ~95   vision routing CAPABILITY-DRIVEN [#59+#60]
                          #       - base64 encode photo
                          #       - if primary Model.SupportsVision (es. minimax-m3): allega image_url
                          #         al turno del loop LLM primario → NIENTE sidecar (switch config-only #60)
                          #       - else (es. DeepSeek-V4 text-only, default): POST a aura-ocr-vl
...
- [ ] `photo.go` **vision routing capability-driven [#60]**: se il primario ha `Model.SupportsVision` (es. `minimax/minimax-m3`) → image_url al loop LLM primario, nessun sidecar; altrimenti (default DeepSeek-V4 text-only) → POST a **`aura-ocr-vl` (GLM-OCR — DECISO 2026-06-07)**. Lo switch fra i due = solo `AURA_LLM_MODEL`, zero codice. Registry `internal/llm/openai_compat/models.go` marca `SupportsVision` per `minimax/minimax-m3`=true, `deepseek/deepseek-v4-flash`=false. PaddleOCR-VL via `OCR_VL_MODEL` override solo se latenza critica.
...
| `MULTIMODAL_FALLBACK_MODEL` | (unset) | operative | 9c | **Selettore tier vision** (#59/5b): unset = tier locale `aura-ocr-vl` (GLM-OCR); `minimax/minimax-m3` = tier cloud per clienti no-GPU/no-sidecar (via OpenRouter). Anche failover se sidecar down. **[#59]** |
