...
| **Phase-BUG** | Critical bug fixes from cleanup audit + scout #7 — overlay loading on `/api/chat`, `logging→api` boundary inversion, errcheck-hidden real bugs (health JSON, backup gzip), `appendUniqueSorted` dead-code | ~1 session | -30 + 2 bugs fixed | **NOW** — concurrent with Phase-WIKI-FIX; fixes web bench invalidation |
