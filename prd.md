...
wired (NodeSpec + HubBridge.Dispatch + WaitForRun). The old standalone
`subagent_dispatch` LLM tool is historical and is not part of the current manifest.
...
registry, MCP, swarm primitives, propose_patch, memory layers, channels,
...
| --- | --- | --- | --- |
| **Phase-Z** | ✅ closed 2026-05-17 — PRD bounded closure | — | — |
| **Phase-FIX** | ✅ closed 2026-05-19 — 4 stories: graceful finalize all budget paths, diagnostic LLM-error UX, 429/transient retry coverage, MaxIterations context surface | — | — |
| **Phase-OP+** | ✅ closed 2026-05-19 — 7 stories shipped as mega commit `8edfd452` (operational memory + tokenjuice + brand refresh) | — | — |
| **Phase-MM Wave 1.5 + Wave 2** | ✅ closed 2026-05-19 — Audio IN E2E (Whisper sidecar + Telegram voice memo → agent loop), 6 commits + 6 lessons banked | — | — |
| **Phase-WIKI-A** | ✅ closed 2026-05-20 — Graphify cheap-wins (3 stories US-WIKI-A01..A03): recall_god_nodes + wiki_path BFS + confidence-labeled `related:` frontmatter | — | — |
| **Phase-MM Wave 3** | ✅ closed 2026-05-20 — TTS reply via Kyutai Pocket-TTS swap (5 stories US-MM-A04..A08, all shipped); audio IN+OUT E2E verified | — | — |
| **Wave 0 quick-wins (2026-05-21)** | ✅ closed — commit `570696b3`: wiki_page cross-tool read hint + accurate fusion comment + read_source extract.md fallback (Bug 1) + agent all-dup force-finalize (Bug 2). | — | — |
| **Phase-WIKI-B** | ⚪ superseded — its research artifacts moved to `docs/_archive/wiki-retrieval-research-2026-05-21/`. Retrieval bedrock shipped through Phase-WIKI-FIX and Phase-GRAPH-FULL; do not execute the old Wave A/B/C plan as-is. | — | superseded |
| **— Wave A (retrieval engine)** | ⚪ absorbed — hybrid retrieval fixed by Phase-WIKI-FIX; graph reads now live through the consolidated `search` action set and graph primitives. | — | absorbed |
| **— Wave B (ingest robustness)** | 🟣 deferred evidence — markitdown/plugin ideas remain useful but need a fresh benchmark before implementation. | ~1 session | future ingest phase only |
| **— Wave C (quality)** | ⚪ partially absorbed — embedding dedup and typed edges shipped in Phase-GRAPH-FULL; reranker/lazy summarization remain future research. | — | partially absorbed |
| **Phase-ONB** | 🟡 deferred — no active queue. Reopen only with a fresh dogfood benchmark against the current wiki graph substrate. | ~1 session | after explicit user re-open |
| **Phase-RAG** | ⚪ cancelled/superseded — tool RAG was removed by Phase-TOOL; future retrieval evals must target the unified `search` surface, not a tool router. | — | cancelled |
| **Phase-KV** | ✅ partially closed by Phase-CACHE. Remaining byte-faithfulness/lookback work needs a fresh plan; do not run the old staged queue. | ~1 session | future cache phase only |
| **Phase-MCP-UI** | 🟡 deferred — dashboard config framework remains a future product slice, not a blocker for the current Phase-CTX route. | ~2-3 sessions | after explicit user re-open |
...
The catalog below formalizes the post-DRIFT phases derived from convergent evidence (4-source agreement is the bar). As of 2026-05-24, Phase-WIKI-FIX, Phase-TOOL, Phase-BUG, Phase-MODERNIZE, Phase-CACHE, Phase-DEFER, Phase-OUT, Phase-GRAPH-FULL, Phase-CTX, Phase-CTX-CLOSE, and Phase-CONS are closed. Phase-DOCSKL is the active corrective phase selected from `scripts/ralph/prd-phase-docskl-staged.json`. Phase-RAG (tool-RAG eval) was cancelled 2026-05-24 per user — the unified `search` surface is the single retrieval contract.

| Phase | Scope (1-line) | Sessions | LOC delta | Trigger |
| --- | --- | --- | --- | --- |
| **Phase-WIKI-FIX** | Substrate bug sweep — FTS5 mirror auto-sync (the KEY fix), embed cache key includes output_dim, skip-and-continue on rebuild, admin /api/wiki/reindex endpoint + dim-mismatch auto-detect, system-page filter, slug dedup, settings.updated_at default, embed n_ctx smoke. Plan + Ralph queue: `.planning/post-drift-2026-05-21/Phase-WIKI-FIX/plan.md` + `scripts/ralph/prd-phase-wiki-fix-staged.json` | ~1-2 sessions, 8 atomic stories | +450 | ✅ **shipped 2026-05-22** — bench post-fix in `docs/quality-bench/runs/post-wiki-fix-2026-05-22.md` |
| **Phase-TOOL** | Tool surface + RAG cleanup — KILL tool RAG entirely (user 2026-05-22: "togli il rag dei tool è una cagata"; the unified `search` tool stays as single retrieval surface), delete 18 orphan unregistered Tool types (~-1500 LOC), description audit EN-only ≤200char + structural markers, kitchen-sink collapses (`task(mode=)` + `create_document(format=)` + `workspace(action=)`, ~-700 LOC), MCP supervisor with `tools/list_changed` listener + per-server circuit breaker + reconnect (Hermes pattern), dim-mismatch detect for `aura_memory_v1_compact`. Plan + Ralph queue: `.planning/post-drift-2026-05-21/Phase-TOOL/plan.md` + `scripts/ralph/prd-phase-tool-staged.json` | ~3-4 sessions, 10 atomic stories | net **-4000** | ✅ **closed 2026-05-22** — 10/10 stories shipped (US-TOOL-01..10), commits `976260fa..227fd2be` + `9f6e5d57` |
| **Phase-BUG** | 3 verified-still-present critical bugs (user-locked 2026-05-22 to ship after Phase-TOOL) — US-BUG-01 `/api/chat` overlay loading wires AGENT.md/SOUL.md/USER.md/TOOLS.md into the web channel; US-BUG-02 inverts internal/logging → internal/api boundary (590 transitive deps → <50); US-BUG-03 propagates silent failures (health_server.go 3 unchecked json.Encode + backup/export.go 5+ unchecked gzip/tar close paths). US-BUG-04 (appendUniqueSorted) dropped — already cleaned per 2026-05-22 grep. Plan + Ralph queue: `.planning/post-drift-2026-05-21/Phase-BUG/plan.md` + `scripts/ralph/prd-phase-bug-staged.json` | ~1 session, 3 atomic stories | +40 + 3 real bugs fixed | ✅ **closed 2026-05-22** — 3/3 stories shipped (US-BUG-01..03), commits `13b6926d`, `498893f8`, `d405e00b` |
| **Phase-MODERNIZE** | Infrastructure hygiene + Wave-1 god-file splits: depguard v2, deadcode CI, 600-LOC linter, lefthook, MODULE-HEALTH, and god-file splits. Plan + Ralph queue: `.planning/post-drift-2026-05-21/Phase-MODERNIZE/plan.md` + `scripts/ralph/prd-phase-modernize-staged.json` | ~3 sessions, 15 atomic stories | +540 (infra) + 0 (splits rebalance) | ✅ **closed 2026-05-23** — Wave A + Wave C shipped, commits `3dbef06e..3821bef1` |
| **Phase-CACHE** | Provider prompt caching (`prompt_cache_key=thread_id`) + server-driven `end_turn` termination + description ≤200 char audit + `lastToolResult` empty-reply fallback + untrusted-content snippet upgrade. | ~1 session | ~+50 | ✅ **closed 2026-05-23** — commits `5d875ada..dd833d68` |
| **Phase-DEFER** | Deferred tool-manifest protocol: split always-on core from deferred tool bodies and teach the prompt to request deferred schemas only when needed. | ~1 session, 3 stories | +65 | ✅ **closed 2026-05-23** — commits `50f8126e`, `1128aa81`, `e60541f6` |
| **Phase-OUT** | Output discipline (truncate / spill / repeated-lookup throttle / tasks_completed / length-recovery / orphan-backfill) + budget enforcement stories (per-tool-class caps, OR-of-four guard, force-finalize on cap). | ~3 sessions, 9 atomic stories | ~+720 | ✅ **closed 2026-05-23** — commits `c8a60b85..6aabfbda` |
| **Phase-GRAPH-FULL** | Wiki graph substrate: aliases, AliasIndex, alias-aware injection, extractor aliases, embedding-based entity dedup, and typed graph edges. | ~1-2 sessions, 5 stories | ~+2000 | ✅ **closed 2026-05-23** — commits `a410a440`, `6bb76d1a`, `875a50e8`, `5556319b`; graph rollback fix `6d96fd7b` |
| **Phase-CTX** | Context engineering substrate — OpenRouter `ModelContextWindow` fetcher (US-CTX-00), `ContextEngine` ABC + `DefaultContextEngine` (hermes pattern, US-CTX-01), `ContextCompressor` with `SUMMARY_PREFIX` (782 bytes) + scaled budget + tool-result pruning + JSON-arg sanitizer + streaming scrubber (US-CTX-02), `payload_summarizer` with circuit breaker (openhuman, US-CTX-03), `AutoCompactEngine` at **50% of model context window** (user-locked threshold, NOT 70%; US-CTX-04), agent-loop wiring + `/api/metrics` counters (US-CTX-05). | 6 stories, ~1 session | ~+900 | ✅ **closed 2026-05-24** — commits `ace821bb..a8cf1f0a`; metrics live (`aura_model_context_window`, `aura_compaction_threshold_tokens`, `aura_ctx_compactions_total`, `aura_payload_summarizations_total`, `aura_payload_breaker_trips_total`); 3 always-on conversions follow-up (PROMPT_CACHE + OP07 + MEMORY_JUDGE: commits `b2d7dc24`, `7e74da44`, `cd6f445b`) |
| **Phase-CTX-CLOSE** | Phase-CTX wrap — prefix protection + hysteresis + focus topic (CTX-06), bench fixture + threshold tuning + snapshot update (CTX-07 split 07A offline + 07B live), structured compaction event log + `/api/conversations/:id/compactions` endpoint (CTX-08). Plan: `.planning/post-drift-2026-05-21/Phase-CTX/{benchmark,plan,progress,source}.md`. | 3 stories, ~1 session | ~+550 | 🟠 **in flight 2026-05-24** — CTX-06 done via 4 parallel Codex commits (`b792b343` prefix protection + `9c5e0759` hysteresis + `d56c4595` focus-topic cap + `0742d3ac` robustness tests); CTX-07A offline runner shipped (`0b9fc290` `f69c0d6f`); CTX-07B live OpenRouter bench + CTX-08 queued |
| **Phase-CONS** | Web ↔ Telegram 1+1 consolidation — CONS-02..13 (Wave A backend 7 stories + Wave B assistant-ui frontend 5 stories). New `internal/agentcore.Builder` + `PerTurnHooks`. Web gains 7 missing features (streaming, voice, ask_user, soft-budget, compaction, archive). Backend SSE in CONS-07 speaks the current Vercel AI SDK UI Message Stream protocol so assistant-ui frontend (Wave B) works out of the box without opting into legacy data-stream mode. Reference survey: `docs/research-2026-05-24-cons-tmp-survey.md` (openhuman channels/routes.rs + ttsClient.ts + microcompact.rs + ask_clarification.rs STUB; assistant-ui templates/minimal/app/api/chat/route.ts; hermes voice_mode pattern). | 12 stories, ~5 sessions | net +710 (dedup -810 + parity +720 + Wave B +800) | closed 2026-05-24 — CONS-02..13 shipped; US-CONS-13 verified audio playback, source attachments, and browser dictation |
| **Phase-DOCSKL** | Corrective post-CONS tool restore: re-expose existing Go PDF/XLSX/DOCX builders through one `create_document(format=pdf|xlsx|docx)` action-enum tool, then expose existing skill list/catalog/info/install/remove backend through one `skill(action=...)` tool with visible admin denial. Plan + staged queue: `.planning/post-drift-2026-05-21/Phase-DOCSKL/plan.md` + `scripts/ralph/prd-phase-docskl-staged.json`. | 2 stories, ~1 session | ~+520 | active 2026-05-24 - first slice US-DOCSKL-01 |
| **Phase-WIKI-SUBNODES** | Heading-level subnodes were overtaken by Phase-GRAPH-FULL's alias/typed-edge graph substrate; keep this only as historical evidence until a fresh benchmark proves heading subnodes still add value. | ~1 session, 1 story | ~+250 | ⚪ superseded — do not execute as-is |
...
study in `docs/_archive/openhuman-research-shipped-2026-05-24/openhuman-multiagent-patterns-2026-05-18.md` and
`docs/_archive/openhuman-research-shipped-2026-05-24/openhuman-harness-architecture-2026-05-18.md`. It is no longer blocked
...
WaitForRun. The old standalone `subagent_dispatch` LLM tool is historical and
not part of the current manifest. Capability #5 from the "Aura come Claude
...
enforcement via `DirectWriteToolNames`, historical per-node risk-tier support, E2E
