...
> **Amendment #92 (2026-07-23, fix-plan 1.7 / BUG-1B): a gated scheduled task can no longer be silently forgotten.** A task forced to `status='pending_approval'` (every `agent_job` per AG-016, or any risky-scored task — `task.go:216-220`) rides the origin-channel `ask_user` pause for its approval: an authenticated, fail-closed resume, NOT the Notifier. The residual liveness gap: if the model never relays the approval directive (or the operator misses the single prompt), the task sits in `pending_approval` forever with no further signal. This adds a throttled recurring re-surface so a pending approval keeps nudging its owner until acted upon. Ratified contract:
>
> 1. **Per-tick reminder sweep, mirroring `sweepNotifications`.** `*cron.Dispatch` gains `sweepApprovalReminders(ctx)`, invoked from `Scheduler.tick` immediately after `sweepNotifications` via a second type-asserted local interface (`approvalReminderSweeper` — the existing `notificationSweeper` idiom, `scheduler.go:258-262`). No new goroutine; it runs on the existing scheduler tick.
> 2. **Throttle via a new nullable column, not a queue table.** Single migration at the next free slot (`ls internal/db/migrations/ | tail -1` at landing; indicatively `0051`) adds `aura.scheduler_tasks.approval_reminded_at timestamptz` (nullable). A new sqlc `ListDuePendingApprovalReminders` selects `status='pending_approval' AND identity_id NOT IN ('', 'local') AND (approval_reminded_at IS NULL OR approval_reminded_at <= now() - $interval) ... FOR UPDATE SKIP LOCKED` (bounded `$limit`); `MarkApprovalReminded($id)` stamps `approval_reminded_at = now()`. Leaving `pending_approval` (ApproveTaskRow → active, or cancel) ends the reminders by construction — no cleanup, no new state machine, no backfill.
> 3. **Re-surface only to channel-owned origins, reusing the existing `ChannelDeliverer` seam.** The sweep reuses `DispatchDeps.ChannelDeliverer` + `originGate` (`deliver.go:48`); it delivers a short nudge to the task's `identity_id` (→ Telegram account → chat, `telegram/deliver.go:49-56`). `identity_id IN ('', 'local')` (CLI/cockpit-local) is filtered in SQL — cockpit approvals are already visible in the AG-UI approvals panel and need no push. No new channel seam; the composition-root wiring (`serve_dispatch.go:33,119`) is unchanged. cron does not gain a `channels`/`tools` import (import-cycle constraint preserved).
> 4. **Idempotent nudge, not a delivery guarantee.** Every selected row is stamped `approval_reminded_at = now()` after the attempt REGARDLESS of outcome (delivered / no-channel / channel-error), so a pending approval re-nudges at most once per interval and a permanently-failing channel cannot spam the tick. No `attempts` counter (unlike `pending_notifications`, this is a recurring reminder that runs until the task leaves `pending_approval`). The reminder text is a bounded nudge — task kind + short summary + "approve or cancel in the cockpit" — and MUST NOT dump the payload (no goal/secret leakage beyond what the origin already saw).
> 5. **Cadence knob + kill switch.** `AURA_SCHEDULER_APPROVAL_REMINDER_SEC` (default **3600**; `<=0` disables the sweep entirely — no query issued, no ticker cost). Bounded batch limit mirrors `pendingNotificationSweepLimit`. Fail-soft: a sweep error is logged and swallowed per tick (the 0.2 swallow-per-tick posture), never terminates the scheduler.
> 6. **Non-goals:** no new notification route, no change to the approval/resume mechanism itself (the `ask_user` fail-closed pause is untouched), no reminder for non-channel (cockpit) approvals (panel-visible), no escalation/auto-expiry of un-approved tasks (a separate future policy). Env-catalog row added for the new knob.
...
| `AURA_SCHEDULER_APPROVAL_REMINDER_SEC` | `3600` | cap | 6 | Cadenza (secondi) del sweep che ri-notifica al canale d'origine i task in `pending_approval` finché non vengono approvati/annullati (amendment #92, fix-plan 1.7). `<=0` disabilita del tutto il sweep (nessuna query emessa). Solo per identità channel-owned (`identity_id NOT IN ('', 'local')`); throttle via colonna `scheduler_tasks.approval_reminded_at`. |
