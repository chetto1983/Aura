...
> **▶ Amendment #109 (2026-08-03 — ArcadeDB Studio-style graph captions are part of the canvas, not inspector-only metadata).** Amendment #72's dense-overview suppression is superseded for the identity memory graph: a node whose projected caption is non-empty MUST render that caption on the canvas, including the bounded 75-node overview, and a relationship whose projected caption is non-empty MUST render it along its edge. Aura adopts Studio's presentation semantics rather than its administration surface: captions are centred inside circular nodes, wrapped and contrast-adjusted; node diameter grows with the compact caption using Studio's `25 + 6 × label length` rule capped at 200 px while retaining degree as a minimum-size signal; relationship captions follow the edge with a background outline; and directed relationships keep an arrowhead. Technical identifiers remain compacted and human captions are capped at Studio's 45-character limit with the complete value preserved in the inspector and accessible DOM. Path highlighting may dim off-path elements but MUST NOT erase their captions. ForceAtlas2 remains Aura's renderer/layout boundary; importing Cytoscape, Studio's query editor, mutable actions, or a second graph implementation is forbidden.
>
> **Machine-checkable acceptance.** Pure tests pin label compaction, the Studio diameter formula/cap, multiline centring, contrast color, and outlined edge text. Renderer tests prove every non-empty node/edge caption is forced into Sigma's label layers, edge labels and arrow programs are enabled, dense overviews retain captions, and pinned-path reducers no longer delete off-path text. The authenticated desktop and mobile Playwright run must be rebuilt from `master`, show visible node names and relationship captions in captured canvases, keep the canvas nonblank, and report no browser or serious/critical accessibility errors.
