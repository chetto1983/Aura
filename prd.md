...
>
> **Scope — two flows share the name `onboarding`, only one is removed.** Identity provisioning (`POST /api/onboarding/start`, `/{token}/step`, `/{token}/provision`, `GET /{token}/telegram-status`, all gated by `identity.create`) is how an operator creates an identity and links its channels. It **survives untouched** — it is the "connect your channels" that first access becomes. The profile interview (`POST /api/onboarding/profile/start`, `POST /api/onboarding/{token}/profile`, `GET /api/onboarding/status`, no capability gate) is what this amendment removes.
>
...
>
> **File targets** (verified present 2026-07-26; the plan phase confirms the interview-only vs shared split before deleting). Removed: the interview step machinery and its LLM answer extractor (`internal/onboarding/interview.go`, `extractor.go`, `extractor_llm.go`), the Agent.md renderer (`draft_render.go`) and their tests; the multi-step web wizard (`web/src/onboarding/OnboardingWizard.tsx`, `ProfileOnboardingWizard.tsx`, `OnboardingStepper.tsx`, `OnboardingWizardNav.tsx`, `onboardingStepDispatch.ts`, `onboardingWizardModel.ts`) and the interview copy in `web/src/i18n/resources.onboarding.ts`. Modified: `cmd/aura/memory_onboarding.go` (one connection, no raw draft), `cmd/aura/serve_onboarding.go` (profile endpoints collapse to one submit), `web/src/onboarding/onboardingApi.ts` + `OnboardingDialog.tsx` (one form). Unchanged: `Answers`, `MapProfile`, the sentinel predicates, and every identity-provisioning path.
>
