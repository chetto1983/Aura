...
>
> **Authoritative data contract.** PostgreSQL is the source of truth; Neo4j is a replayable private analytical projection. New Postgres migrations occupy the verified next free slots `0052`–`0057`: typed owner-scoped adaptive outbox, permanent identity tombstones, distributed policy state, append-only promotion evidence/audit, tombstone privilege hardening, and runtime DML hardening. Cypher migration `0003_adaptive_intelligence` adds private `AdaptiveEpisode`/`AdaptiveEvent` projection labels attached to agent-memory's existing `User` anchor. Raw fact columns are immutable after insert; only lease/status delivery columns may update. Aggregate sequence allocation, tombstone fencing, and policy transition use narrow `SECURITY DEFINER` functions after direct runtime table mutation is revoked. Identity deletion is the sole sanctioned raw-event removal path through FK cascade.
>
