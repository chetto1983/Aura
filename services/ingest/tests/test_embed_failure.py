...
async def _embed_texts(texts: list[str]) -> list[list[float]]:
    ...
    return list(await asyncio.gather(*(embed.embed_text(text) for text in texts)))
...
def test_the_servers_own_explanation_survives():
    detail = embed._embed_failure_detail(
...
def test_the_token_count_and_the_ceiling_are_both_reported():
    ...
    detail = embed._embed_failure_detail(http_error(500, b"boom"), text)
...
def test_an_empty_body_says_so_rather_than_reading_as_a_missing_message():
    detail = embed._embed_failure_detail(http_error(500, b""), "x")
...
def test_a_body_that_cannot_be_read_does_not_replace_the_failure_with_its_own():
    ...
    detail = embed._embed_failure_detail(http_error(503, None), "x")
...
def test_embed_raises_with_the_detail_instead_of_the_bare_http_error(monkeypatch):
    ...
    with pytest.raises(RuntimeError) as caught:
        embed._embed_batch(["una perizia lunga"])
...
def test_an_overflow_is_cut_BEFORE_the_request_not_after_it_fails(monkeypatch):
    ...
    monkeypatch.setattr(embed.urllib.request, "urlopen", never_called)

    oversized = "Perizia tecnica dettagliata. " * 4000
    fitted, cost = embed._fit_for_embedding(oversized)
...
def test_an_oversized_chunk_does_not_take_its_batch_mates_down_with_it(monkeypatch):
    ...
    def urlopen(req, *_args, **_kwargs):
        ...
        return io.BytesIO(json.dumps({
            "data": [{"index": i, "embedding": [0.5] * embed.DIMENSIONS} for i in range(len(inputs))]
        }).encode())
    ...
    assert vectors == [[0.5] * embed.DIMENSIONS] * 2, "the healthy chunk was lost with the oversized one"
...
def test_vectors_are_placed_by_the_response_index_not_by_arrival(monkeypatch):
    ...
    def urlopen(req, *_args, **_kwargs):
        inputs = json.loads(req.data)["input"]
        rows = [{"index": i, "embedding": [float(i)] * embed.DIMENSIONS} for i in range(len(inputs))]
        return io.BytesIO(json.dumps({"data": list(reversed(rows))}).encode())

    monkeypatch.setattr(embed.urllib.request, "urlopen", urlopen)

    assert embed._embed_batch(["a", "b", "c"]) == [[float(i)] * embed.DIMENSIONS for i in range(3)]
...
def test_a_request_is_bounded_by_tokens_as_well_as_by_count():
    ...
    big = embed.EMBED_REQUEST_TOKEN_BUDGET // 2
    assert embed._request_end([big, big, big], 0) == 2
    # An input over the budget on its own still goes, alone, rather than never.
    assert embed._request_end([embed.EMBED_REQUEST_TOKEN_BUDGET * 2, 1], 0) == 1
...
def test_a_failure_that_is_not_an_overflow_is_raised_rather_than_retried(monkeypatch):
    ...
    with pytest.raises(RuntimeError, match="503"):
        embed._embed_batch(["una perizia corta"])
