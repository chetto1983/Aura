...
func newRuntimeDelegationWorker(chat *chatEnv, delivery *swarm.DelegationDelivery, controls *agui.RunRegistry) *runtimeProcessingWorkers {
	...
	workerTemplate := swarm.RunConfig{
		...
		// fallback CRED-07 forbids reaching for when a delegation worker's own
		// resolution fails. Resolver+IdentityID (set per-job in
		// delegation_run.go's runWithHeartbeat) resolves the JOB's owning
		// identity; Runtime is the process-wide secondary for when no Resolver is
		// wired.
	...
	if resolver != nil {
		workerTemplate.Resolver = resolver
	}
	if controls != nil {
