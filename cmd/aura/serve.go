...
func bootServe(ctx context.Context, channelOverride func(name string) (enabled, ok bool), requestShutdown func()) (*serveEnv, error) {
	...
	// auth deps" already uses below. `aura chat` boots through bootChatEnv, a different entry
	// point that never calls this, so the refusal is serve-scoped by construction.
	if err := sandboxImagePreflight(ctx, chat); err != nil {
		chat.close()
		return nil, err
	}
	logMultiUserAvailability(chat.cfg)
	...
	if chat.cfg.ObservabilityCheckEnabled {
		if err := seedObservabilityCheckSweep(ctx, store); err != nil {
			slog.Warn("aura serve: seed observability scrape check", "err", err)
		}
	}
