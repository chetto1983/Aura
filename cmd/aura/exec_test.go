...
func TestParseExecArgs_SessionParsed(t *testing.T) {
	...
	if ea.session != "abc" || ea.lang != "python" || ea.code != "x" {
		t.Fatalf("--session must be parsed (inert handling happens in runExec), got %+v", ea)
	}
}
...
// TestRunExec_SessionExitUsage re-execs the --session inert path → exit 64.
func TestRunExec_SessionExitUsage(t *testing.T) {
	if os.Getenv("AURA_TEST_RUNEXEC_EXIT") != "" {
		runExec(strings.Split(os.Getenv("AURA_TEST_RUNEXEC_ARGS"), "|"))
		return
	}
	cmd := exec.Command(os.Args[0], "-test.run", "TestRunExec_SessionExitUsage") //nolint:gosec // re-exec self
	cmd.Env = append(os.Environ(),
		"AURA_TEST_RUNEXEC_EXIT=1",
	...
	if ee.ExitCode() != exitUsage {
		t.Fatalf("want exit %d for the reserved --session, got %d", exitUsage, ee.ExitCode())
	}
}
