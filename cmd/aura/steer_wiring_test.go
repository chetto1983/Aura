...
// decision in 52-02 committed to: the composition root injects ONE
// process-wide *steer.Inbox into both the cockpit route
// (agui.Server.SetSteerInbox, wired in serve_agui.go's wireAGUIServer over
// chat.steer) and the Telegram channel (telegram.Deps.Steer, wired in
// serve_channels.go's buildTelegramDeps over the SAME chat.steer field). Two
// separately-constructed inboxes would mean a cockpit steer and a Telegram
// steer for the same conversation land in different queues and only one is
// ever drained — nothing would fail, the steer would just vanish.
func TestOneInboxServesBothSurfaces(t *testing.T) {
	...
	// buildTelegramDeps just read, rather than constructing a second inbox.
	...
	t.Run("negative: two independently-constructed inboxes must not compare equal", func(t *testing.T) {
		a := steer.New(steer.Config{})
		b := steer.New(steer.Config{})
		if a == b {
			t.Fatal("two independently-constructed *steer.Inbox values must never be pointer-equal — the positive assertion above would be meaningless if this comparison could never fail")
		}
	})
}
