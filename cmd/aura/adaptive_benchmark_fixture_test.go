...
type cliOutcomeModelArtifact struct {
...
type cliOPEPlanArtifact struct {
	SchemaID               string                 `json:"schema_id"`
	Revision               int                    `json:"revision"`
	...
	EndpointModels         []cliOutcomeModelRef   `json:"endpoint_models"`
	...
	Uncertainty            cliOPEUncertainty      `json:"uncertainty"`
}

type cliOutcomeModelRef struct {
	SchemaID         string                        `json:"schema_id"`
	Revision         int                           `json:"revision"`
	EndpointKind     adaptive.EvidenceEndpointKind `json:"endpoint_kind"`
...
func validAdaptiveAdmissionManifest(t *testing.T) adaptiveBenchmarkAdmissionManifest {
	t.Helper()
	registrations := cliAdmissionArtifacts(t)
	manifest := adaptiveBenchmarkAdmissionManifest{
		SchemaID: "aura.adaptive.admission-manifest/v1", Revision: 1,
		OwnerID:  uuid.Must(uuid.NewV7()).String(),
		CohortID: uuid.Must(uuid.NewV7()).String(),
	}
	for _, registration := range registrations {
		path := filepath.Join(t.TempDir(), registration.Ref.Kind+".json")
		if err := os.WriteFile(path, registration.Canonical, 0o600); err != nil {
			t.Fatal(err)
		}
		manifest.Artifacts = append(
			manifest.Artifacts,
			adaptiveBenchmarkArtifactFile{Ref: registration.Ref, Path: path},
		)
	}
	return manifest
}

func cliAdmissionArtifacts(t *testing.T) []adaptive.EvidenceArtifactRegistration {
	t.Helper()
	...
	return []adaptive.EvidenceArtifactRegistration{
		...
		cliOutcomeModel(t, adaptive.EvidenceEndpointQuality, "b"),
		cliOutcomeModel(t, adaptive.EvidenceEndpointHarm, "c"),
		cliTypedRegistration(
			t, "ope_plan", "aura.adaptive.ope-plan/v1",
			cliOPEPlanArtifact{
				SchemaID: "aura.adaptive.ope-plan/v1", Revision: 1,
				SupportedActions: []string{}, EndpointModels: []cliOutcomeModelRef{},
				MinimumESS:     adaptive.MustExactRational(1, 1),
				MinimumOverlap: adaptive.MustExactRational(1, 2),
				Uncertainty: cliOPEUncertainty{
...
func cliTypedRegistration(
	...
	return cliTypedBytes(t, kind, schemaID, canonical)
}
...
func cliTypedBytes(
	...
	registration := adaptive.EvidenceArtifactRegistration{
		Ref: adaptive.ChildArtifactRef{
			...
			ArtifactSHA256: sha256HexForTest(canonical),
		},
		Canonical: canonical,
	}
	if err := adaptive.ValidateAdmissionArtifactRegistration(registration); err != nil {
		t.Fatalf("invalid typed CLI fixture %s: %v", kind, err)
	}
	return registration
}
