...
// runMCP prints ONLY the error. It used to append the top-level usage banner too, which was
// wrong twice over: every usage error here already carries its own, more specific line
// ("usage: aura mcp doctor <name>|--all"), so a bare `aura mcp` printed the banner TWICE;
// and a transport failure is not a usage mistake, so burying
//
//	mcp transport error: mcp "memory": calling "initialize": ... 404 Not Found
//
// under a wall of subcommand syntax tells the operator to check their spelling when the
// server is what answered. Measured on a live stack 2026-09-06.
