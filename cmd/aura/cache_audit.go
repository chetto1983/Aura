...
func reportHashes(reqs []llm.Request, out, errOut io.Writer) int {
	...
	for i, req := range reqs {
		...
		// messages[1] always-on skill block — only when the request carries one.
		if len(req.Messages) >= 2 {
			...
			if set1 && h1 != prev1 {
				_, _ = fmt.Fprintf(errOut, "messages[1] skills block mutated at request %d -- diff: %s vs %s\n", i+1, prev1, h1)
				return exitMutation
			}
			prev1, set1 = h1, true
		}
...
// a fixed skill set loaded — the messages[1] always-on skill block + the skill tool's
...
// hashMessages1 fingerprints req.Messages[1] — the messages[1] always-on skill block.
// With a fixed always:true skill loaded this must be byte-stable across every turn (a
// skill update would change it, but the audit fixture never changes).
func hashMessages1(req llm.Request) (string, error) {
	if len(req.Messages) < 2 {
		return "", fmt.Errorf("request is missing messages[1] (skills block)")
	}
