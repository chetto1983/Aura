...
func TestPublicShareRouteAllowlist(t *testing.T) {
	...
	tests := []struct {
		...
		{"public token asset stream subpath admitted", http.MethodGet, "/s/abc/asset/x/stream", true},
		...
		{"D-10 internal asset stream route stays authenticated, not public", http.MethodGet, "/api/shares/abc/asset/x/stream", false},
