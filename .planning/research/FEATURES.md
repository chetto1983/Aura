...
**Confidence:** HIGH for the historical audit context; superseded for active v1.0 scope.
...
| Historical telegram coverage target | Core conversation orchestration, streaming, document handling, access control had low coverage in the research snapshot | HIGH | Useful research for v1.1+ test planning. The former numeric target is not an active v1.0 blocker. |
...
| Historical file-generation tool split | Research identified mixed XLSX/DOCX/PDF generation logic in one tool package file | LOW | Deferred to v1.1+ unless restored by the approved planning docs. Not active v1.0 work. |
...
| Automated test coverage enforcement per-package | Prevents regression after a future coverage pass; catches untested paths before they reach production | LOW | v1.1+ research item. Add `go test -coverprofile` to CI or a `make coverage` target only when approved in current planning. |
...
Historical Telegram Coverage Expansion (High, independent)
    └──no deps──>
...
Historical file-generation tool split (Low, independent)
    └──no deps──>
...
- [ ] **Historical telegram coverage expansion** — v1.1+ research item unless restored by approved planning docs.
...
- [ ] **Historical file-generation tool split** — v1.1+ research item unless restored by approved planning docs.
...
### Add After Validation (v1.1+ Historical Research)

Features that build on successful v1.0 hardening but aren't required to close concerns.

- [ ] **Automated coverage scorecard in CI** — Trigger: once a future approved coverage pass lands, add a coverage gate to prevent regression
...
| Historical telegram coverage expansion | HIGH (catches regression in most critical package) | HIGH (383+342+634+507 = 1866 lines to test) | v1.1+ | Historical research only; not an active v1.0 target |
...
| Historical file-generation tool split | MEDIUM (cleaner codebase) | LOW (file-split, no logic change) | v1.1+ | Deferred unless restored by approved planning docs |
...
This matrix is historical. Priority tiers describe the old research ordering, not the approved v1.0 Production Readiness blocker list.
...
| Test coverage gates | 50-80% per package with CI enforcement | 0-30% typical for hobby projects | Historical target for `internal/telegram`; per-package thresholds as future post-validation |
...
*Feature research for: Go codebase hardening — historical broad hardening investigation, superseded for active v1.0 scope*
*Researched: 2026-05-04*
