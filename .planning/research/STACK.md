...
|---------|-----------|---------|-----------|
| UserGate actor/inbox | `map[userID]*userActor` guarded by `sync.Mutex`; each actor owns a bounded `chan gateEntry` inbox | Serialize Telegram message and notification processing per user, preventing race conditions on concurrent messages | Phase 1 checkpoint selected the actor pattern over keyed mutex. It gives FIFO ordering, bounded queueing, `TryAcquire`, and actor shutdown on inactivity without external deps. |
| Request timeout | `context.WithTimeout(ctx, 30*time.Second)` around each actor-processed turn | Prevent a hung LLM call from blocking one user's queue indefinitely | Built-in to `context` package. Composes naturally with actor processing + LLM call + tool execution. |
...
| UserGate actor/inbox (stdlib) | Third-party mutex pool library like `go-per-user-mutex` | The actor model is the Phase 1 checkpoint decision and handles queueing, notification `TryAcquire`, drop-oldest behavior, and idle shutdown in one local component. Third-party wrappers add abstraction without matching those semantics. |
...
|-------|-----|-------------|
| `sqlite-vec` or any second vector store | Qdrant is the CANONICAL single source of truth for embeddings. Persistence is via Qdrant's Docker volume. Adding sqlite-vec, pgvector, or chromem-go serialization creates a split-brain problem where two stores can disagree. | Qdrant only. SQLite FTS can remain for exact text search if intentionally used. |
...
```
[LLM Provider Client]                                  <- Existing provider wiring
        |
...
- **Provider client** remains the single configured outbound LLM path
