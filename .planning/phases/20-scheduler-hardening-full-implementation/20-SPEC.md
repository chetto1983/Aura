...
7. **Route precedence + no-origin fallback (observable contract)**: Explicit routes are honored; tasks with no owning channel degrade to today's behavior.
   ...
   - Target: `notify` unset OR `notify=stdout` → origin channel preferred; `notify=whatsapp|email` set → that route is honored and the channel is skipped; no channel owns the identity (CLI-origin, un-onboarded identity, deleted conversation → NULL/unmapped origin) → fall back to the notify route, else `AURA_SCHEDULER_NOTIFY_DEFAULT`, else stdout (purely additive — no regression).
     **AMENDED (Phase 20 live Step-1 gate, 2026-06-11):** `stdout` is treated like an unset route, NOT as an explicit channel. The scheduling agent auto-populates `notify="stdout"` (the task tool's documented default route) for a plain reminder, so treating `stdout` as "explicit → skip origin" silently re-created the exact Phase-19 headline bug (the reminder landed on the server console, not the origin Telegram chat). `stdout` is the always-available fallback sink (`notify.go`), not a deliberate external channel; only `whatsapp`/`email` pre-empt origin. Single-sourced in `originGate` so both the live-task and swept-row paths inherit it. Verified live in both gates.
