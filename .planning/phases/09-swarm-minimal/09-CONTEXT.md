...
### MCP mounts & live E2E (Areas P/Q/R + swap)
- **D-19 Servers:** `AURA_MCP_MAIL_SERVER` = **martinzarfl/mail-mcp** (Node, `npm install && npm run build`, stdio, 6 env vars SMTP/IMAP, NO OAuth) + `AURA_MCP_WHATSAPP_SERVER` = whatsmeow-based bridge (researcher picks the exact server, e.g. lharries/whatsapp-mcp; user's own number, **messages to self only**). Calendar DROPPED from Phase 9 → Phase 16 (MarimerLLC/calendar-mcp noted candidate, ICS/JSON fixture backend). Both MANDATORY in Gate 3 — phase pre-req: stand up + pair both.
- **D-20 Allowlist at Mount:** `mcptools.Mount` gains an optional per-server tool allowlist. Mail v1: `send_email, fetch_emails, search_emails, get_thread` (no mailbox create/delete/move). WhatsApp v1: send_message + chat read. Bridged tools stay `Deferred=true` (found via the 8.1 BM25 `tool_search`).
- **D-21 Boot-level env-gated mount:** `bootChat` mounts the servers when the env vars are set — stdio subprocess lifecycle coupled to the process, **fail-soft** (boot continues without a failed server, WARN log). mail+whatsapp immediately usable in daily `aura chat`; the eval harness reuses the same wiring. Recipes/doctor/risky-tool labeling stay Phase 16.
