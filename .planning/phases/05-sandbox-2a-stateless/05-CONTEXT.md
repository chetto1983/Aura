...
### Batteries-included package set (user directive — "not a toy")

> **Grounding:** user directive 2026-06-01 — "I want a sandbox like yours, not a toy." Claude Code's own sandbox ships a capable, batteries-included Python. E2B/Modal pre-provision the same way: packages baked into the image, not installed per-call. Under 2a's `network_mode: none` + `read_only` rootfs, runtime `pip` is physically impossible — so the only secure way to be useful NOW is to bake a curated set at BUILD time.

- **D-20 — Curated package set baked at BUILD time, runtime stays stdlib-purity-hardened.** The sidecar `Dockerfile` runs `pip install` of a curated, version-pinned `sandbox/requirements.txt` (numerical/data + parsing libs: numpy, pandas, scipy, sympy, matplotlib, pillow, beautifulsoup4, lxml, pyyaml, python-dateutil, openpyxl — the exact pinned versions + hashes resolved at execution time) during image build, when network IS available. The packages live in the read-only image layers; at runtime the container keeps `network_mode: none` + `read_only: true` + stateless — so user code does `import numpy` with **zero runtime pip, zero network, zero security-posture change**. This OVERRIDES the locked PRD/ROADMAP "(stdlib only, no pip)" wording for user code and requires a PRD-amendment (see § Required PRD Amendments item 3).
  - **D-20a — Arbitrary on-demand `pip install` stays a 2b (Phase 8) capability.** Runtime package installation needs network → it is gated behind 2b's `AURA_SANDBOX_NETWORK_ALLOW_HOSTS=pypi.org` allowlist (PRD line ~1252 already locks the 2b `pip install requests` test). 2a delivers a fixed curated toolkit; 2b adds on-demand installs. The set in `requirements.txt` is the editable knob.
  - **D-20b — Supply-chain + seccomp compatibility are gated.** `requirements.txt` is **version-pinned with hashes** (`pip install --require-hashes` ideal) and every package passes the package-legitimacy audit (real, maintained, expected). C-extension libs (numpy/pandas/scipy) must run under the seccomp allowlist — a positive `import numpy; numpy.arange(9).reshape(3,3)@…` smoke under the hardened runtime proves the moby-default-minus-dangerous floor is not too tight for the baked set (added to the 05-03 integration tier / VALIDATION map). No network libs (requests/httpx) in the 2a default — net-none makes them inert; they belong with 2b.

### Claude's Discretion
- Exact pinned versions + hashes for `sandbox/requirements.txt` (D-20) — resolve current stable at execution time; the SET (numpy/pandas/scipy/sympy/matplotlib/pillow/beautifulsoup4/lxml/pyyaml/python-dateutil/openpyxl) is fixed, the versions are the executor's pin.
...
3. **D-20 — batteries-included package set.** Amend the PRD §Slice 2 + ROADMAP Phase 5 goal "(stdlib only, no pip)" wording: the sidecar bakes a curated, hash-pinned `sandbox/requirements.txt` at BUILD time (numpy/pandas/scipy/sympy/matplotlib/pillow/bs4/lxml/pyyaml/dateutil/openpyxl); runtime stays `net-none` + `read_only` + stateless (no runtime pip). Record that arbitrary on-demand `pip install` remains the 2b/Phase-8 network-allowlist capability (PRD line ~1252). Keep the "no RUNTIME pip" property intact; only the build-time bake is new.
