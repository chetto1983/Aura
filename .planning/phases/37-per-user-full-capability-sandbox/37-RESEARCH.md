...
- skill-snippet exec → `snippet.go` has `SnippetSandboxPath` (`/skills/<name>/<name>.<ext>`) + `SnippetInvocation` returning the in-box path + interpreter. **CORRECTION (checker-verified 2026-07-06, was FALSE):** `action=use` does NOT currently return `SandboxPath` — `skillLoader.Snippet` (skill.go:77) returns only `(instructions, hostPath, interpreter, ok)`, and `actionUse` (skill_read.go:95) renders a `shell_exec` instruction via `renderSnippetUse`, calling NO `backend.Exec`. The 37-07 fix EXTENDS `Snippet` to also return `sandboxPath` (the adapter sources it from `SnippetSandboxPath`) and makes `renderSnippetUse` choose `SandboxPath` (strict) vs `HostPath` (lenient) for the rendered `shell_exec` command — the routed `shell_exec` then runs the box-materialized snippet. `action=use` itself executes nothing (no `backend.Exec`).
