...
<task type="auto">
  ...
  <action>
    Rewrite the three Phase 37E Success Criteria (ROADMAP.md ~552-556) to effort-only: SC1 — Composer exposes a reasoning-effort selector whose levels are auto-detected per active model (no hard-coded list, no placebo); the choice persists per-conversation and restores on reopen. SC2 — `/agent/run` accepts an optional symbolic `effort`, validated server-side in two stages (enum + capability); non-enum/non-advertised → 400; absent/`auto` → today's adaptive default; effort works on OpenRouter AND llama.cpp. SC3 — no governance bypass (symbol-only, server owns the map + capability gate); every level maps to a real wire knob; unit + e2e; coverage ≥85%. Remove every "model selector"/"model override"/allowlist-of-models clause (D-01). Search the whole 37E entry for any "Max is NOT added" / "off/low/mid/high/auto only" text and DELETE it, replacing with a note that the shipped scale is the 7-level capability-gated `auto·off·low·mid·high·extra·max` (D-09a VOID). Update the "PRD-first" line to reference the effort-enum + capability requirements. Leave the `**Requirements:** WEBMODEL-01, WEBMODEL-02, WEBMODEL-03` line intact.
  </action>
  <acceptance_criteria>
    - ROADMAP.md 37E section contains "reasoning-effort" and "capability" / "auto-detected" and "llama.cpp".
    - No occurrence of "Max is NOT added" or "off/low/mid/high/auto only" anywhere in the 37E entry.
    - No "model selector"/"selettore modello" in the 37E Success Criteria.
    - The `**Requirements:** WEBMODEL-01, WEBMODEL-02, WEBMODEL-03` line is unchanged.
  </acceptance_criteria>
  <verify>
