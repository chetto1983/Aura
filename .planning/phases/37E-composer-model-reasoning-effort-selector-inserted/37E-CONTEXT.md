...
- **D-09a (level-set reconciliation — RESOLVED at plan-phase, 2026-07-10):** 37E ships **exactly the locked `off/low/mid/high/auto` set (D-02); `Max` is NOT added.** Rationale: `Max` (an unlimited thinking-budget ceiling, `thinking_budget_tokens:-1`) is only distinct on the local llama.cpp path and would introduce a backend-specific level, whereas `auto` (Aura's adaptive policy, D-04) is the zero-regression default (D-07). The operator's llama.cpp reference UI's `Off/Low/Medium/High/Max` is NOT mirrored; instead 37E maps its four fixed levels onto llama.cpp budgets and keeps `auto` as the adaptive default. (Context for the wire map: llama.cpp graduated budgets from spike 095 are Low≈512 / Med≈2048 / High≈8192; `auto` sends no override. An unlimited-budget `Max` and finer gears remain a deferred idea.)
