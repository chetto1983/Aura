...
|---------|------|------|-------------|------------|-----------------|-----------|-------------------|-------------|--------|
| 25-01-01 | 01 | 1 | CHAT-02 | T-25-01/02/05 | uuid.Parse 404 guard; FTS parameterized; sanitizeErr | go integration | `go test -tags db_integration ./internal/agui/ -run TestConversationsAPI -race` | ❌ W0 | ⬜ pending |
| 25-01-02 | 01 | 1 | CHAT-03 | T-25-03 | reasoning-on cockpit-scoped; Telegram unchanged | go unit (golden) | `go test ./internal/agui/ -run 'TestTranslate|Reasoning'` | ✅ extend translator_reasoning_test.go | ⬜ pending |
...
| 25-04-02 | 04 | 4 | CHAT-04 | — | /0 cache guard; no $NaN; off aura.display namespace | vitest | `cd web && npm run test -- RuntimeFooter usageFromStateDelta` | ❌ W0 | ⬜ pending |
...
| 25-06-02 | 06 | 6 | CHAT-05 | T-25-22/23 | tx-safe ALTER; default backfill; no CONCURRENTLY-in-tx | go migration | `go test -tags db_integration ./internal/db/ -run TestMigrate -race` | ✅ extend migrate_test.go | ⬜ pending |
...
| 25-07-03 | 07 | 7 | CHAT-01 | — | real-stream E2E (no synthetic-only) | playwright E2E | `cd web && npx playwright test chat.spec.ts` | ❌ W0 (chat.spec.ts) | ⬜ pending |
...
- [ ] `internal/agui/conversations_api_test.go` — CHAT-02 (`db_integration`)
...
- [ ] `web/e2e/chat.spec.ts` — Playwright: prompt → stream → resolve inline card → resume
