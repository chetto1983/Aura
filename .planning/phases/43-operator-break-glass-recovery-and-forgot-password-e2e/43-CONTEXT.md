...
### Package placement + CI (resolved 2026-07-11 from research A1/A2)
- **D-09:** Put the **testable break-glass logic in a new `internal/breakglass` package** (guard/sourcing/generator/setter/reseed/audit orchestration); `cmd/aura/recover_operator.go` is thin flag-glue that calls it. **Why placement:** `cmd/aura` is excluded from the 85% owned-surface floor and its `db_integration` tests never execute in CI (RESEARCH Pitfall 1) — R6 ("≥85% on new files" + "runs-not-skips in CI") is only satisfiable with the logic in `internal/*`, where the coverage gate (`coverage_gate.sh ./internal/...`) runs its `db_integration` test.
  - **CI secret (DRIFT-CORRECTED, PATTERNS DC-1):** the research's "add `AURA_AUTHULA_SECRET` to the coverage-gate job" is a **NO-OP** — the secret is already a **workflow-level** `env:` at `.github/workflows/ci.yml:18` (dummy `…a1`), inherited by every job including `knowledge-integration-test`. **Do NOT edit ci.yml.** The **real** gap is LOCAL: `scripts/coverage_docker.sh` sets `CI=true` (:85) but only exports `POSTGRES_PASSWORD`/`NEO4J_PASSWORD` from `.env` — so the new `db_integration` test (which calls `webauth.New`, requiring the secret) `t.Fatal`s locally under the no-skip-as-green guard. **Fix:** export `AURA_AUTHULA_SECRET` in `coverage_docker.sh` via the existing `read_secret` helper (mirror the `PGPW`/`NEOPW` reads from `.env`). Leave `AURA_AUTHULA_DATABASE_URL` unset → it derives from the throwaway `AURA_DB_URL`.
- **D-11 (DRIFT-CORRECTED, PATTERNS DC-2):** `identity.Identity` gained a `Deactivated bool` field (`internal/identity/store.go:68`) since the research guard example. `selectSoleOperator` MUST make an explicit, documented choice about deactivated operators and carry a guard test for it. **Lean:** resolve over `kind='user'` identities; a lone deactivated operator should still be recoverable (that is a plausible lockout state), but a deactivated stale identity must not trip the `>1` refusal against an active operator — i.e. prefer counting **active** `kind='user'` operators for the ambiguity guard. The planner locks the exact rule and adds the deactivated-case guard test.
- **D-10:** The Playwright `password-reset.spec.ts` **mocks both happy and deny paths** via `page.route()` (mirrors `web/e2e/onboarding.spec.ts`); the deny spec mocks `/start` (neutral) + `/question` (deny) and asserts `login.reset.errors.generic` with no factor name in the DOM. The **real** backend deny branch (INNER-JOIN → 0 rows) is covered separately by the R6 backend tests, not the browser.
