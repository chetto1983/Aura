...
|----------|-------------|------------|-------------------|
| CodeQL `go/request-forgery` alert resolves to fixed | SEC-08 | CodeQL re-scan runs in CI/GitHub, not a local unit gate | After merge, confirm the alert state flips to fixed in the repo's code-scanning view (or via `gh api repos/chetto1983/Aura/code-scanning/alerts`). Acceptance per RESEARCH MEDIUM-confidence note: if the conditional guard doesn't close it, escalate to the documented explicit allow-list / unconditional classify. |
