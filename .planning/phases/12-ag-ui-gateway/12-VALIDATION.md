...
| CLI live 💭 reasoning render | 12-06 | 2 | UX-01 | T-12-19 | renderRunnerTurn streams dim 💭 reasoning live; stream-only (not in answer/prose buffer) | unit | `go test -race -run TestRenderRunnerTurnReasoning ./cmd/aura/` (→ ok, NOT "no tests to run") + `grep -c '💭' cmd/aura/chat_render.go` ≥1 | ⬜ | ⬜ pending |
