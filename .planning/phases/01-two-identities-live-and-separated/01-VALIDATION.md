...
| Mutation spot-check ≥70% killed on `internal/gateway` — the reservation-key machinery a prior replay defect lived in, and the surface this phase's ISO-05 evidence rests on | CLAUDE.md gate | `go-mutesting` runs only under WSL and is not wired into CI | Attempted under WSL 2026-09-08 (`go-mutesting ./internal/gateway/`, `internal/gateway` has 11 files): the run is genuinely slow (a full pass over `approvals.go` alone processed ~91 mutations in 600s, projecting well over an hour for the whole 11-file package at this rate) and the operator explicitly stopped the local run mid-session ("lascia stare i mutating li fa la CI" / leave mutation testing alone, CI does it) rather than let it consume the session further. **Result: NOT completed locally this session — deferred to CI's own mutation job, not fabricated.** The killed-ratio gate therefore remains genuinely unmeasured for this plan; record it the next time CI's mutation job runs against this package, or in a dedicated follow-up if CI does not yet run one. **Measured hazard:** `go-mutesting` mutates the target source file IN PLACE in the real working tree per mutant, and normally restores it before the next one; killing the process mid-run (`pkill -f go-mutesting`, used here to stop it per the operator's instruction) left `internal/gateway/approvals.go` mutated on disk (a broken `false` condition, an invalid map-assignment statement, and an empty `if {}` block) — caught only because the operator explicitly asked to verify the codebase was left clean, then fixed with `git checkout -- internal/gateway/approvals.go` (verified byte-identical to HEAD via `git diff --exit-code`, build+test green afterward). Anyone re-running this check must `git status --porcelain` / `git diff --exit-code` the target package immediately after, especially after a manual interrupt. |
| Each ISO-05 assertion shown to go red when its property is deliberately broken | ISO-05 | The break is a temporary source edit reverted afterwards; it cannot live in CI as a permanently-failing test | Per plan `01-05` T3, four breaks applied and reverted 2026-09-08 (all four caught, one break has two callouts): same `SessionID` for both runs → `TestTwoIdentityConcurrentSidecarPathsAreDisjoint/paths_disjoint` AND `/unreachable` both went red (`/unreachable` failed with "B's tool surface successfully read A's sidecar content using A's spill id — cross-identity read succeeded"); same shared `SessionID` also applied to the steer harness → `TestTwoIdentityConcurrentRunsDoNotCross/steer` went red 2 of 3 runs (a genuine race on the shared queue, not deterministic — recorded as such); one shared `*Budget` for both agents → `TestTwoIdentityConcurrentRunsDoNotCross/budget` went red ("run B (MaxSteps=5) completed 4 LLM calls, want exactly 3"); the session id injected into `NewLlmAgent`'s system message (`llm_agent_construct.go`) → `TestTwoIdentityConcurrentStablePrefixIsByteIdentical` went red on the byte-equality check. All edits reverted; `git status --porcelain` confirmed clean before the next step. Result: **all four surfaces confirmed to fail their own assertion when broken.** |
