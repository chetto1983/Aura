...
- Live latency follow-up from Telegram chat on 2026-05-08:
  ...
  - Verification: focused overlay/settings/Telegram tests passed, `verify-go.ps1` passed, `docker compose config --quiet` passed, rebuilt/restarted Aura with Compose, live `/status` returned ok, and container probe confirmed `PROMPT_OVERLAY_PATH=/app` plus `/app/AGENT.md` readable/writable by Aura.
  - LLM Wiki schema correction: reread `docs/llm-wiki.md` and tightened `AGENT.md` into a concise runtime schema for Aura: raw sources are immutable, compiled wiki pages are the maintained artifact, `AGENT.md` is the co-evolving schema, and ingest/query/lint loops must update `wiki/index.md`, `wiki/log.md`, links, and page frontmatter instead of doing one-off RAG answers.
  - Garage/setup follow-up idea: keep Garage as backup/artifact vault for now; plan a separate local-layout bootstrap slice before DB/setup wizard to ensure parent directories for `AURA_ENV_PATH`, `DB_PATH`, `LOG_DIR`, `WIKI_PATH`, `SKILLS_PATH`, and `MCP_SERVERS_PATH`, create minimal env/MCP files only when missing, and only later evaluate Garage as bootstrap/sync source after restore/import exists.
