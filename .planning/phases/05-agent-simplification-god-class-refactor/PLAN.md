...
- 2026-05-08 deletion pass: removed the remaining live profile/preflight taxonomy instead of disabling it. Deleted `internal/orchestration/capabilities.go`, `skill_policy.go`, `conversation/swarm_prompt.go`, and `cmd/debug_orchestration`; removed `AURA_SKILL_PREFLIGHT`; replaced `AURA_TOOL_PROFILE_MODE` with `AURA_TOOLSET_MODE`; removed legacy `memory`, `swarm_research`, `sandbox_compute`, and `admin_review` compatibility aliases; renamed profile telemetry/hooks to toolset; and rebuilt the embedded dashboard so stale preflight/settings labels are gone from `internal/api/dist`.
...
- 2026-05-08 Docker runtime cleanup: removed the broad `.:/app` Compose bind mount so the live Aura container no longer sees the whole developer repository at runtime. Replaced `.dockerignore` with a default-deny build context that includes only `go.mod`, `go.sum`, `cmd/aura`, `internal`, and `.env.example`; runtime wiki, skills, workspace, and data remain mounted under `/workspace` and `/data`. Also moved the first logger initialization after dotenv/config load so Docker honors `LOG_DIR=/data/logs` instead of creating `/app/logs`. Verified with `go test ./cmd/aura -count=1`, `docker compose config --quiet`, `docker compose up -d --build aura`, live `/status`, and a container probe showing `/app` contains only `/app/runtime`.
...
- [x] Step 2: Move the model/tool iteration.
