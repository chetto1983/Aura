...
## Deep-Dive Resolutions (2026-06-14)

Three deep-research passes (web + the live Aura codebase + d:/tmp curated sources) resolved the four open items below. These are verified and folded into plans 17-01..17-08.

### (1) Backup model — option-b hybrid (network pg_dump + APOC bolt export)

- **Postgres:** network `pg_dump` over the DSN from inside the socket-less box — `pg_dump -h postgres -U aura_migrate -Fc -f <dest> aura` (PGPASSWORD env, fixed-argv no-injection preserved). REQUIRES the `postgresql-client-17` client bundled in the fat image (plan 17-02): Debian bookworm's default `postgresql-client` is **v15**, which REFUSES to dump the **v17** server (`postgres:17.10`). Install it from the official **PGDG apt repo** (`apt.postgresql.org`) — NOT bookworm's default and NOT bookworm-backports. `pg_dump` lands at `/usr/lib/postgresql/17/bin/pg_dump`, surfaced on PATH via `pg_wrapper`.
- **Neo4j:** `CALL apoc.export.cypher.all(null,{stream:true,streamStatements:true,batchSize:1000,format:'cypher-shell'})` over bolt → write the concatenated `cypherStatements` to `<dest>.cypher`. APOC is already enabled (no Neo4j config change); `stream:true` bypasses `apoc.export.file.enabled`. Neo4j Community has **no online backup** and `neo4j-admin database dump` is offline-only — the APOC streamed export over bolt is the socket-free, online-safe path.
- **Restore:** `pg_restore` for Postgres, `cypher-shell` for Neo4j; `scripts/restore_drill.sh` needs a Neo4j leg added (currently Postgres-only).
- **Caveat:** verify the streamed APOC payload survives the line-buffered MCP channel; if `mcp-neo4j-cypher`'s read tool can't carry it, use a small direct-bolt (or `cypher-shell`) seam for the Neo4j leg.
- **backup.go reshape:** replace the `dockerCLI` seam with `pgDumper`/`neo4jDumper` seams; DELETE `resolveDocker()` + the docker `LookPath`; remove the WR-04 docker-exec host-stat block (PG now writes directly to the box's own `AURA_BACKUP_DIR` mount); KEEP `backupDir()`/the retention sweep/`MissedBackupAlert` (SC#3); stays ≤600 LOC.
- **Cite:** Neo4j ops-manual (dump is offline / Community has no online backup), APOC `export.cypher` docs, the live `internal/cron/handlers/backup.go` (`docker exec` at backup.go:102, the WR-04 comment at 47-53), the live `compose.yaml`.

### (2) whatsapp — chetto1983 fork + spike-002 patch, FastMCP transport flip

- The sibling is a **FastMCP streamable-HTTP server**; the stdio→streamable-HTTP conversion is a **one-line FastMCP transport flip** in the front (`transport="streamable-http"`), NOT an external proxy.
- Vendor the chetto1983 whatsapp-mcp fork + the spike-002 `bridge-patch.diff` into `docker/whatsapp/` (like `docker/agent-memory/` — reproducible, not clone-at-build). Two-stage: `golang:bookworm` builds the **CGO** whatsmeow bridge (`CGO_ENABLED=1`, go-sqlite3) → `python:3.11-slim` runs the FastMCP front; an `entrypoint.sh` runs BOTH (bridge REST `:8080` + front `mcp.run(transport="streamable-http", host="0.0.0.0", port=8080)`).
- Named volume **`aura-whatsapp-session`** for the whatsmeow store (`whatsapp.db`+`messages.db`) → survives `down/up`, no re-QR; headless first-run pairing = scan the QR from `docker compose logs -f whatsapp`.
- Loopback bind `127.0.0.1:8092:8080` (8091 is agent-memory); the `aura` service MUST NOT `depends_on` it (fail-soft, confirmed at `cmd/aura/main.go:206-225`). Default port **8092** matches `whatsappRecipeURL()` in catalog.go (plan 17-03).
- The existing `internal/mcp/whatsapp_integration_test.go` tool-name assertions (`search_contacts`/`list_messages`/`list_chats`/`send_message`) still hold (the FastMCP tool surface is unchanged by the transport flip).
- **Residual:** whatsmeow is an unofficial client → WhatsApp **ToS/ban risk**; ~20-day re-auth; multi-arch must cross-build the CGO bridge for arm64.
- **Cite:** the chetto1983 whatsapp-mcp fork, the spike-002 `bridge-patch.diff`, the live `internal/mcp/manager/catalog.go` (`whatsappRecipeURL`/`memoryRecipeURL`), `docker/agent-memory/` vendoring pattern.

### (3) Caddy — @authed matcher + respond 401, tls internal, token via env

- Token mechanism = a Caddyfile **`@authed` named matcher** (accepts `X-Aura-Token` header OR `?token=` query) + `handle @authed … reverse_proxy` + a fall-through `respond 401` — **NOT `forward_auth`** (over-engineering for a single shared token; reserve `forward_auth` for future per-identity RBAC).
- `tls internal` (Caddy internal CA, no ACME). Token injected via `{$AURA_ACCESS_TOKEN}` env (never baked). Generated by `install.sh` via `openssl rand -hex 32` → `.env`.
- **Only the `caddy` service binds `0.0.0.0:443`**; every data/sidecar stays `127.0.0.1`. Persist `caddy-data` (holds the internal-CA root).
- **Residual:** `tls internal` auto-trust FAILS inside Docker → other LAN hosts get a browser warning unless they import the root from `/data/caddy/pki/authorities/local/root.crt` (accepted per SPEC.md:160). Document in README.
- **Live verify:** `curl -k` 401-without-token + `nc -z` data-ports-refused probes.
- **Cite:** Caddy docs (`tls internal`, named matchers, `respond`), the live `compose.yaml` loopback topology.

### (4) gVisor — 3-line compose override + appliance-only runsc install

- `compose.gvisor.yaml` is exactly `services:` → `aura:` → `runtime: runsc` (3 content lines, no capability stripping). Opt-in = `docker compose -f compose.yaml -f compose.gvisor.yaml up -d`.
- Provisioning is **appliance-only, native-Linux, arch-aware** — gvisor apt repo (`echo "deb [arch=$(dpkg --print-architecture) …] https://storage.googleapis.com/gvisor/releases release main"`) or `runsc install`, then `systemctl reload docker`; gVisor needs Linux 4.14.77+ (supports amd64+arm64); NEVER attempted on Docker Desktop. The provisioning step lives in 17-07's appliance path; compose can't install a runtime.
- Spike 010 proves the python/subprocess workload survives `runsc` (full in-box parity). KVM-free. The cost is **perf** (latency on syscall-/I-O-heavy paths), not breakage — which is why it is optional on a CPU-budgeted mini-PC.
- **Cite:** gvisor.dev install + compatibility docs, spike 010, the live `compose.yaml`.
