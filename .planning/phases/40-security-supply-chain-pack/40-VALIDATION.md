...
| 40-05-01 | 05 | 1 | SEC-05 | T-40-05-RETAG / T-40-05-CODEQLSPLIT | Every `uses:` SHA-pinned + version comment; no `@latest`; CodeQL init/analyze same SHA (D-20) | ci/grep | `! rg -n "uses:\s*\S+@v[0-9]" .github/workflows/` | ✅ | ✅ passed |
| 40-05-02 | 05 | 1 | SEC-05 | T-40-05-SBOMGAP | GoReleaser `sboms:` archive + source; pinned `download-syft` before GoReleaser (D-17) | config/grep | `rg -n "sboms:" .goreleaser.yaml && rg -n "artifacts: source" .goreleaser.yaml` | ✅ | ✅ passed |
| 40-05-03 | 05 | 1 | SEC-05 | T-40-05-RETAG / T-40-05-SC | `workflow_pin_gate.sh` blocks unpinned/@latest, including multi-segment actions and `run: |`; self-tested (D-19) | shell self-test | `bash scripts/workflow_pin_gate_test.sh && bash scripts/workflow_pin_gate.sh` | ✅ | ✅ passed |
| 40-06-01 | 06 | 2 | SEC-01 | T-40-06-EXFIL / T-40-06-CORRUPT | Inbound exact-match redactor redacts configured secrets; agent-discovered values survive; 12-byte floor (D-10) | unit | `go test ./internal/secret/ -run TestRedactExact -count=1` | ✅ | ✅ passed |
| 40-06-02 | 06 | 2 | SEC-01 | T-40-06-SIDECAR / T-40-06-SPILLBYPASS / T-40-06-CAPDRIFT | No verbatim configured secret in turn body, spill, or `.result` sidecar; guard and byte offsets agree after redaction (D-09/D-11) | unit + db_integration | `go test ./internal/agent/tools/ -run TestNewResult -count=1 && go test -tags db_integration -race -p 1 ./internal/conversations/ -run TestAppendTurnRedaction` | ✅ | ✅ passed |
