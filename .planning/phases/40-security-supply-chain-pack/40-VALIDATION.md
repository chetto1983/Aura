...
|---------|------|------|-------------|------------|-----------------|-----------|-------------------|-------------|--------|
| 40-01-01 | 01 | 1 | SEC-09 | T-40-01-STALE / T-40-01-OVERCLAIM | REQUIREMENTS/ROADMAP/PRD SEC-09 amended to keyed-hash-or-documented-FP before any code (D-08) | docs/grep | `grep -q "keyed hash" .planning/REQUIREMENTS.md` | ✅ | ⬜ pending |
| 40-01-02 | 01 | 1 | SEC-01 | T-40-01-OVERCLAIM | SEC-01 scoped to configured-secrets-at-rest, unknown secrets outbound-only (D-10b) | docs/grep | `grep -qi "configured secrets" .planning/REQUIREMENTS.md` | ✅ | ⬜ pending |
| 40-01-03 | 01 | 1 | SEC-02 | T-40-01-STALE | SEC-02 amended to same-origin-only, PRD CORS text superseded (D-14b) | docs/grep | `grep -qi "same-origin only" .planning/REQUIREMENTS.md` | ✅ | ⬜ pending |
...
| 40-03-01 | 03 | 1 | SEC-06 | T-40-03-SMUGGLE / T-40-03-UNKNOWN | `strictDecodeJSON` rejects trailing-JSON / unknown-field / oversize / wrong content-type; per-route `allowEmpty` (D-16/D-16b) | unit | `go test ./internal/agui/ -run TestStrictDecodeJSON -count=1` | ❌ W0 | ⬜ pending |
| 40-03-02 | 03 | 1 | SEC-06 | T-40-03-SMUGGLE | 6 privileged routes reject trailing-JSON (400); scheduler-edit consciously deferred (D-16c) | unit (httptest) | `go test ./internal/agui/ -run TestStrictDecode -count=1` | ❌ W0 | ⬜ pending |
| 40-04-01 | 04 | 1 | SEC-03 | T-40-04-EXPOSE | Console refuses non-loopback bind unless `--unsafe-non-loopback` AND `AURA_INTEGRATIONS_CONSOLE_TOKEN` (D-15) | unit | `go test ./cmd/aura/ -run TestConsoleBindGuard -count=1` | ✅ | ⬜ pending |
| 40-04-02 | 04 | 1 | SEC-03 | T-40-04-NOAUTH / T-40-04-SILENT | Unsafe mode requires token per request (401 without); loopback stays unauthenticated + warns (D-15) | unit (httptest) | `go test ./cmd/aura/ -run TestConsole -count=1` | ✅ | ⬜ pending |
...
| 40-07-01 | 07 | 2 | SEC-02 | T-40-07-CORS / T-40-07-CSRF | `withCORS` deleted; no `Access-Control-*` header; OPTIONS not 204; SameSite=Strict preserved (D-14) | unit (httptest) | `test ! -f internal/agui/server_cors.go && go test ./internal/agui/ -run TestNoCORSHeaders -count=1` | ✅ | ⬜ pending |
| 40-07-02 | 07 | 2 | SEC-02 | T-40-07-DEADCODE | `AGUICORSPermissive` / `gateCORS` / `AURA_AGUI_CORS_PERMISSIVE` fully removed (no residue) (D-14) | unit + grep | `rg -n "AGUICORSPermissive\|gateCORS\|AURA_AGUI_CORS_PERMISSIVE" internal/ cmd/ ; go test ./internal/config/ -count=1` | ✅ | ⬜ pending |
| 40-08-01 | 08 | 3 | SEC-09 | T-40-08-FORGE / T-40-08-REUSE | `HashLookupToken` = peppered HMAC-SHA-256; `DeriveResetTokenPepper` HKDF, fails closed on non-64-hex (D-06) | unit | `go test ./internal/agui/ -run 'TestHashLookupToken\|TestDeriveResetTokenPepper' -count=1` | ✅ | ⬜ pending |
| 40-08-02 | 08 | 3 | SEC-09 | T-40-08-KEYABSENCE | Online mint→lookup round-trip resolves same identity post-pepper; fail-closed on bad secret (D-06b) | db_integration | `go test -tags db_integration -race -p 1 ./internal/agui/ -run TestPasswordReset` | ✅ | ⬜ pending |
| 40-08-03 | 08 | 3 | SEC-09 | T-40-08-DOA / T-40-08-FIXEDSALT | Break-glass pepper threaded + presence guard (blank secret exits non-zero before mint); CodeQL clears or documented-FP (D-06b/D-07/D-08) | unit + db_integration | `go test ./cmd/aura/ -run 'TestBreakGlass\|TestIdentityRecover\|TestRecover' -count=1` | ✅ | ⬜ pending |
...
| CodeQL `go/weak-sensitive-data-hashing` clears for the peppered HMAC | SEC-09 (D-07/D-08) | CodeQL CLI is not installed locally (only a query-pack cache dir exists); the real gate is the scheduled/push `codeql.yml` run | `gh extension install github/gh-codeql` → build a go DB → analyze with the `security-and-quality` suite, within the 3-strike budget. If it does not clear / CLI unavailable → write the documented-FP dismissal (256-bit CSPRNG token, not a password / no limited input space) per D-08 |
