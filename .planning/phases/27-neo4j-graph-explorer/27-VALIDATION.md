...
> **CLOSED 2026-06-19 (operator directive "for now close phase 27 as done").**
> Per-task statuses below are back-filled from the durable execution evidence —
> the four plan SUMMARYs (`27-01`…`27-04-SUMMARY.md`) + the task commits
> `57ff5ca0`, `8d21af9d`, `ecdf37af`, `adbfc587` (mobile polish `cb4d5521`) — not
> from a fresh re-run in this session. Frontend automated gates recorded in
> `27-04-SUMMARY.md`: 599/599 Vitest, `src/graph` 94.57% stmts, 75.88% Stryker
> (≥70% floor), 31/31 contrast pairs WCAG-AA, 12/12 live Playwright graph+a11y on
> chromium+mobile-chrome (real WebGL + axe + keyboard + session-expiry-401).
> **Caveat:** the working tree carries uncommitted post-commit graph WIP
> (`web/src/graph/*` + `internal/webui/dist` rebuild + untracked
> `web/e2e/_uiux-shots.spec.ts`) that is NOT covered by this closure.
...
- [x] `internal/knowledge/graphview_test.go` — intent→Cypher (params not interpolated, caps clamped), row→contract (labels via `apoc.convert.toJson`, edge endpoints), `assertReadOnly` (each verb present/absent, verbs in string literals, verbs in `CALL{}`) (GRAPH-01/04)
- [x] `internal/knowledge/graphview_integration_test.go` (`//go:build neo4j_integration && db_integration`) — pin the live mcp-neo4j-cypher serialization shape (Open Question A1) + `:Conversation`/`:Message` footprint probe (Open Question A3; `session_id == ThreadID`, else schema-overview fallback) + **goroutine-leak detection inherited** from `client_unit_test.go`'s sole untagged `goleak.VerifyTestMain` (the real client spawns mcp subprocess goroutines; this file adds NO second TestMain — that would duplicate the symbol; unit tests use a fake GraphReader with no goroutines so they need no goleak)
- [x] `internal/agui/graph_api_test.go` — httptest over a fake `GraphView` (route registration, 400 bad-intent, contract JSON shape) + a `RequireAuth(s.Mux(), …)` gate test asserting **401** on both routes when unauthenticated + seam-not-reached (mirrors `TestImageProxyRequireAuthGate`)
- [x] `web/src/graph/__tests__/graphIntent.test.ts` — filter/color/intent/normalization logic with NO WebGL (jsdom) + graphApi mocked-401 reject cases (B3)
- [x] `web/src/graph/__tests__/SigmaCanvas.test.tsx` — mocked `@react-sigma/core` renderer (role="img" + props flow); `web/src/graph/__tests__/GraphExplorer.test.tsx` — seed→schema-overview fallback + named 401-auth-error-state case (B3); `web/src/graph/__tests__/NodeInspector.test.tsx` — read-only action set (no add-note) + named `inspector renders citations` case (B4/W4)
- [x] Mock module for `@react-sigma/core` (or the whole `SigmaCanvas`) so component tests run in jsdom (Pitfall 4)
- [x] `web/e2e/graph.spec.ts` + `web/e2e/graph-a11y.spec.ts` — render + keyboard/tap/axe across desktop + mobile profiles (GRAPH-02/03)
- [x] Stryker `mutate` scope extension to include `web/src/graph/**`
...
**Manual-only status at closure (2026-06-19):**
- Dense-graph legibility — **covered** by the live Playwright run against the live Neo4j stack (the production graph has 0 `:Conversation` nodes, so the schema-overview is the first-class default, caps 75/200 + `graph.cap.notice` on overflow) + operator close acceptance.
- Backend `go-mutesting` spot-check on `graphview*.go` — **not separately recorded** in this closure (frontend Stryker 75.88% on the pure graph logic IS recorded). Deferred; re-run on WSL if a later audit requires the backend mutation number.
...
- [x] Feedback latency < 2 min (full integration tier)
- [x] `nyquist_compliant: true` set in frontmatter

**Approval:** granted — closed 2026-06-19 by operator directive, backed by the four plan SUMMARYs + task commits (`57ff5ca0`, `8d21af9d`, `ecdf37af`, `adbfc587`) and the recorded automated gates above. Uncommitted working-tree graph WIP is out of scope of this sign-off.
