...
1. **Agent interface + InvocationContext**: Interface base unificata che tutte le fasi successive implementano.
   ...
   - Target: `internal/agent/agent.go` definisce `type Agent interface { Name() string; Description() string; Run(InvocationContext) iter.Seq2[*Event, error]; SubAgents() []Agent; FindAgent(name string) Agent }`. `InvocationContext` carries: `Ctx context.Context`, `Agent Agent` (self-reference per workflow), `RequestID uuid.UUID` (UUIDv7 TraceID/run_id shape), `SpanID [8]byte` (crypto/rand, OTel/W3C SpanID shape), `ParentSpanID *[8]byte` (nil per root), `Branch string` (hierarchical path "root.iter-2.worker-3"), `Budget *Budget` (shared tree). Extension points (`SessionStore`, `LLMClient`, `Tools`) sono campo wire-noop in Phase 2 — riempiti in Phase 3/4. ~80 LOC totali.
...
2. **Event + Actions full shape (AG-UI forward-compat)**: Type unico per tutti i runtime/LLM-emitted Event con field forward-compat per Phase 12 AG-UI gateway senza refactor.
   ...
   - Target: `internal/agent/event.go` definisce `type Event struct { RequestID uuid.UUID; SpanID [8]byte; ParentSpanID *[8]byte; Author string; Branch string; ThreadID string; MessageID uuid.UUID; LLMResponse *LLMResponse; Actions Actions; Timestamp time.Time }`. `LLMResponse struct { Content string; ToolCalls []llm.ToolCall; FinishReason string }` riusa `internal/llm.ToolCall` e la sua `ID` come `tool_call_id` forward-compat. `Actions struct { Escalate bool; StateDelta map[string]any; ArtifactDelta map[string]any }`. Field `Author` = `<workflow_agent_name>` per workflow-runtime-emitted (es. `"InterviewLoop"`), `"user"` per user-input events, agent name per LLM events (adk-go pure pattern, NON constant globale). ~80 LOC.
...
3. **Budget tree shared atomic**: 3-cap budget contract con shared atomic counter propagato per reference parent→child (SC#3 enforcement).
   ...
   - Target: `internal/agent/budget.go` definisce `type Budget struct { steps *atomic.Int32; deadlineWallclock time.Time; dedupWindow int; dedupRing *dedupRing; branchSoftCap int }`. `NewBudgetFromEnv()` legge `AURA_LOOP_MAX_STEPS=25`, `AURA_LOOP_MAX_WALLCLOCK_SEC=300`, `AURA_LOOP_DEDUP_WINDOW=3` e gli hardening knobs `AURA_LOOP_DEDUP_EXEMPT_TOOLS`, `AURA_LOOP_BRANCH_SOFT_FRACTION`, `AURA_LOOP_NODE_TIMEOUT_SEC`, `AURA_LOOP_DEDUP_RESULT_CAP=2048`. `(*Budget).ConsumeStep() (ok bool, reason string)` usa atomic decrement-then-check-then-restore, returns `(false, "max_steps")` solo sul hard cap, `(false, "wallclock")` se `time.Now() > deadlineWallclock`; soft-cap fairness is a non-terminal scheduling signal, not hard exhaustion. Dedup is two-phase: pre-execution fingerprint `sha256(name + canonicaljson.Marshal(args))` blocks period-1/period-2 repeats before side effects; post-result preview only vetoes dedup when repeated args show progress. `(*Budget).Child(fanout int) *Budget` ritorna copia con stesso `*atomic.Int32` (shared), stesso `deadlineWallclock`, **distinct** `dedupRing` (child ha proprio dedup state per evitare false positive cross-branch) e passive branch soft cap. ~200 LOC split across `budget.go` + `budget_dedup.go`.
   - Acceptance: `TestBudget_ConsumeStep_AtomicDecrement_NoRace` (10 goroutines × 100 consume = exactly 1000 decrements via `go test -race`); `TestBudget_ConsumeStep_ExactlyOneWinner_When_CounterIsOne`; `TestBudget_Child_SharesStepsCounter` (parent consume 5 → child sees `Remaining()=20`); `TestBudget_Child_DistinctDedupRing` (parent dedup state non visibile a child); `TestBudget_BeforeToolCall_CanonicalHashOrderIndependent` (`{"a":1,"b":2}` == `{"b":2,"a":1}` hash identical after caller canonicalization); `TestBudget_DedupResultPreviewVetoesOnlyWhenProgress`; `TestBudget_Wallclock_TerminatesAtDeadline` (fake clock preferred to avoid goleak-hostile background timers).
...
5. **Loop workflow agent (SC#2)**: Esegue sub-agents N volte o fino a escalate/budget-exhausted, emette explicit termination Event.
   - Current: NON ESISTE.
   - Target: `internal/agent/workflow/loop.go` `type LoopAgent struct { name string; maxIterations uint; subs []Agent }`. `Run(ctx) iter.Seq2[*Event, error]` ripete sub-agents fino a (a) `maxIterations` raggiunte, (b) `event.Actions.Escalate=true` emesso da sub, OR (c) hard budget exhaustion from `ctx.Budget.ConsumeStep()`. Su (c) emette **explicit budget-exhausted Event**: `Event{Author: <loop_agent_name>, Actions: Actions{Escalate: true, StateDelta: {"termination_reason": "budget_exhausted", "limit_hit": reason, "steps_consumed": <N>}}}`. Soft-cap `"branch_soft_cap"` is never emitted as budget exhaustion by LoopAgent; it is only a fairness signal used by ParallelAgent scheduling. ~70 LOC inclusa dedup pre-check via `ctx.Budget.BeforeToolCall(...)` / result-veto via `ctx.Budget.AfterToolResult(...)`. `NewLoop(name string, maxIter uint, subs ...Agent) Agent` constructor.
...
6. **Parallel workflow agent (SC#3)**: Esegue sub-agents concurrent, errgroup + ackChan backpressure, escalate da any child cancella siblings via ctx.
   - Current: NON ESISTE.
   - Target: `internal/agent/workflow/parallel.go` `type ParallelAgent struct { name string; subs []Agent }`. `Run(ctx) iter.Seq2[*Event, error]` spawn `len(subs)` goroutines via `errgroup.WithContext(ctx.Ctx)`, ognuna chiama `sub.Run(childCtx)` con `childCtx = ctx.Child(egCtx, sub, len(subs))` (stesso `Budget *atomic.Int32` shared, distinct dedup ring, passive soft cap). Events yielded via shared `resultsChan` con `ackChan` per backpressure (synchronous: consumer must ack before producer continues). Escalate da any child → captured `cancel()` → siblings ricevono ctx.Done() → drain pulito `(nil,nil)` senza `context.Canceled` noise. ~120 LOC.
...
- ENV hardening vars nuove: `AURA_LOOP_DEDUP_EXEMPT_TOOLS`, `AURA_LOOP_BRANCH_SOFT_FRACTION`, `AURA_LOOP_NODE_TIMEOUT_SEC`, `AURA_LOOP_DEDUP_RESULT_CAP=2048` documentate in `.env.example` + `prd.md`
- Attribution/licensing: any adk-go-adapted source carries an Apache-2.0 attribution comment and the repository adds a third-party notice entry for `google/adk-go`
...
- **Coverage threshold** ≥85% (CLAUDE.md override) across the Phase 2 unit-only surface: `internal/agent`, `internal/agent/workflow`, `internal/canonicaljson`, and `cmd/aura` dry-run paths. Verificato via `go test -cover ./internal/agent/... ./internal/canonicaljson/... ./cmd/aura/...` with combined result ≥ 0.85.
...
- **No native OTel dep** — `go.mod` di Phase 2 NON include `go.opentelemetry.io/otel/trace`. UUIDv7 generato via `github.com/google/uuid` v1.6.0 (new direct dep in Phase 2; not already present in the Phase 1 require set).
- **Canonical JSON sorted keys** per dedup hash — implementazione handrolled in `internal/canonicaljson/` (no external dep), riusabile in Phase 4 conversation_turn hash, Phase 11 skill content_hash. It is an Aura-internal deterministic serializer, NOT RFC-8785 compliance.
...
- [ ] `go test -cover ./internal/agent/... ./internal/canonicaljson/... ./cmd/aura/...` reports combined ≥ 0.85 statement coverage across the Phase 2 unit-only surface.
...
- [ ] `.env.example` updated with `AURA_LOOP_MAX_STEPS=25`, `AURA_LOOP_MAX_WALLCLOCK_SEC=300`, `AURA_LOOP_DEDUP_WINDOW=3`, `AURA_LOOP_DEDUP_EXEMPT_TOOLS`, `AURA_LOOP_BRANCH_SOFT_FRACTION`, `AURA_LOOP_NODE_TIMEOUT_SEC`, `AURA_LOOP_DEDUP_RESULT_CAP=2048` documented.
- [ ] go.mod adds `github.com/google/uuid` as a new direct dependency.
- [ ] Third-party notice/licensing attribution for adapted `google/adk-go` Apache-2.0 workflow patterns is present.
