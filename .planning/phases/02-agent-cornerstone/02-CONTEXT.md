...
- **D-12 — Per-branch SOFT CAP at `Budget.Child()` (prevents sibling starvation).** `softCap = ceil(remaining / fanout)` (tunable via `AURA_LOOP_BRANCH_SOFT_FRACTION`): throttles a branch toward its fair share while the shared hard counter remains authoritative. Soft-cap exhaustion is a non-terminal scheduling/fairness signal; `LoopAgent` must not emit budget-exhausted for it. Hard total bound preserved; budget closer to ~50 LOC + test, not the original ~30 LOC estimate. Closes the one gap where the flat counter (Strands ships it flat) allows a greedy branch to eat 20/25 steps and starve siblings → deep-narrow result on a breadth task. *Validated: scout-1 Risk-1 (the 88→95 lever; academic frontier DOVA uses proportional allocation for exactly this).*
...
- **D-18 — TWO-TIER / TWO-PHASE fingerprint (primary name+args, result as VETO only).** **Reverses the earlier `(name,args,result)`-in-hash choice** after scout-2 showed it is **fail-open**: any tool returning a volatile field (timestamp, page-token, request-id) never produces a repeating triple → the loop is never detected. Correct design: **primary fingerprint `sha256(name + canonical_json(args))`** — `BeforeToolCall` fires *before* re-executing (blocks side-effects), matches OpenFang/Hermes#481/Claude-Code#4277/Gemini. Bounded result preview is recorded later by `AfterToolResult` and used ONLY as a progress VETO (args repeat but result changed → progress, suppress dedup — the zeroclaw#2152 insight) → volatile results fail *safe* (look like progress) instead of fail-open. A single API that requires result preview before execution is invalid; keep pre-check and result update separate. *Validated: scout-2 (the 86→95 lever).*
