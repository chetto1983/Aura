...
| AC-12 | **Every in-scope finding closed with its named test; ≥85% owned-surface coverage; nothing dropped** | HARDEN-12 | A: ledger 64/64 + cache gate (A5). B: `make coverage` ≥85%. | A ✅ (ledger) / B ✅ (coverage 89.4%) |
...
## Part B result — OPERATOR LIVE SIGN-OFF (DONE)

Completed on the live stack:

1. **B1 coverage** — `make coverage` (destructive PG wipe): owned-surface **89.4%** ≥85%
   across the `db_integration neo4j_integration` matrix (2026-06-15 campaign).
2. **B2 quality** — `golangci-lint run ./...` = 0, `govulncheck ./...` = 0, mutation ≥70%
   on the three critical files (commit `595fc6a1`).
3. **B3 live** — `aura serve` + the acceptance matrix driven **2026-06-16** at HEAD
   `85b5e1ae` (daemon rebuilt to HEAD first): 9/9 rows PASS (host `aura chat` tool trace,
   `/metrics` scrape, CDP Telegram round-trip, GLM-OCR multimodal fs-cap, MCP reconnect,
   reasoning fallback, skill self-extension + operator alert, DSN secret boundary,
   tool-invocation ledger redaction). Evidence:
   [`docs/audit/22-LIVE-SIGNOFF-2026-06-15.md`](../../../docs/audit/22-LIVE-SIGNOFF-2026-06-15.md)
   Part B3.

Each live check recorded a ground-truth assertion that does NOT read `r.Reply` (DB row /
`· <toolname>` trace / `/metrics` line / rendered body). One low-risk follow-up logged on
row #9 (DSN `scheme://user:pw@` userinfo not yet scrubbed from `tool_invocations.args_raw`).
...
| Gate | Owner | Status |
|------|-------|--------|
| Part A — automated floor | executor (22-05) | ✅ done @ `036575b5` (re-verified @ `28c1f7c7`) |
| Part B — coverage / quality / live | operator | ✅ done (B1/B2 2026-06-15, B3 2026-06-16 @ `85b5e1ae`) |

Phase 22 is **Gate-3 CLOSED** — Part A automated floor + Part B (coverage / quality / live
sign-off) all done.
