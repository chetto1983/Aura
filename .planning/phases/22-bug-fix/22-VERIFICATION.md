...
| 8 | Unknown-tool and swarm-child output is default-untrusted and cannot launder prompt injection | VERIFIED | `trustedToolNames` explicit allowlist in `trust.go:21-28`; `untrustedSource` returns untrusted for any tool not on the allowlist; `runner_adapter.go:62` stamps `TrustUntrusted` on swarm child reports; `TestTrustDefault` + `TestSwarmRunnerAdapter` PASS (A4) |
