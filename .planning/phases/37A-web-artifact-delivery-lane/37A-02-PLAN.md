...
must_haves:
  truths:
    - "A send_file under an authenticated identity with a thread id ingests the file (via the AssetDeliverer seam) and the aura.artifact descriptor gains asset_id + size_bytes + mime_type + tool_call_id (D-01 always-ingest-when-authenticated, channel-agnostic; the new fields ride the EXISTING aura.artifact event verbatim per D-07 — no new event)"
    ...
    - "Any ingest miss — nil Assets, empty identity, empty thread id, or an IngestAgentDelivery error — degrades to the legacy {path,filename,caption} descriptor with NO asset_id, and never errors the turn (D-02 best-effort degrade; the empty-thread-id / no-ConvID case is D-05)"
  ...
  key_links:
    ...
    - from: "descriptor tool_call_id"
      ...
      via: "in-package tool-call ctx (result.go), NO internal/agent import"
      pattern: "toolCallID"
  prohibitions:
...
Output: `AssetDeliverer` + `SendFile.Assets` + a ctx-aware `emitDelivery` that ingests on BOTH tails; the `asset_id`/`size_bytes`/`mime_type`/`tool_call_id` descriptor keys; the `sendFileAssetAdapter` + `runtimeToolHandles.SendFile` + serve-boot set; a Telegram non-regression assertion.
...
- **`tool_call_id` on the descriptor (WEBART-02 / Landmine 5):** `aura.artifact`'s value is only the descriptor map and carries no correlation id, so the web reducer (37A-04) can't attach the card. Add `tool_call_id` to the descriptor. The descriptor still rides Meta→ArtifactDelta→`emitToolResultCustom` verbatim — ZERO translator/`llm_agent_events` change (Landmine 6).
...
<task type="auto" tdd="true">
  ...
  <behavior>
    ...
    - degrade matrix (each → descriptor has path but NO asset_id, ToolResult carries no Go error): (a) `Assets == nil`; (b) identityctx unset (empty identity); (c) empty sessionID (no thread); (d) `IngestAgentDelivery` returns an error
  ...
  <acceptance_criteria>
    ...
    - `go test -race ./internal/agent/tools/ -run 'TestSendFile_Degrade|TestSendFile_Ingest|TestSendFile_DescriptorFields'` exits 0; the degrade matrix has all four cases and the both-tails case is present (grep the test)
  ...
  <done>send_file ingests on both tails via the AssetDeliverer seam, enriches the descriptor with asset_id/size_bytes/mime_type/tool_call_id on success, degrades to path-only on any miss without erroring the turn, and keeps Deferred/Mutating + channel-agnostic invariants.</done>
</task>
...
- Grep guards: `send_file.go`/`send_file_ingest.go` import neither `internal/agent` nor `internal/assets`; `Spec()` returns Deferred:true/Mutating:false; the degraded descriptor has no `asset_id`.
- The channel-agnostic grep test for send_file still passes.
- go.mod/go.sum byte-unchanged.
</verification>

<success_criteria>
A channel-driven authenticated `send_file` (host path AND sandbox-routed) ingests to an owned asset and emits `asset_id`+`size_bytes`+`mime_type`+`tool_call_id` on `aura.artifact`; every failure mode degrades to path-only without erroring the turn; Telegram is unregressed; the production tool is wired, static/CLI paths are nil. Zero translator/event changes.
</success_criteria>
