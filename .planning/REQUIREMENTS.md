...
- [ ] **CONC-02**: Per-user mutex uses `TryAcquire` — notification paths (scheduler, task dispatch) never deadlock re-entering the same user's gate
...
- [ ] **BUDGET-02**: Budget check is inside the UserGate mutex region for atomicity with conversation processing and accurate per-user accounting
...
- [ ] **CLEAN-03**: Qdrant startup health validation — block startup until Qdrant `/health` passes (with configurable timeout); skip full re-embed pass if collection has `points_count > 0`
...
*Last updated: 2026-05-10 after v4.0 scope confirmation*
