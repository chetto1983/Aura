...
- [ ] **INFRA-03**: `Agent` interface + workflow agents (Sequential/Loop/Parallel) pattern stolen-not-imported da `google/adk-go` (~380 LOC). InvocationContext include `request_id` UUIDv7 per OTel correlation. Budget contract con 3 env vars (`AURA_LOOP_MAX_STEPS=25`, `AURA_LOOP_MAX_WALLCLOCK_SEC=300`, `AURA_LOOP_DEDUP_WINDOW=3`) + child budget inheritance dal parent's remaining (NON fresh). Zero goroutine leak via `goleak.VerifyNone` mandatory. [Slice 0.9 + amendments #1, #9, #15, #19]

### Core Agent (Slice 1–1.8)

- [ ] **CORE-01**: LLM client OpenAI-compat handrolled ~280 LOC (no SDK), DeepSeek-V4 via OpenRouter default (`deepseek/deepseek-v4-flash:exacto`), ToolResult pattern con preview+sidecar, SSE streaming + ctx-cancel end-to-end, OTel span per LLM call. [Slice 1 + amendment #9]
...
- [ ] **CAP-04**: KV cache builder stable-prefix + provider-aware (DeepSeek/Anthropic/OpenAI/Gemini). Architectural rule: **two system messages** — `messages[0]` cache-stable byte-identical, `messages[1]` mutable. CI job `scripts/cache_invariant_audit.sh` asserts SHA-256(`messages[0]`) constant across 20-turn replay (cross-slice). 80% cache hit target su DeepSeek-V4. [Slice 4 + amendment #16]
- [ ] **CAP-05**: Web tools — `web_search` via SearXNG container; `web_fetch` via `codeberg.org/readeck/go-readability/v2` + `JohannesKaufmann/html-to-markdown/v2`. SSRF defense: IPv6 blocklist + DNS rebinding pin. [Slice 5 + amendment #3]
