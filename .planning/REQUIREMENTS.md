...
- [x] **SEC-01**: Aura's configured secrets are redacted before persistence at every at-rest surface (turn body, spill, tool-result sidecar); unknown secret-like values in tool output remain agent working data at rest and are pattern-redacted only on the outbound surface (display, export, ledger, trace). Full reasoning-trace mode requires an explicit production warning/fail-fast, retention config, and encrypted sink. *(F-021)*
- [x] **SEC-02**: Wildcard/permissive CORS is removed; the cockpit is served same-origin only and emits no CORS headers. Introducing a cross-origin consumer later requires an explicit credentialed allowlist and a re-evaluation of the `auth.go` SameSite/CSRF posture. *(F-022)*
- [x] **SEC-03**: The integration validation console refuses non-loopback bind unless an explicit unsafe flag + authentication are configured (logs a warning). *(F-047)*
- [x] **SEC-04**: A prompt-injection/tool-policy-bypass regression suite proves that destructive or irreversible shell/file/MCP actions are denied or withheld under `server_production`, while ordinary mutations remain usable only through the shipped containment, tenant-scope, egress, durable-policy, and idempotency boundaries. A separate manual live-LLM tier measures model resistance without pretending the enforcement gate detects prompt provenance. *(F-019 security part)*
- [x] **SEC-05**: CI publishes an SBOM (syft / cyclonedx-gomod), `govulncheck` is a blocking gate, and all third-party Actions + tool versions are SHA/exact pinned; a workflow-lint gate rejects `@latest`/semver-floating refs. *(F-051)*
- [x] **SEC-06**: Privileged JSON routes (`/agent/run`, approvals resolve, onboarding, assets, governance writes) use strict decoding — size cap, content-type check, `DisallowUnknownFields`, single-decode EOF, per-route `allowEmpty`. *(F-052)*
