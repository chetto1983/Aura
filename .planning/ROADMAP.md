...
- [x] 37F-01-PLAN.md — PRD-amendment (WEBSHARE-01..04 + i tre tier + snapshot model + `shared_links`/`share_audit` + amendment D-08 reasoning DROPPED e D-13 hash-indexed equality) + ADR 0039 (sharing vs. identity isolation)
...
- [x] 37F-07-PLAN.md — `shared_links` store (lazy fail-closed token predicate = the security gate) + `share_audit` + the 4th admin audit-union leg
- [x] 37F-14-PLAN.md — `ShareToggle` in the floating cluster 37B reserved in code + `useSharePanel` + AppShell refactor-on-touch (R-02)
- [x] 37F-16-PLAN.md — Public read-only page `/s/{token}` (null-origin iframe, no owner PII, no oracle, renderers reused unedited)
...
**Goal:** Idempotent mutating tools + a production observability surface. Allocate migrations from the live on-disk head (verified `0042_drop_compaction`; expected Phase 39 slots `0043+`); never reuse historical migration `0026`.
...
- [ ] 39-01-PLAN.md — Durable identity-scoped public-operation registry, live-next migration, sqlc store, replay/conflict/in-progress/indeterminate semantics
