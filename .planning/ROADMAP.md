...
- 🔨 **v2.0.0 Industrial Hardening & Multi-User Production** — Phases 31–39 (in planning) — close the 51-finding audit to an honest 10/10 via per-user full-capability sandbox + multi-user identity isolation + ToolGateway + observability/security/ops industrialization
...
### 🔨 v2.0.0 Industrial Hardening & Multi-User Production (Phases 31–39) — IN PLANNING

**Goal:** Close the entire 2026-06-21 industrial audit (51 findings, F-001..F-052) to an **honest 10/10** production-readiness (from 4.6/10) — via a per-user full-capability sandbox (Docker, resolving F-001 without stripping the full-host surface), multi-user identity isolation (no RBAC), Authula cutover, a central ToolGateway, and observability/security/ops industrialization. Every hardening behavior is a **no-op under `dev`/`local_trusted`** — the operator's daily full-host experience is unchanged; hardening activates under `server_production`. Dependency rule: profiles + ledger → gateway → identity → sandbox → MCP → idempotency/obs → security → ops/eval. Requirements + traceability in [`REQUIREMENTS.md`](REQUIREMENTS.md); research in [`research/SUMMARY.md`](research/SUMMARY.md).

- [ ] **Phase 31: Runtime Profiles + Config Validation** (keystone) — `PROF-01..06` (F-002/007/016/018/026/041)
  - Goal: 4 validated profiles (`dev`/`local_trusted`/`single_user_hardened`/`server_production`) in `internal/config`; production fails fast on unsafe defaults.
...
- [ ] **Phase 32: Agent-Loop Correctness + Durable Ledger** — `LOOP-01..11` (F-003/004/005/009/010/029/030/031/040/045/048)
...
- [ ] **Phase 33: ToolGateway + Policy Engine** — `GATE-01..04` (F-001 gateway, F-006/011/020)
...
- [ ] **Phase 34: Multi-User Identity Isolation + Authula Cutover** — `MUSR-01..06` (F-012/028/032/039/050)
  - Goal: owner-scope every user-facing store/API/job to the authenticated principal; cut over to Authula (no RBAC).
...
- [ ] **Phase 35: Per-User Full-Capability Sandbox** — `SBX-01..05` (F-001 sandbox, F-036)
...
- [ ] **Phase 36: MCP Governance Hardening** — `MCPH-01..09` (F-013/014/027/033/034/035/037/038/046)
...
- [ ] **Phase 37: Idempotency + Observability Pack** — `OBS-01..06` (F-008/017/020/023/024/049)
...
| v2.0.0 Industrial Hardening & Multi-User Production | 31–39 (9 phases) | 0/— | 🔨 In planning | — |
