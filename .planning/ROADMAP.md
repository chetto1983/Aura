...
- [x] 02-09-PLAN.md — the admin spend overview: three reconciliation calls server-side, one endpoint, a zero-dependency dashboard (wave 6)
...
**Success Criteria** (what must be TRUE):
  ...
  3. From inside a live sandbox, the host is unreachable: no Docker socket, no host filesystem outside the identity's roots, and no Aura credential in the environment of a launched stdio MCP server. The shared writable uv/npm/pip cache finding was reproduced and fixed on 2026-09-14; [cache evidence](phases/03-the-boundary-under-attack/03-CACHE-CHECK.md) records private volumes, legacy-container migration and the real pip replay test. The remaining host and credential boundaries still need the full attack suite; the cache test does not certify them.
...
| 2. Permissions Decide What a User May Do | 9/10 | In Progress|  |
