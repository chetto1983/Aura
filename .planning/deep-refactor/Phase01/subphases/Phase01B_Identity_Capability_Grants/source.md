...
| `D:/Aura/internal/telegram/access.go` | `/start`, `/login`, pending approval, and owners depend on env/persisted allowlist. | Migrate allowlisted Telegram users into channel accounts and owner/user principals. | Do not remove allowlist checks before equivalent capability tests pass. |
