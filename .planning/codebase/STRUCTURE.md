...
### `cmd/` — Entry Points
- Purpose: All executable binaries — the main application and debug/smoke-test harnesses
...
- Contains: `implementation-tracker.md` (slice-level completion tracking), `llm-wiki.md` (LLM Wiki pattern reference), `telebot-v4-monitoring.md` (dependency watchpoint), and focused reports under `docs/audits/`
- Historical plans/specs and stale early PRD/PDR/progress artifacts were removed from the active docs tree; use git history plus `docs/implementation-tracker.md` for shipped slice history.
...
- Committed: Yes — generated by `web/` and embedded into the Go binary via `//go:embed`
