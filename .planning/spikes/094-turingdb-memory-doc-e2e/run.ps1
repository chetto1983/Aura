...
$PrintingPressVersion = "{}"
try {
  $PrintingPressVersion = (cli-printing-press version --json)
} catch {
  $PrintingPressVersion = "{}"
}
$PrintingPressVersionB64 = [Convert]::ToBase64String([Text.Encoding]::UTF8.GetBytes($PrintingPressVersion))

$LogPath = Join-Path $ScriptDir "run.log"
docker run --rm `
  -e "TURINGDB_CONTAINER_IMAGE=$Image" `
  -e "PRINTING_PRESS_VERSION_B64=$PrintingPressVersionB64" `
  ...
  --out /work/.planning/spikes/094-turingdb-memory-doc-e2e/results.json 2>&1 | Tee-Object -FilePath $LogPath
exit $LASTEXITCODE
