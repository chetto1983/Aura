...
| 108 | studio-media-editing | video-studio-audio | standard | Given a three-layer video lane over two clips with audio at 30 fps — the shape the multi-track editor produces and the one spike 107 never rendered — when VideoFlow's browser renderer exports it, then the two numbers the studio design rests on are measured: decode-per-source vs decode-per-layer, and whether the 0.1 ms `sourceStart` nudge proven at 24 fps still holds at 30 | **VALIDATED** - the mixer decodes once per LAYER, never per source (3 layers/2 sources = 3 calls; 16 layers/1 source = 16), and every call decodes the WHOLE file whatever `sourceDuration` says, so a 60 s stereo clip cut into 8 costs 8 decodes, **1.91 s = 29.5 %** of a 6.5 s render and **175.8 MB** of PCM alive at once; `performance.memory` is blind to all of it (175.8 MB of buffers held moves `usedJSHeapSize` by **4 226 B**, 8/8 steps), and neither Windows process counter resolved it either. The fix needs no fork and was measured, not argued: `decodeLayerAudio` reads `layer.decodedBuffer` off ANY layer and `initLayers()` is idempotent, so priming one buffer per distinct source gives **16 decodes → 1** and **8 → 1**, render −20 % and **−40 %**, audio identical sample for sample (0 delta over 2 880 512 samples). The 1e-4 s nudge is **still required at 30 fps and worse than at 24**: 49/240 frames show the previous one without it (20.4 % vs 107's 10.4 %, up to **45 %** on a layer that starts mid-timeline), 0/240 with it. Also found: `muted` in settings is a no-op — the mixer reads `mute` in properties — and muting never saves the decode | studio, web, videoflow, audio, decode, mixer, frame-timing, 30fps |
