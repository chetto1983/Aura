...
| 106 | studio-media-editing | video-to-model | standard | Given the 8 s H.264 clip with burned-in per-frame timestamps, when it is sent as native video to llama.cpp b10951 and to an OpenRouter video model, then each backend's capability key and wire part are measured rather than guessed, and the answer proves the frames were decoded | **VALIDATED** - llama.cpp `/props` answers `{"vision":true,"video":true,"audio":false}`, so the key is literally `video` and `llamaCppModalities` needs no rename; `input_video` works on b10951 with **raw base64, no `data:` prefix**, the 8 s clip costing 15 720 prompt tokens (`truncated = 0`, 96% of a 16k context) and Qwen3-VL-2B-Q8_0 reading `00:00:00.083 f2` / `00:00:07.833 f188` correctly in **11.3 s cold, 3.74 s warm** on an RTX 3060 (`server-cuda-b10951`, `-ngl 99`; 1 656 tok/s prompt, 72 tok/s generation) against 730 s / 70.1 s on the CPU image of the same build; OpenRouter `z-ai/glm-5.3-flash` takes the opposite shape, `video_url` with a `data:video/mp4;base64,` URI, answering correctly in 11-22 s for $0.0018-0.0030 with the video billed inside `prompt_tokens`. Task 3 implements both branches; Ollama gets none | llm, video, llama.cpp, openrouter, multimodal, capabilities, wire-format |
| 107 | studio-media-editing | studio-timeline-editor | standard | Given the Studio clip, a text and a photo edited on an OSI timeline over a VideoFlow project (drag, trim, split, keyframes, speed, undo), the browser alone exports a valid MP4 with every layer in place, pitch kept at 0.5x/2x, no off-origin request | **VALIDATED** - VideoFlow renders 4 s in 0.4-0.8 s (H.264+AAC, text/keyframed photo verified by ffmpeg diff) with a same-origin `loadFont` override (0 vs 26 Google requests), a 1e-4 s `sourceStart` bias (10/96 boundary frames otherwise show n-1) and a build stub of its 147 KB-gz font registry; split = two layers over one source; dnd-timeline 9/9 desktop+touch at 26 KB gz (react-timeline-editor needs `touch-action: none`, 76 KB); immer-patch history, one entry per gesture; signalsmith-stretch (MIT) 8.000/2.000 s at 440.6/440.3 Hz, VideoFlow `pitch` fails, soundtouchjs 10% short; mediabunny packet-retime mux verified | studio, web, videoflow, timeline, undo-redo, time-stretch, mediabunny |
| 108 | studio-media-editing | video-studio-audio | standard | Given a three-layer video lane over two clips with audio — the shape the multi-track editor produces and the one spike 107 never rendered — when VideoFlow's browser renderer exports it, then the two numbers the studio design rests on are measured: decode-per-source vs decode-per-layer, and whether the 0.1 ms `sourceStart` nudge proven at 24 fps still holds at 30, rendered at BOTH rates so frame rate and composition are not confounded | **VALIDATED** - the mixer decodes once per LAYER, never per source (3 layers/2 sources = 3 calls; 16 layers/1 source = 16), and every call decodes the WHOLE file whatever `sourceDuration` says, so a 60 s stereo clip cut into 8 costs 8 decodes, **1.85 s = 32.7 %** of a 5.6 s render and **175.8 MB** of PCM alive at once; `performance.memory` is blind to all of it (175.8 MB of buffers held moves `usedJSHeapSize` by **4 226 B**, 8/8 steps) and neither Windows process counter resolved it. The fix needs no fork and was measured, not argued: `decodeLayerAudio` reads `layer.decodedBuffer` off ANY layer and `initLayers()` is idempotent, so priming one buffer per distinct source gives **16 decodes -> 1** and **8 -> 1**, decode time 140 -> 10 ms and 1 847 -> 234 ms, audio identical sample for sample (0 delta over 2 880 512 samples). The 1e-4 s nudge is **required at 24 fps and at 30 and fixes both completely**, measured on ONE composition at three configurations: without it 49/240 at 30 fps/30 fps source, 26/192 at 24/24, 6/192 at 24/30; with it **0 in all three**. Like for like at a 1:1 ratio the same lane mismatches more at 30 fps (20.4 %) than at 24 (13.5 %), but the project/source RATIO dominates the rate (24 over 30 puts only 1 frame in 4 on a source boundary) and the layers of one lane spread more than the two rates do (layer two, start 4 s with `sourceStart` 0, mismatched 3-4x as often as layer one at both rates, so the nudge must NOT be gated on `sourceStart > 0`); 107's 10/96 reproduces to the frame as this run's layer one at 24/24. Also found: `muted` in settings is a no-op — the mixer reads `mute` in properties — and muting never saves the decode | studio, web, videoflow, audio, decode, mixer, frame-timing, 30fps |
