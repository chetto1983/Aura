...
type: standard
validates: "Given a video lane of three layers over two source clips at 30 fps — the shape the multi-track editor produces and the one spike 107 never rendered — when VideoFlow's browser renderer exports it, then the two numbers the design rests on are measured rather than assumed: whether the audio mixer decodes once per source or once per layer, and whether the 0.1 ms `sourceStart` nudge proven at 24 fps still holds at 30"
verdict: VALIDATED
related: [105, 107]
...
| 2 | Does the `+0.1 ms` nudge still hold at 30 fps? | **Still needed, and worse than at 24 fps.** 49 of 240 frames repeat the previous one without it; `sourceStart + 1e-4` gives 0 of 240 |
...
```bash
# 1. Media: two 4 s clips that differ only in their sine (440 / 880 Hz), and one 60 s stereo clip.
#    Vite serves ../media as its publicDir, so the clips go there too.
...
cp out/clip-a.mp4 out/clip-b.mp4 out/clip-long.mp4 ../107-studio-timeline-editor/media/
...
`measuredAt 2026-09-20T16:58:13Z`, Chrome 153 headless on Windows.
...
3. **Decodes track layers, never sources.** Seventeen renders over fourteen scenarios:
...
4. **What it costs.** Decode time per call is tight — 8–10 ms for the 4 s mono clip over 49 calls,
   229–252 ms for the 60 s stereo one over 13. Render wall time is not: the same scenarios moved by
   ...
   |---|---|---|---|
   | `audio`, 3 layers of 4 s mono | 27 ms over 3 calls | 3.6 % | 722 / 727 / 740 ms |
   | `split-8`, 4 s mono | 65 ms over 8 calls | 12.1 % | 536 ms |
   | `split-16`, 4 s mono | 134 ms over 16 calls | 18.5 % | 723 ms |
   | `long-1`, 60 s stereo | 233 ms | 5.9 % | 3 966 ms |
   | `long-4`, 60 s stereo | 925 ms over 4 calls | 20.8 % | 4 450 ms |
   | `long-8`, 60 s stereo | **1 910 ms** over 8 calls | **29.5 %** | 6 467 ms |

   Splitting one 60 s clip into eight adds **1.68 s of pure decode** and holds **175.8 MB of PCM
...
5. **`performance.memory` cannot see any of that, measured.** The brief asked for `usedJSHeapSize`
   ...
   not on the V8 heap. The heap figures the renders do show (25.1–35.4 MB for the 4 s scenarios,
   26.8 → 77.8 MB for `long-8`) are the fetched `ArrayBuffer`s and the encoder's own allocations, not
...
7. **The nudge at 30 fps: still needed, and worse.** Both clips are the same `testsrc2` video, which
   the probe asserts rather than assumes (worst per-channel frame difference between them: 0.0000),
   so every output frame is matched to clip-a's 120 frames by argmin of the full-frame difference.
   At 30 fps the lane's expected mapping is `n < 120 → n`, `n ≥ 120 → n − 120`.

   | render | frames | mismatches | verdict |
   |---|---|---|---|
   | `audio` (no nudge) | 240 | **49** | FAIL |
   | `audio-nudge` (`sourceStart + 1e-4`) | 240 | **0** | PASS |

   Every one of the 49 is off by exactly **−1** — the previous source frame, the same defect 107
   diagnosed in `VideoFrameSource.holds(t)`. The rate is **20.4 %** at 30 fps against 107's 10.4 %
   (10/96) at 24, and it is not spread evenly:

   | layer | timeline | `sourceStart` | frames | mismatches | rate |
   |---|---|---|---|---|---|
   | one | 0–4 s | 0 | 120 | 14 | 11.7 % |
   | two | 4–6 s | 0 | 60 | **27** | **45.0 %** |
   | three | 6–8 s | 2 | 60 | 8 | 13.3 % |

   The layer that starts mid-timeline with `sourceStart` 0 — exactly what a cut produces — is four
   times worse than the one that starts at the origin. 107's 10 % was the best case.
...
8. **The fix needs no fork, and it was measured rather than argued.** `decodeLayerAudio` reads
   ...
   |---|---|---|---|---|
   | `audio-mute-prop` per-layer | 3 | 2.2 MB | 26 ms | 736 ms |
   | `audio-mute-prop` per-source | **2** | 1.5 MB | 19 ms | 670 ms (−9 %) |
   | `split-16` per-layer | 16 | 11.7 MB | 134 ms | 723 ms |
   | `split-16` per-source | **1** | 0.7 MB | 10 ms | 576 ms (−20 %) |
   | `long-8` per-layer | 8 | 175.8 MB | 1 910 ms | 6 467 ms |
   | `long-8` per-source | **1** | 22.0 MB | 232 ms | 3 905 ms (**−40 %**) |

   The output is not merely the same size — the exported MP4s are byte-for-byte the same length and
   their decoded audio is **identical sample for sample**: worst delta `0` over 384 000, 194 560 and
   2 880 512 samples respectively.
...
9. **The lane is otherwise valid.** ffprobe of the export: `8.000000 s`, `622066 B`, one video stream
   ...
   per-second audio to five decimals (the main-thread MP4 is larger, 881 791 B, as in 107).
   57 requests, **0 off-origin**, 0 console errors.
...
- **Cost:** 8–10 ms per decode for a 4 s mono clip, **229–252 ms** for a 60 s stereo one — 1.91 s,
  **29.5 %** of a 6.5 s render, for eight layers of one file.
- **A per-source cache needs no fork and changes nothing audible:** 16 decodes → 1, 8 → 1, render
  −20 % and −40 %, audio identical sample for sample.
- **The nudge is still required at 30 fps and is still `1e-4` s.** 49/240 → 0/240. Worst case 45 % of
  a layer's frames, on the layer shape a cut produces.
...
decodes, 1.9 s of the render and 176 MB of live PCM to produce audio that one 22 MB buffer already
...
`mute` per layer, so no per-layer semantics change; measured at −40 % render time on `long-8` for
bit-identical audio.

Two riders, both measured:

- **The nudge stays**, applied to every video layer's `sourceStart`, and is worth an upstream issue:
  it is worse at 30 fps than at 24, and worst on a mid-timeline layer.
...
- **Wall times are not a benchmark.** One run per scenario, on a machine also running Docker and a
  Vite dev server; the same scenario moved by up to ±40 % between runs. Only the decode counts and
  the frame-mismatch counts are exact integers, stable across every run.
- **The nudge's size.** `1e-4` s was confirmed sufficient at 30 fps; no smaller value was bisected,
  and 25 / 29.97 / 50 / 60 fps and VFR remain untested, as does whether the nudge shifts audio sync.
