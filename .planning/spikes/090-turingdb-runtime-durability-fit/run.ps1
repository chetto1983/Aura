...
docker run --rm `
  ...
  --entrypoint python `
  $Image `
  /work/.planning/spikes/090-turingdb-runtime-durability-fit/probe_runtime.py `
  --scratch /scratch/090-runtime-durability `
  --out /work/.planning/spikes/090-turingdb-runtime-durability-fit/results.json 2>&1 | Tee-Object -FilePath $LogPath
exit $LASTEXITCODE
