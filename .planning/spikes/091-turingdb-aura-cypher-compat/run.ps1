...
docker run --rm `
  ...
  --entrypoint python `
  $Image `
  /work/.planning/spikes/091-turingdb-aura-cypher-compat/probe_compat.py `
  --scratch /scratch/091-cypher-compat `
  --out /work/.planning/spikes/091-turingdb-aura-cypher-compat/results.json 2>&1 | Tee-Object -FilePath $LogPath
exit $LASTEXITCODE
