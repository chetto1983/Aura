...
func openFactGraph(ctx context.Context, password, runID string, dim int) (*factGraph, error) {
	...
	return &factGraph{driver: driver, runID: runID, index: "adaptive_bench_fact_embedding", dim: dim}, nil
}
...
func (g *factGraph) seed(ctx context.Context, facts []knowledgeFact, vectors [][]float64) error {
	...
	_, err := session.ExecuteWrite(ctx, func(tx neo4j.ManagedTransaction) (any, error) {
		if _, err := tx.Run(ctx, "MATCH (n:AdaptiveBenchFact) DETACH DELETE n", nil); err != nil {
			return nil, err
		}
		...
		if _, err := tx.Run(ctx, `
UNWIND $rows AS row
CREATE (n:AdaptiveBenchFact {id: row.id, text: row.text, run_id: $run_id, embedding: row.embedding})
WITH row, n
OPTIONAL MATCH (target:AdaptiveBenchFact {id: row.related_id})
...
`, map[string]any{"rows": rows, "run_id": g.runID}); err != nil {
	...
	_, err = session.Run(ctx, fmt.Sprintf(
		"CREATE VECTOR INDEX %s IF NOT EXISTS FOR (n:AdaptiveBenchFact) ON (n.embedding) OPTIONS {indexConfig: {`vector.dimensions`: %d, `vector.similarity_function`: 'cosine'}}",
		g.index, g.dim,
	), nil)
	if err != nil {
		return err
...
YIELD node, score
WHERE node:AdaptiveBenchFact AND node.run_id = $run_id
...
OPTIONAL MATCH (node)-[:RELATED_TO]->(related:AdaptiveBenchFact)
...
func (g *factGraph) close(ctx context.Context) {
	...
	session := g.driver.NewSession(ctx, neo4j.SessionConfig{DatabaseName: "neo4j"})
	_, _ = session.Run(ctx, "MATCH (n:AdaptiveBenchFact {run_id: $run_id}) DETACH DELETE n", map[string]any{"run_id": g.runID})
	_ = session.Close(ctx)
