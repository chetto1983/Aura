...
docker run --rm `
  ...
  --entrypoint python `
  $Image `
  /work/.planning/spikes/093-turingdb-llm-graph-access-path/probe_access.py `
  --scratch /scratch/093-access-path `
  --out /work/.planning/spikes/093-turingdb-llm-graph-access-path/results.json 2>&1 | Tee-Object -FilePath $LogPath
exit $LASTEXITCODE
