...
- ✓ **PROF-01..06, QUAL-04 (env catalog)**: Runtime profiles + config validation — 4 validated profiles (`dev`/`local_trusted`/`single_user_hardened`/`server_production`) in `internal/config`; a profile-aware `Validate()` that fails fast on unsafe production defaults (sample object-store creds, empty `GARAGE_RPC_SECRET`, permissive CORS, single-replica, destructive-shell `off`, missing web-auth, non-absolute run-dir) via a `ValidateProfile` aggregator over pure tier-gated bespoke gates (strict {hardened,prod}, prod-only replication/destructive differentiators); a generic kind-driven re-parse pass (Fatal under strict / Warn under lenient) keyed on a `KnobSpec` single-source-of-truth registry (F-016); the F-002 destructive-shell empty→defaults flip; and the operator `aura config validate --profile <p> [--json]` CLI (fail-closed, lists every unmet requirement). Verified 12/12 must-haves, golangci-lint=0, mutation ≥80% — v2.0.0 (Phase 33)
- ✓ **GATE-01..04**: ToolGateway + Policy Engine — one in-process policy-enforcement point (`gateway.Decide`) interposed in `execTool` at all 3 `NewLlmAgent` roots (runner/swarm/cron): a fail-closed mutating classifier + monotone de-escalator (GATE-01), the command-hook fail-closed default (GATE-02), a synchronous durable pre-execution reservation + idempotency replay on the append-only `aura.tool_invocations` ledger (GATE-03), and a crash-orphan reconciler that appends `end{indeterminate}` without ever re-invoking a mutating orphan (GATE-04). The interactive `approve` path is consent-bound — a server-side challenge + operator-visible question match keyed on the authenticated conversation id (CR-01 confused-deputy fix, the byte-for-byte analog of `shell_exec`'s `ApproveChallenge`). Re-verified 11/11 must-haves, code-review clean, unit `-race` + live `db_integration` green, gateway coverage 89.5% (F-001 gateway/F-006/F-011/F-020/F-031) — v2.0.0 (Phase 35)
...
- [x] ToolGateway (policy + approval + durable mutating-tool ledger) — F-001/F-011/F-020/F-031 — validated Phase 35
...
---
*Last updated: 2026-07-04 — Phase 35 (ToolGateway + Policy Engine) complete: a single in-process policy-enforcement point (`gateway.Decide`) at all 3 agent roots + fail-closed mutating classifier + command-hook fail-closed default + synchronous durable ledger reservation/idempotency + crash-orphan reconciler + a consent-bound interactive approval path (CR-01 confused-deputy fix). GATE-01..04 all validated, re-verified 11/11, code-review clean, gateway coverage 89.5%. Prior: Phase 33 (Runtime Profiles + Config Validation) — 4 validated profiles (dev/local_trusted/single_user_hardened/server_production) + profile-aware fail-fast config validator + KnobSpec env catalog + `aura config validate` CLI (PROF-01..06/QUAL-04; F-002/007/016/018/026/041), verified 12/12, golangci-lint=0. Milestone **v2.0.0 Industrial Hardening & Multi-User Production** (close 51-finding 2026-06-21 audit → honest 10/10; per-user full-capability sandbox + multi-user identity isolation + Authula cutover + ToolGateway + observability/security/ops industrialization; phases 31+). Prior: v1.0.0 Aura Deep Search Web Cockpit SHIPPED 2026-06-29 (9 phases [22–30], 45 plans, 113 tasks; audit PASSED 56/56; override_closeout w/ 6 deferred GPU/live-CI tiers; coverage 88.1%). v0.0.0 Substrate SHIPPED 2026-06-15 (24 phases, 144 plans, 233 tasks; coverage 90.3%).*
