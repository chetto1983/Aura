...
- **D-21 Boot-level mount (SUPERSEDED by existing code, spike-discovered):** boot mounting ALREADY EXISTS — `buildRegistryWithMCP` (`cmd/aura/main.go:104`) mounts every enabled server from the managed registry `~/.aura/mcp/servers.json` (`internal/mcp/managed_config.go`), managed by the shipped `aura mcp {install,add,list,doctor,tools,enable,disable,remove}` CLI (Codex commit `ae11737a`). **No `AURA_MCP_MAIL_SERVER`/`AURA_MCP_WHATSAPP_SERVER` env vars** — the managed config is the registration path (the `AURA_MCP_*_SERVER_JSON` vars remain test-tier overrides used by `internal/mcp/*_integration_test.go`). Phase 9 verifies fail-soft boot behavior (a dead server must not kill `aura chat`) rather than building the wiring. Recipes/doctor labeling stay Phase 16 (note: a basic `aura mcp doctor` already exists). **Sidecar-lifecycle study (2026-06-04, `docs/research/mcp-sidecar-lifecycle-study.md`) locks the supervision model:** fail-soft boot is currently NOT met — `buildRegistryWithMCP` (`main.go:122-124`) aborts on any single mount failure and `bootChat` (`chat.go:140-143`) exits(1); Phase 9 fixes it to per-server WARN-and-drop. Optional cheap add: lazy reconnect-on-use (one re-spawn attempt on a dead server's tool call — Claude Code's model). EXPLICIT non-goals (industry does not build them for trusted servers): periodic ping ticker, restart-on-crash supervisor, lazy mount, and ALL OpenClaw plugin-host machinery (typed RPC/mTLS/registry snapshots — that spec is a separate future capability for untrusted third-party plugins, see `docs/superpowers/specs/2026-06-02-openclaw-plugin-compatibility-design.md`). The whatsmeow bridge companion daemon: preferred = compose service with healthcheck (established sidecar pattern, image from the fork); alternative weighed in planning = single-binary daemon-free whatsapp-mcp fork (would need the read-back patch re-applied); NOT an in-process supervisor.
