...
**Status:** source-audited for US-CTX-07A/07B as of 2026-05-24.
...
| `.planning/aura-deep-refactor-decisions.json` ADR-034 observability | Trace metadata and payload artifacts are separate. | Future US-CTX-08 event rows should store metadata and redacted previews only. | Do not store raw prompt/tool payloads as default trace rows. | deferred to US-CTX-08 |
...
| --- | --- | --- |
| Live OpenRouter credentials for the bench run | available only through environment or compose secrets, never printed | US-CTX-07B must skip or block cleanly when the key is absent. |
| Exact fixture contents | not yet implemented | US-CTX-07A owns committed fixture JSON and parser tests. |
| Per-event compaction persistence schema | not selected yet | US-CTX-08 owns schema/API design after benchmark evidence exists. |
