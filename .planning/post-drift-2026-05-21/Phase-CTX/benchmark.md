...
| B-CTX-TJ-REG-1 | TokenJuice structured-tool regression repair | code tests + local DB evidence | `go test ./internal/tokenjuice ./internal/agent/governance ./internal/agent ./internal/agent/tools/... -count=1`; `go build -buildvcs=false ./...`; `go vet ./...`; read-only local SQLite probe against `data/aura.db` recent `conversations` tool rows | local `data/aura.db` rows plus regression fixtures in `internal/tokenjuice` and `internal/agent` | Structured Aura tool payloads (`file`, `web`, `skill`, `source`, `search`, etc.) pass through TokenJuice unchanged; fresh file-read content and metadata remain visible to the next LLM round; `execute_shell` still compacts long terminal output. | package tests, build, and vet pass; broad suites may only be blocked by host permissions unrelated to the patch | no fresh tool-output context rot | passed 2026-05-26; package tests/build/vet passed. `go test -buildvcs=false ./cmd/...` blocked by Windows Python logon in `cmd/aura`; `go test -buildvcs=false ./internal/...` blocked by snapshot write permissions, docker cache walk permission, and Python logon; all other packages passed when host-blocked packages were excluded. |
