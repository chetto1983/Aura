...
- 2026-05-11: Removed the legacy Pyodide runtime path and dead settings/config fields while preserving the dashboard locale system. Sandbox execution now uses the process runner, XLSX/DOCX extraction uses the sandbox extractor interface, OCR auto-enables from `MISTRAL_API_KEY`, and the dashboard settings catalog is compacted to 12 rows. Verification: `go test ./...`, `go vet ./...`, `go build ./...`, `npm run i18n:check`, and `npm run build` passed.
