...
8. **OQ-8 (multi-user horizon):** When the post-v1.0.0 multi-user milestone lands, the `identity_auth_links` table generalizes 1:N and per-identity `capability_grants` are issued. Out of scope here; flagged so the link schema is designed forward-compatible (UNIQUE on `authula_user_id`, not on `identity_id`).
