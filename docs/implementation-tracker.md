...
2026-05-10 runtime answer discipline implementation slice: implemented `.planning/phases/11-runtime-answer-discipline/PLAN.md` inside existing modules instead of adding another tool-search/orchestration layer. `cmd/debug_telegram_sandbox` now supports forbidden tool calls, forbidden final fragments, and minimum shell-call assertions, with `scripts/test-runtime-answer-discipline-smokes.ps1` covering memory natural answers, network-share capability questions, operational-status answers, and explicit raw shell requests. Telegram's hot model-visible surface no longer includes `execute_shell`; shell remains registered/discoverable through `tool_search`. Added runtime intent policy so `execute_code`/`execute_shell` are recoverably blocked for broad conversational prompts, but allowed for explicit diagnostics, commands, builds/tests, installs, calculations, file work, container/runtime checks, or raw-output requests. Terminal handling now synthesizes shell output through a no-tool finalization pass unless raw output was requested, hidden-tool rejection finalizes naturally instead of telling the user to use `tool_search`, and final-answer fallbacks reject raw memory evidence, shell metadata, tool JSON, workspace dumps, and similar markers. Runtime overlay bootstrap creates missing `SOUL.md` and `TOOLS.md` in real runtime overlay workspaces without creating or injecting `AGENT.md`/`AGENTS.md`. Verification: `go test ./internal/agentruntime ./internal/agentloop ./internal/tools ./internal/conversation ./internal/telegram ./cmd/debug_telegram_sandbox -count=1`; `go test ./...`; `go build ./...`; `go vet ./...`; `powershell -NoProfile -ExecutionPolicy Bypass -File scripts\test-runtime-answer-discipline-smokes.ps1 -SkipRawShell` passed on the local Pyodide host, with the raw shell smoke intentionally skipped because it requires process/container runtime with `execute_shell` registered.

2026-05-10 runtime answer discipline planning slice: after inspecting live Docker logs, archived Telegram turns, official OpenAI/Anthropic tool-use guidance, and local examples under `D:\tmp` (`hermes-agent`, `nanobot`, `picobot`), opened `.planning/phases/11-runtime-answer-discipline/PLAN.md`. The plan records the actual remaining failures: raw `search_memory` evidence envelopes, raw `execute_shell`/terminal dumps, hidden-tool scolding after useful evidence, and missing runtime persona/tool overlays. It keeps the fix inside existing modules (`internal/tools.Registry`, `internal/agentloop`, `internal/agentruntime`, `internal/telegram`) instead of adding another orchestration layer, and it updates `.planning/STATE.md` so the next active slice is runtime answer discipline before returning to v4.0 MCP marketplace work. Verification: docs-only planning slice; no Go/runtime checks required.

2026-05-10 natural memory answer guardrail slice: fixed the Telegram agent loop path that could leak raw `search_memory` output (`Memory evidence...`, `Evidence envelope`, scores, IDs, JSON) into the user-visible final answer when the model spent its tool budget. `agentloop.Run` now performs a no-tool finalization pass on budget exhaustion when enabled, with explicit instructions to synthesize naturally from existing tool results, and its fallback refuses raw memory evidence. Terminal command formatting now masks bare stderr and structured tool-error JSON, and failed terminal tools no longer stop the turn as terminal completions. Telegram caps `search_memory` and `tool_search` to two calls per turn to prevent retrieval spirals. The debug Telegram sandbox now preserves mounted absolute `/workspace` paths when it is itself running inside a Linux/container workspace, avoiding misleading host/temp workspace smokes. Verification: `go test ./internal/agentruntime ./internal/agentloop ./internal/telegram ./cmd/debug_telegram_sandbox`; Docker test-container `cmd/debug_telegram_sandbox` for the PMS offer-request memory scenario passed with natural Italian final text, `tool_calls_count=2`, `llm_calls=3`, no shell call, and no raw evidence envelope; `go test ./...`.

2026-05-10 agent-loop guardrails and smoke metrics slice: accepted and cleaned the existing loop guardrail edits after inspecting live logs and archived conversations. The post-tool-search runtime was already much lighter (core tools only, simple turns around 4.3k prompt tokens), but logs showed the remaining failure mode was behavioral: hidden `read_file` guesses, shell syntax errors, and multi-turn stalls. `agentloop.Run` now supports tiered budgets (`simple_qa`, `orchestration`, `code_exec`) that start small and expand only after `tool_search` or sandbox tools are actually selected. Hidden-tool rejection can fire a spiral breaker with direct `tool_search` guidance instead of spending more model calls. Retry guidance is counted only from real tool error results and no synthetic unmatched `tool` message is inserted, preserving function-call protocol safety. Telegram logs/snapshots and debug smokes now carry `retry_nudges_sent`, `spiral_breaker_fired`, and `tiered_budget_tier`; the debug CLI can assert those fields. Shell syntax errors now hint to use Python via `execute_code`. Verification: `go test ./internal/agentloop ./internal/telegram ./cmd/debug_telegram_sandbox -count=1`; `go test ./...`; `go build ./...`; `go vet ./...`; `docker compose config --quiet`.

2026-05-10 tool search regression smokes slice: added debug smoke counters (`tool_search_calls`, `execute_code_calls`, `execute_shell_calls`, `model_visible_tool_count`, `tool_result_context_chars`, `tool_results_compacted`, `internal_orchestration_tool_calls`) and budget gates (`-expect-visible-tools-max`, `-expect-tool-search-calls-max`, `-expect-execute-code-calls-min`, `-expect-tool-result-context-chars-max`) to `cmd/debug_telegram_sandbox`. Counters populate from message inspection in `debugTextSmokeResultFromMessages`; gates enforce runtime budgets in `validateDebugExpectations`. Created `scripts/test-agent-tool-search-smoke.ps1` with three Docker-first smokes: tool_search discovery, execute_code orchestration, and container diagnostic with Python. Updated v4.0 MCP plan with integration rules: MCP tools searchable only after review-gated registration, tool_search is discovery not enablement. Verification: `go test ./cmd/debug_telegram_sandbox ./internal/telegram -count=1`; `go test ./...`; `go build ./...`; `go vet ./...`.

2026-05-10 hybrid tool search backend slice: added embedding/vector tool search backend behind the existing lexical `Registry.Search`. Config now supports `TOOL_SEARCH_BACKEND=fts|vector|hybrid` (default `fts`) and `TOOL_SEARCH_TOP_K` (1-10, default 5). `internal/tools/registry_search_vector.go` provides `ToolVectorIndex` using the dedicated embedding config (`EMBEDDING_API_KEY`/`BASE_URL`/`MODEL`) and Qdrant for vector storage; the index embeds tool definitions and queries via OpenAI-compatible `/embeddings`, stores points in the `aura_tool_search` Qdrant collection, and tolerates missing embedding keys by falling back to FTS with a health warning. `Registry.SetVectorIndex` wires the index into the registry, and `Registry.Search` now merges lexical + vector scores when available with deterministic name-based tie-breaking. `Registry.ToolVectorHealth()` exposes the index backend, doc count, last rebuild time, last error, embedding model, and fallback status for `/status` and health rollups. Verification: `go test ./internal/tools -run "TestToolVector|TestNewToolVector|TestRegistrySetVectorIndex|TestRegistrySearch|TestToolSearch" -count=1`; `go test ./internal/config -run "TestLoadToolSearch|TestNormalizeToolSearch" -count=1`; `go test ./...`; `go build ./...`; `go vet ./...`.

2026-05-09 programmatic execute_code orchestration slice: added the bounded manifest MVP for Programmatic Tool Calling without introducing a new runtime layer. `execute_code` now documents and accepts `tools_allowed` plus `max_calls`; scripts can write `/tmp/aura_out/aura_tool_calls.json` with internal tool requests, and Aura filters that control file out of normal artifacts before persistence or Telegram delivery. Internal calls execute server-side through the existing `internal/tools.Registry` only after validation against both the script's `tools_allowed` list and the active model-visible turn allowlist threaded through tool context by Telegram. Recursive/control tools (`execute_code`, `execute_shell`, `tool_search`) are blocked from internal orchestration. Regular artifacts still persist/deliver as before. Verification: `go test ./internal/tools -run "TestExecuteCodeTool" -count=1`; `go test ./internal/telegram -run "TestModelToolNames|TestToolSearch|TestDebugSmoke|TestOnTools" -count=1`; `go test ./internal/tools ./internal/telegram ./internal/sandbox ./cmd/debug_telegram_sandbox -count=1`; `go test ./...`; `go build ./...`; `go vet ./...`; `docker compose config --quiet`.

2026-05-09 registry-native tool_search slice: rewrote the planned tool search slice to stay inside existing modules instead of adding `internal/toolsearch`. `internal/tools.Registry` now owns lexical capability search through `Registry.Search(query, limit, excluded...)`, deriving searchable text from existing canonical tool definitions, categories, schemas, and examples. Added the `tool_search` LLM tool as a thin wrapper over the registry and registered it after all runtime tools are available, excluding itself from results. Telegram's initial model-visible surface is now only registered core tools (`search_memory`, `schedule_task`, `tool_search`, `execute_code`, `execute_shell`). The existing agent loop gained a dynamic `ToolsProvider`, and Telegram expands the active turn allowlist after `tool_search` results so a discovered MCP/native tool can be called on the next LLM pass without exposing every tool upfront. Prompt copy now tells Aura to discover hidden capabilities with `tool_search` and prefer code/shell orchestration for multi-step work. Verification: `go test ./internal/tools ./internal/agentloop ./internal/agentruntime ./internal/telegram -count=1`; `go test ./...`; `go build ./...`; `go vet ./...`.

2026-05-09 runtime restart/shell telemetry acceptance slice: reviewed the pre-existing user/runtime edits and accepted them as coherent with the container-first autonomy work. The dashboard/API restart path now has a 30s in-process cooldown so repeated restart requests fail fast instead of spawning multiple replacement processes. Agent loop telemetry counts both `execute_code` and `execute_shell` as sandbox usage. `execute_shell` now always emits a blank line between metadata and stdout, which keeps terminal formatting predictable even for empty stdout. Terminal execute formatting now treats metadata-only shell results such as `exit_code:` as completed sandbox work while still preserving scalar results such as `5050`. Telegram conversation telemetry renamed `tokens_used` to `tokens_lifetime` to clarify that the value is cumulative for the conversation context. The tracked local Claude settings now allow the explicit `docker exec aura-aura-1 ...` debug command used by the container runtime workflow. Verification: `go test ./cmd/aura ./internal/agentloop ./internal/agentruntime ./internal/tools ./internal/telegram -count=1`; `go test ./internal/api -run TestRestart -count=1`; `go test ./...`; `go build ./...`; `go vet ./...`.

2026-05-09 tool-result context hygiene slice: added deterministic post-turn compaction for completed tool results. `conversation.Context.CompactCompletedToolResults` preserves `tool` messages and `ToolCallID` values, only compacts results that already have a later assistant message, keeps the newest two compactable tool results full in Telegram turns, and replaces older large or secret-like payloads with bounded previews. Secret-like lines containing API key, token, password, authorization, or bearer markers are redacted. Telegram now runs compaction after archive writes and before async context enforcement, so archive behavior remains unchanged while the next prompt carries less raw tool output. Verification: `go test ./internal/conversation -run TestCompactCompletedToolResults -count=1`; `go test ./internal/conversation ./internal/telegram -count=1`; `go test ./...`; `go build ./...`; `go vet ./...`.

2026-05-09 Telegram command/reset slice: added `SessionStore.Clear(userID)` so Aura can delete a single sender's in-memory conversation context, active marker, and runtime snapshot without touching archive history, wiki/source memory, or SQLite rows. Telegram now registers `/help`, `/clear`, `/reset`, and `/tools`; bot startup installs a real Telegram command menu with `/start`, `/status`, `/help`, `/clear`, `/tools`, and `/login`. `/clear` and `/reset` are allowlist-gated and reply with a short confirmation. `/tools` reports the actually model-visible tools for the current runtime without pretending the future `tool_search` slice is already registered. Verification: `go test ./internal/agentruntime -run TestSessionStoreClearDeletesConversationActiveAndSnapshot -count=1`; `go test ./internal/telegram -run "TestBotCommands|TestInstallBotCommands|TestOnClear|TestOnHelp|TestOnTools" -count=1`; `go test ./internal/agentruntime ./internal/telegram -count=1`; `go test ./...`; `go build ./...`; `go vet ./...`.

2026-05-09 agent tool-search/programmatic execution planning slice: created `.planning/phases/10-agent-tool-search-programmatic-execution/PLAN.md` as the companion runtime plan for the user's requested architecture. The plan records official Anthropic/OpenAI references, Aura's current code baseline, immediate Telegram command gaps, protocol-safe tool-result compaction, searchable runtime tool catalog, `tool_search`, small hot model-visible tool surface, bounded `execute_code` orchestration, future hybrid embeddings, and Docker regression smokes. The v4.0 MCP plan now links to this companion plan and states the integration rule that MCP/provider tools are searchable only after Aura's review-gated registration enables them. Verification: docs-only planning slice; no Go/runtime checks required.

2026-05-09 runtime shell autonomy slice: extended the process sandbox runtime beyond Python-only `execute_code` with a process-only `execute_shell` tool. `sandbox.Manager` now exposes a narrow `CommandExecutor` boundary only when the backend is the direct process runtime; Pyodide/container runtimes do not register shell execution. `ProcessRunner` runs shell commands in the configured Aura workspace with the same timeout/output/artifact limits and inherited container environment, and the Telegram terminal-tool path treats `execute_shell` like `execute_code` so one-shot commands can complete without another model loop. The Docker image now installs pip plus operational CLI tools (`curl`, `file`, `git`, `jq`, `procps`, `python3`, `python3-pip`, `python3-venv`, `ripgrep`, `sqlite3`, `unzip`, `wget`, `xz-utils`, `zip`) and sets `/data/.local/bin`, `PIP_CACHE_DIR=/data/.pip`, `PIP_USER=1`, and `PIP_BREAK_SYSTEM_PACKAGES=1` so `pip install ...` lands in Aura's non-root user directory instead of failing on Debian's externally managed Python. Container docs now document the shell surface and smoke command. Verification: `go test ./internal/sandbox ./internal/tools ./internal/telegram ./internal/toolsets -count=1`; `go test ./...`; `go build ./...`; `go vet ./...`; `docker compose config --quiet`; `docker compose up -d --build aura`; live container checks confirmed `SANDBOX_RUNTIME_MODE=process`, Python 3.11.2, pip 23.0.1, git 2.39.5, ripgrep 13.0.0, jq 1.6, sqlite3 3.40.1, curl 7.88.1, file 5.44, zip/unzip present, `/data/.local/bin` on PATH, `/data/.pip` cache, `pip install --dry-run rich` resolves packages without PEP 668 failure, direct Python smoke `sum(range(101)) == 5050`, and Aura healthy on `127.0.0.1:18080`.

2026-05-09 runtime prompt/process execution repair slice: traced the live failure through `data/logs/aura-2026-05-09.log` and Compose logs. The bad turn used `execute_code` through the Pyodide sidecar and failed on `/tmp/aura_out`, while normal turns were still paying for `prompt_modules=base,runtime,registered-tools,overlay,skills`. `AGENT.md` is now runtime workspace guidance only: `internal/conversation.LoadPromptOverlay` injects `SOUL.md`, `USER.md`, and `TOOLS.md`, but deliberately skips both `AGENT.md` and development-only `AGENTS.md`. Added a `process` sandbox runtime that runs `python3` directly in the Aura container with the same mounted `/workspace` access as Aura, collects `/tmp/aura_out` artifacts, and remains compatible with the existing `execute_code` tool boundary. Docker now installs Python, sets `SANDBOX_RUNTIME_MODE=process`, removes the `pyodide` service/dependency, and docs/examples no longer describe the sidecar as the default. Verification: `go test ./internal/conversation ./internal/sandbox ./internal/tools ./internal/telegram ./cmd/aura -count=1`; `go test ./...`; `go build ./...`; `go vet ./...`; `docker compose config --quiet`; `docker compose up -d --build --remove-orphans aura`; container checks confirmed no `pyodide` service, `SANDBOX_RUNTIME_MODE=process`, Python 3.11.2 available, `/health` alive, and logs show `sandbox process runtime available, execute_code enabled`.

2026-05-09 v4.0a database MCP read-only policy and setup refactor slice: mirrored the mail MCP cleanup for database without copying another connector-specific god class. Added `internal/api/managedMCPServer` as the reusable guided-setup boundary for command resolution, existing config lookup, preservation, and atomic `mcp.json` upsert. Moved MCP model-exposure rules out of Telegram into `internal/mcppolicy`, reusing the same mail send/IMAP mutation lists in the dashboard manifest and adding database read/write policy lists. The real ExecuteAutomation database MCP server advertises 10 tools; Aura now exposes only the four reviewed read tools (`list_tables`, `describe_table`, `read_query`, `export_query`) and hides write/schema/insight tools (`write_query`, `create_table`, `alter_table`, `drop_table`, `append_insight`, `list_insights`). Verification: `go test ./cmd/debug_telegram_sandbox ./internal/mcppolicy ./internal/api ./internal/telegram -count=1`; `go test ./internal/mcp ./internal/config -count=1`; `go test ./...`; `go build ./...` passed with a Windows stat-cache permission warning; `go vet ./...`; `docker compose up -d --build aura`; live `/status` ok; container logs show database MCP `tools=10 aura_tools=4`; LLM debug E2E called `mcp_database_list_tables`, `mcp_database_describe_table`, and `mcp_database_read_query` against `/workspace/tmp/mcp-e2e.db` and returned `Ada/Roma` plus `Bruno/Milano`.

2026-05-09 v4.0a mail MCP write flag repair slice: fixed the log-confirmed mail deletion failure where Aura exposed reviewed IMAP mutation tools, but the real `mail-mcp` server rejected them with `write tools are disabled; set MAIL_IMAP_WRITE_ENABLED=true`. New mail setup saves now write both `AURA_MAIL_*_ENABLE_IMAP_MUTATIONS=true` and `MAIL_IMAP_WRITE_ENABLED=true`; Telegram startup normalizes legacy mail MCP configs by injecting the server-required write flag before launching the stdio server; and the mail tool exposure policy now also honors the native `MAIL_IMAP_WRITE_ENABLED` flag for manual configs. Patched the live ignored `runtime-workspace/mcp.json` without printing secrets, rewrote it as UTF-8 without BOM after Docker surfaced a Linux JSON parse failure from PowerShell encoding, and rebuilt/restarted Aura. Verification: `go test ./internal/api ./internal/telegram -run "TestMCPMailSetup|TestMCPToolEnabledForAuraMailPolicy" -count=1`; `go test ./internal/api ./internal/telegram -count=1`; `go test ./internal/mcp ./internal/config -count=1`; `docker compose up -d --build aura`; live `/status` ok; container logs show `MCP server registered` for `server=mail` with `tools=30` and `aura_tools=30`.

2026-05-09 v4.0a hard orchestrator deletion slice: physically removed the live `internal/orchestration` package and took it out of Telegram/agentruntime instead of adding another bypass. The normal chat path now composes one lightweight prompt, exposes the registry's tool definitions directly, and lets the model choose tools without text-intent routing, toolset selection, before/after orchestration hooks, document-route tool suppression, or autonomous-category merging. Workspace file tools are registered into the same model-visible registry as MCP/web/source/document/scheduler/sandbox tools, so Aura can write files when the workspace tools are enabled. Removed `AURA_TOOLSET_MODE` and `AURA_ORCHESTRATION_LOG_LEVEL` from config/settings/dashboard/Compose/examples, renamed terminal policy semantics to `on/off`, and rebuilt the embedded dashboard assets. Verification: `go test ./internal/telegram ./internal/agentruntime ./internal/config ./internal/settings ./internal/api ./cmd/debug_telegram_sandbox -count=1`; `npm --prefix web run i18n:check`; `npm --prefix web run build`; `go test ./...`; `go build ./...`; `go vet ./...`; `docker compose config --quiet`; `docker compose up -d --build aura`; live `/status` ok.

2026-05-09 v4.0a model-led tool surface slice: reduced the intent-orchestrator bottleneck by moving normal capability exposure from hardcoded default routing toward registry metadata. Tools can now be tagged with the `autonomous` category, and Telegram appends registered autonomous tools to the model-visible allowlist each turn. Configured `web_search`/`web_fetch`, source read/list/lint, and tool-registry read/list are tagged autonomous at registration, so Aura can choose them directly without keyword routing. Mail MCP exposure was tightened at the same boundary: read/search/list/verify mail MCP tools can be autonomous, while send/delete/move/bulk mutation tools remain registered but not model-visible by default. The default loop no longer treats `search_memory` as terminal, so a memory lookup can be followed by another exposed tool such as web search when the model decides it is needed. Verification: `go test ./internal/tools ./internal/telegram ./internal/orchestration -count=1`; `go test ./...`; `go build ./...`; `go vet ./...`; `docker compose up -d --build aura`; live `/status` ok; live `conversation complete` showed `tools_exposed` includes `web_search,web_fetch` plus read-only mail MCP tools, excludes mail send/delete/move/bulk tools, and reduced the prompt from the previous ~17k-token all-MCP exposure to ~8k tokens.

2026-05-09 v4.0a MCP tool exposure repair slice: fixed the log-confirmed gap where the mail MCP server was connected and registered (`tools=30`, `aura_tools=30`) but normal Telegram turns still exposed only `search_memory,schedule_task`, causing Aura to tell the user it had no MCP access. The runtime now appends registered MCP tools to the active turn from the tool registry itself, not from user-text regexes or keyword cues. MCP tools declare the `mcp` category through the tool definition layer, and Telegram asks the registry for enabled tools in that category before composing LLM tool definitions. This follows the Picobot/Hermes/Nanobot pattern: configured tools become model-visible tools; orchestration does not guess from phrases such as "mail". During Docker verification `/status` exposed a separate derived-index issue: `compact_memory_fts` could make SQLite integrity report degraded even after the compact memory rebuild. Added a compact-memory FTS rebuild from canonical `compact_memory_documents`, plus startup repair/recheck only for that derived FTS table. Verification: `go test ./internal/tools ./internal/telegram -count=1`; `go test ./internal/orchestration -count=1`; `go test ./internal/memoryindex ./cmd/aura -count=1`; `go test ./...`; `go build ./...`; `go vet ./...`; `docker compose up -d --build aura`; container logs show the mail MCP server registered and `/status` ok after rebuild.

2026-05-09 v4.0a user-controlled mail MCP tools slice: made advanced mail actions genuinely end-user configurable instead of permanently blocked. The mail setup API now persists and reports `enable_imap_mutations`, and the dashboard exposes a clear advanced toggle for IMAP move/delete/flag updates alongside the existing SMTP send/reply/forward toggle. Aura's Telegram tool registration now filters mail MCP tools from the LLM registry by the saved user settings: SMTP tools require SMTP configuration, IMAP mutation tools require the explicit mutation toggle, while non-mail MCP servers are unchanged. `/mcp/providers` is now dynamic: configured/connected mail and database status updates the provider card, capability enabled flags, risk badges, setup hints, approved tools, and blocked tools. Mail probe verifies the real configured account through `imap_verify_account`, and the mail search/read adapter now injects the configured account id by default so end users do not need to know internal account aliases such as `davide`. Verification: `go test ./internal/api ./internal/telegram -count=1`; `npm --prefix web run i18n:check`; `npm --prefix web run lint`; `go test ./...`; `npm --prefix web run build`; `go build ./...`; `go vet ./...`; `docker compose up -d --build aura`; live `/api/mcp/setup/mail` reported configured/connected with SMTP and IMAP mutations enabled; live `/api/mcp/providers` reported `mail-mcp` status `enabled`, 5 enabled mail capabilities, 16 approved tools, and 0 blocked tools; live mail MCP probe passed; live read-only `mail/search` against `INBOX` returned 1 message through `imap_search_messages`; logs show `tools=30 aura_tools=30` because the user enabled both advanced groups.

2026-05-08 v4.0a focused MCP end-user setup slice: pruned the MCP marketplace shortlist down to the two useful managed connectors, `mail_mcp` and `database`, removing the Google Workspace/Gmail fallback/Outlook/extra example clutter from the provider surface. Added guided `GET/POST /mcp/setup/database` API support that writes the real ExecuteAutomation database MCP server into the runtime MCP config for SQLite, PostgreSQL, MySQL, or SQL Server, preserves unrelated MCP entries, keeps existing passwords when the form secret is blank, masks secrets in responses, reports restart/binary state, and probes the configured `database` alias. The dashboard `/mcp` page now renders database setup alongside the existing mail wizard so end users configure host/path/credentials without editing raw JSON. The Docker runtime now installs both real MCP binaries: pinned `mail-mcp` and `@executeautomation/database-server` under Node 22 as `/usr/local/bin/ea-database-server`; `mcp.example.json` now documents only the managed mail and database entries. Verification: `go test ./internal/api -run "TestMCPProviders|TestMCPMailSetup|TestMCPDatabaseSetup|TestMCPProvider" -count=1`; `go test ./internal/api ./internal/mcp ./internal/telegram -count=1`; `go test ./...`; `go build ./...`; `go vet ./...`; `npm --prefix web run i18n:check`; `npm --prefix web run lint`; `npm --prefix web run build`; `docker compose config --quiet`; `docker compose up -d --build aura`; live runtime checks for `mail-mcp`, `node --version`, and `ea-database-server`; Playwright E2E `AURA_DASHBOARD_URL=http://127.0.0.1:18080 npx playwright test e2e/mcp-connectors.spec.ts --reporter=line`.

2026-05-08 v4.0a end-user mail MCP setup slice: replaced the manual `mcp.json` mail path with a guided dashboard setup surface and shipped the real `mail-mcp` runtime in the Docker image. The API now exposes `GET/POST /mcp/setup/mail`, validates Gmail/Outlook/custom IMAP inputs, writes only the managed `mail` entry inside the runtime MCP config while preserving other servers, masks secrets in responses, supports "keep existing secret" edits, reports connected/restart/binary-present state, and keeps MCP loading at process startup. The Docker runtime now installs the official `tecnologicachile/mail-mcp` v0.4.5 Linux binary with SHA256 pinning at `/usr/local/bin/mail-mcp`, so end users do not need to place binaries in `/workspace`. The `/mcp` connector card now includes an end-user mail form with provider presets, read-only IMAP defaults, optional reviewed SMTP prep, save action, and the existing restart action; raw MCP JSON remains only in diagnostics. Embedded dashboard assets were rebuilt. Verification: `go test ./internal/api -run "TestMCPMailSetup|TestMCPProviders|TestMCPProvider" -count=1`; `go test ./internal/api ./internal/mcp ./internal/telegram -count=1`; `npm --prefix web run i18n:check`; `npm --prefix web run lint`; `npm --prefix web run build`; `go test ./...`; `go build ./...`; `go vet ./...`; `docker compose up -d --build aura`.

2026-05-08 v4.0a real MCP mail server wiring slice: aligned the mail connector path with real MCP runtime instead of adding Aura-specific plugin machinery. `mcp.json` stdio entries now support an `env` map, and Telegram startup passes those env vars to `mcp.NewStdioClient`, so real servers such as `tecnologicachile/mail-mcp` can receive IMAP/OAuth configuration through the existing MCP process path. Provider manifests now include accepted MCP server aliases, allowing `mail-mcp` to probe a configured server named `mail` or `mail-mcp`. `mcp.example.json` now includes a read-only mail server template with IMAP env only; write/send flags remain absent. Verification: `go test ./internal/mcp ./internal/api ./internal/telegram -run "TestLoadServersValid|TestMCPProviderProbeMailMCPFindsConfiguredMailAlias|TestMCPProviderMail" -count=1`; `npm --prefix web run i18n:check`; `npm --prefix web run build`; `go test ./...`; `go build ./...`; `go vet ./...`.

2026-05-08 v4.0a mail provider probe/read adapter slice: added the first executable mail connector contract behind the MCP marketplace surface. `POST /mcp/providers/{id}/actions/probe` now checks a connected MCP server whose name matches the provider id, reports approved search/read tools, and flags blocked tools such as send/delete when they are advertised. Added read-only canonical mail endpoints for `mail.search` and `mail.read`, currently mapping `mail-mcp` IMAP/EWS tools behind explicit allowlists, with token/password/secret-like text redaction before API responses. The dashboard Probe button now calls the backend and renders ready/missing capabilities plus blocked advertised tools; enablement and send/delete remain deferred. Embedded dashboard assets were rebuilt. Verification: `go test ./internal/api -run "TestMCPProviderMail|TestMCPProviders" -count=1`; `go test ./internal/api -count=1`; `npm --prefix web run i18n:check`; `npm --prefix web run build`; `go test ./...`; `go build ./...`; `go vet ./...`.

2026-05-08 v4.0a connector configuration frontend slice: added the first mail-first MCP marketplace implementation surface without enabling any provider tools by default. The API now exposes read-only connector provider manifests at `/mcp/providers` for the approved shortlist: generic `mail-mcp`, Google Workspace, Gmail dedicated fallback, Outlook Assistant, and ExecuteAutomation Database in a read-only business profile. The dashboard `/mcp` panel now opens as a connector configuration surface with tabs for Connectors, Installed, Health, Review Queue, and Raw MCP diagnostics; the old raw MCP server/tool/schema/manual invoke UI remains available only under the Raw MCP tab. Frontend DTOs and API client methods were added for provider summaries, and Italian/English locale keys were updated. Verification: `go test ./internal/api -count=1`, `npm --prefix web run i18n:check`, and `npm --prefix web run build`.

2026-05-08 Runner Boundary default-tool diet closure: cut the default toolset down to `search_memory` and `schedule_task`, moved `daily_briefing`, `list_tasks`, and `cancel_task` out of the hot path into explicit admin/ops, and removed stale prompt guidance that advertised workspace/web/document/admin tools when they were not exposed. Default `search_memory` is now capped to 3 results and treated as a terminal tool, so Aura performs one compact retrieval and then finalizes with a no-tool answer instead of bouncing through repeated retrieval or hidden workspace calls. Added `scripts/test-runner-boundary-smokes.ps1` with status, memory, document, and admin briefing smokes. Verification: targeted tests passed; smoke results were status `tool_calls_count=1`, `tools_called=search_memory`, `elapsed_ms=10401`; memory `tool_calls_count=1`, `tools_called=search_memory`, `elapsed_ms=10195`; document `tool_calls_count=1`, `tools_called=create_docx`, `elapsed_ms=3826`; admin briefing `tool_calls_count=1`, `tools_called=daily_briefing`, `elapsed_ms=6095`.

2026-05-08 Runner Boundary implementation slice: moved active conversation lifecycle from Telegram-owned `active`/`ctxMap` maps into `internal/agentruntime.SessionStore`, with tests for context creation/reuse and active marker cleanup. Runtime debug snapshots now live in `agentruntime.SessionStore` as `agentruntime.Snapshot`; Telegram no longer owns `orchMap`, and `RunDebugTextSmoke` reads canonical runtime snapshot/session state. Extended `agentruntime.Result` with prompt/toolset/tool exposure and retrieval-capsule metadata, added explicit `llm_calls` and `tool_calls_count` to debug smoke output, and added `-expect-llm-calls-max` / `-expect-tool-calls-max` gates so broad smokes can enforce real loop budgets. Generic terminal result formatting, raw-JSON/tool-markup safety checks, and no-tool terminal LLM finalization now live in `internal/agentruntime/terminal.go`, with Telegram keeping delivery behavior. Compact-memory Qdrant mirror sync now has a thread-safe `memoryindex.VectorHealthTracker`, wired into startup sync, `/api/health` as `compact_memory`, and Telegram `/status`. Verification: `go test ./internal/agentruntime ./internal/agentloop ./internal/orchestration ./internal/telegram ./internal/memoryindex ./internal/api ./cmd/debug_telegram_sandbox -count=1`.

2026-05-08 Runner Boundary & Health Hardening planning slice: opened `.planning/phases/09-runner-boundary-health-hardening/PLAN.md` as v3.3 before v4.0. The plan uses `subagent-driven-development` and targets the remaining ADK-style boundary work: move generic session lifecycle and terminal-tool finalization out of Telegram into `internal/agentruntime`, make runner events/results canonical for debug smokes, expose compact-memory Qdrant mirror health through health APIs, and enforce broad non-document latency/tool-call budgets with Docker-first smokes. `.planning/STATE.md` and `.planning/ROADMAP.md` now mark v3.3 active while keeping v4.0 MCP Marketplace as the next milestone.
...
2026-05-08 runner boundary health hardening closure slice: merged the runner-boundary work to `master`, rebuilt the Docker runtime, and verified container health after removing keyword routing from the hot path. `AURA_TOOLSET_MODE=auto` now uses the default toolset instead of inferring compute/document/admin from substrings; skill manifest loading is cached and always available as progressive-disclosure metadata; swarm is no longer exposed in the default toolset; speculative retrieval capsule routing was deleted from Telegram. Docker `/status` reports `compact_memory` with `collection=aura_memory_v1_compact docs=487 vector=1024`, and logs show compact mirror sync start/success. This closure was later tightened by the default-tool diet entry above: broad default smokes now use only one `search_memory` call, while `daily_briefing` is explicit admin/ops. Verification: `go test ./...`; `docker compose config --quiet`; `docker compose up -d --build aura`; live `/status`; `cmd/debug_telegram_sandbox` smokes.
