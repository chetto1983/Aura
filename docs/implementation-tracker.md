...
2026-05-08 Telegram conversation god-class split slice: moved archive turn persistence into `conversation_archive.go`, speculative wiki retrieval timeout/search helpers into `conversation_context.go`, orchestration snapshot storage/pruning into `conversation_snapshot.go`, and the remaining terminal/tool-result formatting helpers into `conversation_format.go`. This was a behavior-preserving move; `conversation.go` dropped from 1008 to 807 lines, with the next high-value split still being the actual model/tool loop and executor extraction. Verification: `go test ./internal/telegram`.
