...
fresh install leaves `./data` root-owned, so aura crashes with "permission
denied" when it tries to create `aura.db` or its log directory; the
`garage-init` sidecar would also have no S3 key file to register with the
Garage daemon.
...
is required. Released installs pull the published GHCR image; the first-run
setup wizard collects the Telegram token and writes it to SQLite:
...
- `backups/<timestamp>/aura-backup.tar.gz`: full restore point with `aura.db`
  (settings + secrets live in SQLite), `wiki/`, and `skills/`.
...
- All runtime configuration lives in SQLite (`/data/aura.db`). The first-run
  setup wizard writes the Telegram token there. `.env` is not read anywhere
  in the codebase — secrets for new installs come from the wizard, secrets
  for upgrade installs must be migrated manually (set the relevant env vars
  in compose or use the dashboard once Aura is running).
...
- `compose.yaml` starts Garage with `--single-node`. The S3 access key
  generated by `aura-secrets` is registered with the daemon by a one-shot
  `garage-init` sidecar (alpine + `garage` CLI from the upstream image) that
  runs after `garage` is up: it imports the key, creates the
  `aura-artifacts` bucket, and grants read+write+owner. Every step is
  idempotent. Aura reads the same key files via
  `GARAGE_S3_ACCESS_KEY_FILE` / `GARAGE_S3_SECRET_KEY_FILE` and waits on
  `garage-init` completion before starting.
