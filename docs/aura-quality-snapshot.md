...
|---|---|---|---|---|---|
| Sandbox execution baseline (sandbox-agent pivot) | live command + workspace persistence pass | 2026-06-15 | Re-attested 2026-06-15: the matching changed path is `compose.yaml`; the diff touched appliance env forwarding, setup/migration orchestration, and MCP/sidecar wiring, with zero `docker/sandbox-agent/**`, `internal/sandboxagent/**`, or `sandbox_exec` surface. Phase 8/08.1 validation evidence stands; bespoke Phase 5 escape bench superseded and removed. Prior 2026-06-12 re-attestation covered an aura-agent-memory-mcp-only compose touch. | Phase 8 sandbox-agent pivot | `internal/sandboxagent/**`, `internal/agent/tools/sandbox_exec.go`, `docker/sandbox-agent/**`, `compose.yaml` |
