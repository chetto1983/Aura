...
| GraphRAG retrieval recall@5 @ 100K corpus | >= 0.8 | 2026-06-11 | **NOT_E2E_FUTURE_BENCHMARK** - no 100K benchmark harness was executed in Phase 20. The Neo4j smoke passed on the operator host (`recall@5=5/5`, p95=3 ms after warm-up), but that 5-document smoke is not recorded as the 100K benchmark. Follow-up: memory-scale benchmark phase. | Phase 15 Slice 11d | `internal/memory/**`, `internal/db/migrations/neo4j/**` |
| Vector search p95 latency @ 100K corpus | <= 30ms | 2026-06-11 | **NOT_E2E_FUTURE_BENCHMARK** - no 100K vector latency harness was executed in Phase 20. The warmed Neo4j smoke p95 was 3 ms and remains smoke evidence only, not the 100K row. Follow-up: memory-scale benchmark phase. | Phase 15 Slice 11d | `internal/memory/retrieval/**`, sidecar `aura-llama-embed` config |
