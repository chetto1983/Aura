...
**Created:** 2026-05-29
**Last updated:** 2026-06-08 (Phase 13 Channels+Telegram+Multimodal Gate-3 CLOSED: mdv2.go mutation 81.8% + renderer.go 74.1% [both ≥70%, autopsy-hardened over 3 passes], owned-surface coverage 86.8%, lint=0; live autonomous E2E loop **11/11 = 100%** via scripts/telegram_e2e.sh [telegram 3 + multimodal 3 against the live 9c sidecars + setup-wizard :9081 5/5]; surfaced + fixed 2 compose defects vs the validated spikes [STT image→hwdsl2 spike-027, markitdown image deferred]. Prior: 2026-06-07 Phase 12 AG-UI Gate-3 closed.)
...
| Telegram channel (UX-02/03/04) — MarkdownV2 escape fuzz (10K Unicode, 400 rate) + `mdv2.go`/`renderer.go` mutation + owned-surface coverage + live Bot-API sends (response-asserted) + multimodal STT/OCR/TTS round-trip | 400 rate = 0% / mdv2+renderer mutation ≥70% / owned-surface ≥85% / live sends response-asserted / multimodal round-trip | 2026-06-08 (automated Gate-3 metrics DONE; live E2E autonomous loop 11/11 = 100%) | **mdv2 fuzz 400-rate = 0%** (10K-Unicode seed corpus replays clean + a 20s `-fuzz` campaign, 77K execs, 0 crashes). **`mdv2.go` mutation 81.8%** (36/44) + **`renderer.go` mutation 74.1%** (40/54) — both ≥70% (go-mutesting WSL, autopsy-targeted over 3 hardening passes: renderer 24%→59%→67%→74%). Residual survivors are documented equivalents: the END/RUN_FINISHED `flush(true)` removals are masked by the post-loop close flush (3 finalization points); `capRunes` `<=n`≡`<n` (identical at exactly-n); `chatRate<=0` variants with chatRate≥0; the `out!=nil` success guard. **owned-surface coverage 86.8%** (unit + db_integration, measured non-destructively to spare a parallel Codex PG; canonical `scripts/coverage_gate.sh` deferred to a Codex-paused window). **live `telegram_integration` 3/3 PASS** — sendPhoto (table→PNG) / sendDocument (artifact, FileName+MIME+FileSize echoed) / sendVoice (OGG/Opus) asserted on the Bot-API RESPONSE (`msg.Photo`/`msg.Document`/`msg.Voice`), never getUpdates (spike 017/019); **goleak-clean** after a dedicated-`http.Transport` reap fix (telebot's keep-alive `http2ClientConn.readLoop` was outliving the test). **live `multimodal_integration` 3/3 PASS** against the 9c CPU sidecars (operator-delegated autonomous E2E loop, `scripts/telegram_e2e.sh`): TTS→STT round-trip (Kokoro opus → faster-whisper turbo transcript non-empty, 24s cold), GLM-OCR describe non-empty (4.5s), TTS opus bytes. **Required two compose defect fixes the live run surfaced** (the spikes were the ground truth): STT was on `fedirz/faster-whisper-server`+`WHISPER__MODEL=large-v3-turbo` (HTTP 500, invalid model size) → restored to spike-027's `hwdsl2/whisper-server`+`WHISPER_MODEL` on :9000 (turbo native, OGG/Opus direct); the `markitdown` documents sidecar image `ghcr.io/microsoft/markitdown:latest` does not exist (registry-denied) → tracked in deferred-items (UX-04 docs leg, not in the multimodal tier). **setup-wizard :9081 E2E 5/5** (status 401 no-token / 200 w/token / `POST /setup/token` live getMe → `bot_username` / `POST /setup/onboard-link` → `t.me/<bot>?start=<uuid>` / onboarding mint persisted in PG). **Autonomous E2E total 11/11 = 100%** (telegram 3 + multimodal 3 + setup 5). Live-driven: SC#1 (setup 401/200) + SC#4 (voice+image via the STT/OCR sidecars) + the SC#2 render-surface sends (response-asserted). SC#3 `/cancel` + SC#5 `/cost`/`/search` are unit-proven (`TestCancelCancelsInflightTurnCtx`/`TestCostEqualsCLI`/`TestSearchEqualsCLI`) + mounted in `aura serve` (not driven via a live Telegram inbound, which needs an MTProto userbot — documented scope). | Phase 13 Slice 9 | `internal/channels/telegram/**`, `internal/setup/**`, `internal/channels/**`, `cmd/aura/serve.go`, `.github/workflows/ci.yml` |
