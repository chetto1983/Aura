...
**Created:** 2026-05-29
**Last updated:** 2026-06-12 (Phase-15 close-out E2E re-measure: KV 87.6%, smoke recall@5 5/5 p95 25ms + agent-memory advisory 1.000/44.55ms, CoT 36/37 post-memory-doctrine, Telegram CI live job green, AG-UI live smoke green, sandbox re-attested — amendment #20 staleness sweep.)
...
|---|---|---|---|---|---|
| Sandbox execution baseline (sandbox-agent pivot) | live command + workspace persistence pass | 2026-06-12 | Re-attested 2026-06-12: the matching changed path was compose.yaml whose diff touched ONLY the aura-agent-memory-mcp service block (15-04) — zero sandbox surface; Phase 8/08.1 validation evidence stands, bespoke Phase 5 escape bench superseded and removed | Phase 8 sandbox-agent pivot | `internal/sandboxagent/**`, `internal/agent/tools/sandbox_exec.go`, `docker/sandbox-agent/**`, `compose.yaml` |
| KV cache hit rate (DeepSeek-V4 Flash, 20-turn replay) | >= 80% | 2026-06-12 | **PASS** - `TestKVCacheWarmingE2E` (post-<memory>-doctrine prompt f1062711): turn-2 **87.6%**, turn-3 86.7%, turn-4 79.4% (turns 5-6 OpenRouter instance-switch 0%); peak post-first **87.6%** >= 80 gate. D-04 cache invariant audit 22 unchanged hashes, cot_eval cache_prefix_stability 1/1. | Phase 6 Slice 4 | `internal/llm/**`, `scripts/cache_invariant_audit.sh` |
| GraphRAG retrieval recall@5 @ 100K corpus | >= 0.8 | 2026-06-12 | **NOT_E2E_FUTURE_BENCHMARK** - no 100K benchmark harness was executed in Phase 20. The Neo4j smoke passed on the operator host (`recall@5=5/5`, p95=3 ms after warm-up), but that 5-document smoke is not recorded as the 100K benchmark. Re-measured 2026-06-12: `make smoke` recall@5 **5/5** (p95 25 ms warm) + Phase-15 advisory snapshot vs the agent-memory package recall@5 **1.000** (15-05). Still smoke/advisory evidence, not the 100K benchmark. Follow-up: memory-scale benchmark phase. | Phase 15 Slice 11d | `internal/agent/mcptools/**`, `internal/knowledge/**`, `docker/agent-memory/**` |
| Vector search p95 latency @ 100K corpus | <= 30ms | 2026-06-12 | **NOT_E2E_FUTURE_BENCHMARK** - no 100K vector latency harness was executed in Phase 20. The warmed Neo4j smoke p95 was 3 ms and remains smoke evidence only, not the 100K row. Re-measured 2026-06-12: warmed smoke p95 **25 ms** (5-doc corpus) + Phase-15 advisory p95 **44.55 ms** vs the agent-memory package (15-05). Still smoke/advisory evidence, not the 100K row. Follow-up: memory-scale benchmark phase. | Phase 15 Slice 11d | `internal/knowledge/**`, `cmd/aura/memory*.go`, sidecar `aura-llama-embed` config |
| Telegram channel (UX-02/03/04) — MarkdownV2 escape fuzz + `mdv2.go`/`renderer.go` mutation + owned-surface coverage + live Bot-API sends + multimodal sidecars + inbound command route | 400 rate = 0% / mdv2+renderer mutation >=70% / owned-surface >=85% / live sends response-asserted / multimodal round-trip / inbound commands reply in Telegram Web | 2026-06-12 | **PASS** - CI run 27406130150 (2026-06-12, branch tabula-rasa): live Telegram Bot-API (telegram_integration tier) job **success** on GitHub Actions. - Phase 20 live scorer re-run: `scripts/telegram_e2e.sh` under WSL scored **11/12 = 91%** (threshold 90): Bot-API sends 3/3, multimodal 4/4, setup wizard 4/5 in-script. The remaining in-script sub-check was WSL Docker integration only; host Docker query for the printed onboarding token returned `row count=1`, closing setup wizard as **5/5**. Historical mutation/coverage stay green: `mdv2.go` 81.8%, `renderer.go` 74.1%, owned-surface refreshed in Phase 20 at **85.9%**. | Phase 13 Slice 9 | `internal/channels/telegram/**`, `internal/setup/**`, `internal/channels/**`, `cmd/aura/serve.go`, `.github/workflows/ci.yml` |
| Skills system (CAP-07 / CAP-08 / D-35, post-#51 shape) — xlsx North-Star E2E + tiers + mutation + coverage | E2E judge >=0.90 over 2 dims (capability_gap_recognition + skill_output_quality) + artifact-not-reply (FRESH .xlsx exists/opens/today) + structured-arg self-install evidence (`npx skills add` against a concrete xlsx/excel/spreadsheet-capable source) / unit+db+sandbox tiers green / validator.go hard mutation >=70% and writer.go advisory / combined coverage >=85% / SC#1-SC#4 true (SC#5 superseded by #51) | 2026-06-11 | **PASS** - deterministic tiers green (unit/race, 60s `FuzzSkillValidator`, db_integration, `TestSnippetExec`). Chat-surface gate **PASS 6/6 = 100%** (`scripts/chat-e2e-gate.sh`, 324.2s, fresh/openable/today xlsx + PG turns). Structural guard **PASS** (`TestSkillsE2E`, judgeMean=1.00, selfInstall=true target=true selector=true, fresh/openable/today xlsx). Historical mutation remains: validator.go 83.3% hard PASS; writer.go advisory db-subprocess artifact accepted with DB tests as witness. Owned-surface coverage refreshed at **85.9%**. | Phase 11 Slice 7 (7a-7g) | `internal/skills/**`, `internal/agent/tools/skill*.go`, `internal/eval/skills_cot_eval_test.go`, `.github/workflows/skills.yml`, `scripts/cache_invariant_audit.sh` |
