...
|---|---|---|---|---|---|
| Sandbox execution baseline (sandbox-agent pivot) | live command + workspace persistence pass | 2026-06-21 | Re-attested 2026-06-21: the matching changed path is `compose.yaml`; the diff only sets `WHISPER_API_KEY: ""` for the loopback-published `aura-stt` sidecar because current `hwdsl2/whisper-server` fresh volumes auto-generate Bearer auth unless this is explicitly empty. Local witness: `docker compose config` rendered `aura-stt.environment.WHISPER_API_KEY: ""`; zero `docker/sandbox-agent/**`, `internal/sandboxagent/**`, or `sandbox_exec` surface changed. Phase 8/08.1 validation evidence stands; bespoke Phase 5 escape bench superseded and removed. Prior 2026-06-16 re-attestation covered `aura-pim-mcp` HTTP sidecar, GHCR default image/pull-policy wiring, and `aura-pim-data` volume for calendar/mail/contacts MCP wiring; prior 2026-06-15 re-attestation covered appliance env forwarding/setup orchestration compose wiring; prior 2026-06-12 re-attestation covered an aura-agent-memory-mcp-only compose touch. | Phase 8 sandbox-agent pivot | `internal/sandboxagent/**`, `internal/agent/tools/sandbox_exec.go`, `docker/sandbox-agent/**`, `compose.yaml` |
...
| GraphRAG retrieval recall@5 @ 100K corpus | >= 0.8 | 2026-06-20 | **NOT_E2E_FUTURE_BENCHMARK** (re-baselined 2026-06-20: GraphView live fallback expectations were aligned with the documented drawable overview behavior and Sigma now renders parallel Neo4j relationships via graphology multigraph mode. This patch does not change retrieval ranking, embedding, HNSW index, or corpus-search production behavior; the 100K benchmark remains a future benchmark. Local witnesses: `go test -race -count=1 -tags 'db_integration neo4j_integration' ./internal/knowledge/ -run TestGraphViewLive_Footprint`, `npx vitest run src/graph/__tests__/SigmaCanvas.test.tsx`, `npm run lint`, `npm run typecheck`, `npm run build`, `go test ./...`, and `bash scripts/quality_snapshot_gate.sh`.) Prior 2026-06-19: Phase 27 touched `internal/knowledge/**` for read-only GraphView/GraphReader contracts, intent guardrails, and graph API integration. This is Neo4j graph-explorer read-path/UI evidence plumbing, not the retrieval ranking, embedding, HNSW index, or corpus-search surface; the 100K benchmark remains a future benchmark. Local witnesses: containerized `go test -buildvcs=false ./internal/knowledge/... ./internal/agui/...`, `go vet`, `go build`, `go test -race`, plus Phase 27 Playwright desktop/mobile graph E2E and targeted graph mutation passed. Prior 2026-06-15: CI-only touch — the `internal/knowledge/client_unit_test.go` loopback compose-test fix (sanction the one caddy public ingress), zero retrieval/HNSW/embedding surface. Prior 2026-06-13: the `internal/knowledge` change was O-05's `probe.go` — a dial → VerifyConnectivity → close readiness probe for `/readyz`, with zero retrieval/HNSW/embedding surface; the future-benchmark status + smoke/advisory evidence stand. No 100K benchmark harness was executed in Phase 20. The Neo4j smoke passed on the operator host (`recall@5=5/5`, p95=3 ms after warm-up), but that 5-document smoke is not recorded as the 100K benchmark. Re-measured 2026-06-12: `make smoke` recall@5 **5/5** (p95 25 ms warm) + Phase-15 advisory snapshot vs the agent-memory package recall@5 **1.000** (15-05). Still smoke/advisory evidence, not the 100K benchmark. Follow-up: memory-scale benchmark phase. | Phase 15 Slice 11d | `internal/agent/mcptools/**`, `internal/knowledge/**`, `docker/agent-memory/**` |
