...
| 1b | swarm-child output enveloped untrusted | **PASS (code+test)** | Live swarm-spawn is non-deterministic to force from a prompt; covered by `runner_adapter.go:62` (`res.Provenance = …Trust: TrustUntrusted`) + `TestRunnerAdapterDrivesEngine` (runner_adapter_test.go, race-clean, A4). |
