...
| **1.12** Reasoning lost on reload (operator-requested 2026-07-23) | NEW — PRD **Amendment #91** | Persist the turn's accumulated CoT (bounded, `AURA_REASONING_PERSIST_MAX_RUNES` default 65536) + `reasoning_duration_ms` (first→last delta wall time, UI-spec OQ-1 ratified) onto new nullable `conversation_turns` columns (single migration at next free slot, floor `0049` at plan time) in `persistAssistantAnswer`; snapshot projection + cockpit `snapshotToThreadMessages` gain the display-only fields so the drawer/pill survives reload. LLM context stays STRUCTURALLY reasoning-free (`llm.Message` has no field — pinned by regression test). Redacted stream ⇒ nothing stored (HARDEN-05). Supersedes #57's never-persist clause only. | **migration** / **PRD #91** | med |
