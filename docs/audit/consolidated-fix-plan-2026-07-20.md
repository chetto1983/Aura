...
|---|---|---|---|---|
| **0.1** Evaluator dies on 23505 | CONFIRMED (high) | `ON CONFLICT (scope_id, evidence_digest) DO NOTHING` on `AppendCompactionRolloutEvidence` + new `GetCompactionRolloutEvidenceByDigest` fallback-SELECT (**`DO UPDATE` is impossible — BEFORE-UPDATE immutability trigger**); tolerate 23505 in `Run` (continue); treat empty `{}` windows as "no observation" (skip Apply), the guard that actually stops the storm. Minimal survive-the-spike scope only. | no migration (query+sqlc) / no | med |
