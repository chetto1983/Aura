...
2. **Toolchain pre-bake** — aura Dockerfile (pandoc/file/xxd + npm `docx`/`NODE_PATH` + pip python-docx/openpyxl/pandas) + boot smoke (docx `require`, python-docx import, pandoc/file/xxd present).
