...
- Migration `0071_adaptive_focal_cohorts` and its source/live migration tests
  ...
  `APPROVE`.
- PRD Amendment #93.5 commits `cf6c6e1bc`, `37b6fa646`, `c7b1b417c`, and
  `30ea2f09c` define the single transaction, deterministic catalog hashes,
  owner-scoped real conversation, static champion, and owner-global
  conversation uniqueness.
- Forward migrations `0072` and `0073` are complete in `02ba3c676` and
  `f7402a70b`: direct database writers cannot claim a fabricated/cross-owner
  conversation or randomize the same owner conversation through a second
  cohort.
- Typed `CohortStore` is complete in `45b845652`. Save/Load reconstruct the
  canonical cohort fail-closed. `Enroll` locks the owner, claims the winning
  focal slot, invokes the local draw exactly once, validates every cohort,
  scope, catalog, hash, arm, action, and champion binding, and appends the
...
- Verified evidence at the approved head includes clean head-67 install,
  ...
  repository file-size checks, canonical snapshot ID/hash/order and deep-copy
  immutability, owner/provider/model/policy/cutoff scope rejection, top-five
...
- [x] Typed `CohortStore` and atomic claim API — migrations `0072`/`0073` in
  `02ba3c676`/`f7402a70b`, implementation `45b845652`; real disposable DB
...
- Create: `internal/adaptive/randomization.go`
- Create: `internal/adaptive/randomization_test.go`
- Create: `internal/adaptive/randomization_receipt.go`
- Create: `internal/adaptive/randomization_receipt_test.go`
...
- Add: focused enrollment/randomization DB integration tests under
  `internal/adaptive/`
...
- [x] **Step 5: Implement durable focal claims.** Approved through
  `45b845652`, with forward migrations `0072` and `0073`. Unique
  `(cohort_id, request_id)` binds request membership and unique
  `(owner_id, evaluation_conversation_id)` permits only one randomized request
  per owner conversation across all cohorts. One owner-locked transaction
  inserts the winning claim, invokes only the local assignment builder, binds
  the complete schema-2 Assignment, records it through
...
- [x] **Step 5a: Write replacement enrollment fault tests** in
  ...
  after winning the unique claim; consumes exactly one byte from store-owned
  `crypto/rand`; maps its low bit to the frozen arm; and writes claim,
  Assignment, and canonical `RandomizationReceipt` in one owner-locked
  transaction. Inject entropy-read, validation, Assignment-write,
  receipt-write, rollback, definite-commit, commit-unknown, exact-retry, and
...
- [x] **Step 5b: Replace the historical enrollment draw** in
  ...
  rechecked next-free migration and
  `internal/db/queries/adaptive_policy.sql`. Preserve the checked historical
  claim/lock/identity work above, but do not treat enrollment as #93.7-complete
  until Step 5a is green and the callback can no longer choose an arm.
...
- [ ] **Step 2: Replace string recall with typed recall** carrying fenced text, ordered IDs, effective limits, revisions, and coherent epoch. The ordered schema-`2.0` candidate catalog is `static`, `long_term_top_4`, `long_term_top_8`: disabled recall or configured maximum below 4 exposes only exogenous `static` with no randomized Assignment; maximum 4–7 exposes `static` plus `long_term_top_4`; maximum at least 8 exposes all three, always capped by the configured maximum. `static` means provider-off, with no separate `off` action/event ID. Context-budget omission records actual Delivery action `none` with fallback `context_budget` while preserving intended arm/action for ITT; `none` is not a candidate action. Short-term and reasoning flags stay false.
