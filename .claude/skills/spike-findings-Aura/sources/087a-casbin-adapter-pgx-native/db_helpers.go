...
// live Aura schema is left untouched (the casbin_rule table is a spike artifact,
// not a shipped Aura table yet).
...
// columnExists reports whether aura.casbin_rule has the named column (proves the
// adapter did not silently recreate the table with its own shape).
...
// countRows returns the number of rows of the given p_type ('p' or 'g') in the
// live table — the ground-truth persistence assertion.
