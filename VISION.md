...
Aura is your personal, always-on Telegram assistant that thinks in long-term timelines, not single messages. The goal is to give power users and developers a reliable companion that can remember context, orchestrate tools, and help manage both information and actions across weeks and months.
...
- Combine LLMs, a local wiki, search, and other tools into one cohesive brain.
...
- **Composability**: Each capability (LLM, wiki, search, budget tracking, skills, tasks) is modular, so you can enable only what you need.
- **Transparency**: Behavior should be debuggable and observable through logs, health endpoints, and simple configuration.
...
- Fast: low-latency responses and lightweight Go services, even on modest hardware.
...
- Extensible: easy to add new “skills” and tools without rewriting the core.
...
- Connect to different LLM providers, from cloud APIs to local models.

## Long-term direction

Over time, Aura should evolve into a robust personal automation platform built on top of Telegram:

- Deeper memory and knowledge: richer wiki and graph views in the web dashboard, smarter retrieval, and better summarization of long-term history.
- More skills: reusable, well-documented skills for common workflows (planning, tracking, integrations with external systems).
- Stronger observability: richer health endpoints, metrics, and traces that make it easy to understand what Aura is doing and why.
...
- Maintainers focus on stability, DX, and documentation so that contributions stay easy to reason about.
