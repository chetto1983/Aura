...
- `recipe:calendar` → forked `chetto1983/aura-pim-mcp` @ `aura/pim-sidecar` (calendar-mcp 1.4.1, Blazor UI stripped, 21 provider operations curated into ONE `calendar` action tool over 17 actions, 46-05/46-06), a separate compose service pinned by commit to GHCR (`ghcr.io/chetto1983/aura-pim-mcp:4e0f518333773a892f9f5a718981466e6054ae2e`). Only one workflow writes that fork's `:sidecar`, but the tag moves under that name, so the pin is the raw commit sha its publish workflow tags alongside.
