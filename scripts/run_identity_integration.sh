#!/usr/bin/env bash
# Run the internal/identity db_integration tier in WSL against the live Postgres.
# Derives AURA_DB_URL / AURA_DB_MIGRATE_URL from POSTGRES_PASSWORD in ~/aura.env.
# Invoked as: wsl bash -lc 'set +H; bash ~/run_identity_integration.sh'
set -euo pipefail
...
cd /mnt/d/Aura
go test -tags db_integration -race -count=1 "$@" ./internal/identity/
