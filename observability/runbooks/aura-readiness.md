...
Open the linked panel on dashboard `aura-overview`. Filter only by catalog-owned bounded dimensions such as `operation`, `outcome`, `error_class`, and `state`; never add identity, conversation, request, tool-call, operation-key, URL, path, or raw-error labels. Compare the alert start with `/readyz` codes (`postgres_unavailable`, `memory_unavailable`, `listener_unavailable`, `migration_incompatible`, `scheduler_stalled`, `draining`, `dependency_unavailable`) and then use Tempo for traces in the same time range. Trace IDs stay in traces and logs, not metric labels.
...
2. Check the Aura, PostgreSQL, ArcadeDB (plus its `arcadedb-mcp` sidecar), and scheduler container health without restarting all services at once.
