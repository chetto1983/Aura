...
var httpMutationRoutes = map[string]mutationRouteMeta{
	...
	// a new id, which the host updater would act on again.
	"POST /api/system/update/apply": httpMutationMeta("system_update_apply"),
	"POST /api/system/update/defer": httpMutationMeta("system_update_defer"),
	...
	// an exemption argued from a handler's internals is one refactor away from being
	// false.
	"POST /api/governance/mcp/{name}/authorization":            httpMutationMeta("mcp_authorization_start"),
	"DELETE /api/governance/mcp/{name}/authorization":          httpMutationMeta("mcp_authorization_revoke"),
	...
	"POST /api/studio/images":                                  httpMutationMeta("studio_image_create"),
