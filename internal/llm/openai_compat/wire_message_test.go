...
func TestToSDKMessagesGuaranteesContentForNonAssistant(t *testing.T) {
	...
	wire, _, err := toSDKMessages(msgs, nil, nil, llm.ReasoningTargetNone)
	if err != nil {
...
func TestToSDKMessagesKeepsAssistantContent(t *testing.T) {
	wire, _, err := toSDKMessages([]llm.Message{{Role: llm.RoleAssistant, Content: "hi there"}}, nil, nil, llm.ReasoningTargetNone)
	if err != nil {
		t.Fatalf("toSDKMessages: %v", err)
	}
