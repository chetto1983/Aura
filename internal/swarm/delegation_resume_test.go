...
// 51-06b Task 1/2's coverage: buildResumeTurns' answer-substitution mechanism,
// mintFreshUUID, DelegationResumeObserver.ProcessOnce against a fake store, and the
// end-to-end proof that a SECOND runChild call seeded from a FIRST call's returned
// history actually continues the same worker rather than re-asking. The atomic
// pause-and-park transaction itself (PauseAndPark) is a consumer-declared interface
// satisfied at cmd/aura and proven live under db_integration
// (delegation_delivery_db_test.go's sibling coverage for the analogous Delivery seam).
