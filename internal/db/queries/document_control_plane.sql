...
ORDER BY created_at, id;
-- name: UpsertDocumentPipelineStage :one
INSERT INTO aura.document_pipeline_stages (
    ...
    sqlc.arg(identity_id), sqlc.arg(document_id), sqlc.arg(version_id),
    sqlc.arg(stage), sqlc.arg(input_fingerprint), sqlc.arg(producer_version),
    sqlc.arg(pipeline_generation), sqlc.arg(status), sqlc.arg(max_attempts),
...
RETURNING *;
-- name: ClaimDocumentPipelineStage :one
...
RETURNING *;
-- name: CompleteDocumentPipelineStage :one
UPDATE aura.document_pipeline_stages
SET status = 'succeeded', artifact_storage_object_id = sqlc.narg(artifact_storage_object_id),
...
WHERE id = sqlc.arg(id) AND identity_id = sqlc.arg(identity_id)
  ...
  AND lease_generation = sqlc.arg(lease_generation)
RETURNING *;
-- name: HeartbeatDocumentPipelineStage :one
UPDATE aura.document_pipeline_stages
...
RETURNING *;
-- name: UpdateDocumentPipelineStageStatus :one
UPDATE aura.document_pipeline_stages
SET status = sqlc.arg(status), error_class = sqlc.arg(error_class),
    error_code = sqlc.arg(error_code), error_message = sqlc.arg(error_message),
    next_attempt_at = sqlc.arg(next_attempt_at), locked_by = NULL, locked_until = NULL,
    completed_at = CASE WHEN sqlc.arg(status)::text IN ('succeeded', 'failed', 'dead_letter')
                        THEN now() ELSE NULL END,
    updated_at = now()
WHERE id = sqlc.arg(id) AND identity_id = sqlc.arg(identity_id)
  AND status = 'running' AND locked_by = sqlc.arg(locked_by)
