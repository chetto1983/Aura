CREATE FUNCTION aura.adaptive_schema2_ascii_id_valid(p_value text, p_maximum_length integer) RETURNS boolean
...
CREATE FUNCTION aura.adaptive_schema2_safe_slug_valid(p_value text, p_maximum_length integer) RETURNS boolean
...
END;
$$;
CREATE FUNCTION aura.adaptive_schema2_revision_value_valid(p_kind text, p_value text) RETURNS boolean
    LANGUAGE plpgsql IMMUTABLE STRICT SET search_path = pg_catalog
AS $$
BEGIN
...
CREATE FUNCTION aura.adaptive_schema2_assignment_payload_valid(
    p_payload jsonb, p_owner_id uuid, p_aggregate_id text, p_decision_id uuid)
...
END;
$$;
CREATE FUNCTION aura.adaptive_schema2_delivery_payload_valid(p_payload jsonb, p_decision_id uuid) RETURNS boolean
    LANGUAGE plpgsql IMMUTABLE STRICT SET search_path = pg_catalog
AS $$
DECLARE
...
BEGIN
    ...
    FOR result_id IN SELECT value FROM jsonb_array_elements(p_payload->'result_ids') LOOP
        IF jsonb_typeof(result_id) <> 'object'
            ...
            OR jsonb_typeof(result_id->'kind') <> 'string' OR result_id->>'kind' NOT IN (
...
ALTER TABLE aura.adaptive_outbox
    ...
    ADD CONSTRAINT adaptive_outbox_delivery_schema_check CHECK (
        event_kind <> 'delivery' OR payload->>'schema_version' IS NOT DISTINCT FROM '2.0'
    ),
    ADD CONSTRAINT adaptive_outbox_schema2_assignment_check CHECK (
        payload->>'schema_version' IS DISTINCT FROM '2.0'
