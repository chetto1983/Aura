DROP TABLE IF EXISTS aura.retention_operation_items;
DROP TABLE IF EXISTS aura.retention_operations;

DELETE FROM aura.agent_job_runs
WHERE task_id IN (SELECT id FROM aura.scheduler_tasks WHERE kind = 'retention_sweep');
DELETE FROM aura.scheduler_tasks WHERE kind = 'retention_sweep';
