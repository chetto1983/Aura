CREATE FUNCTION aura.adaptive_policy_snapshot_artifact_valid(
    p_artifact bytea,
    p_owner_id uuid,
    p_domain text,
    ...
    LOOP
        ...
        LOOP
            ...
            THEN
                RETURN false;
            END IF;
            outcome_ids := array_append(outcome_ids, outcome_id);
            ...
            END LOOP;
            IF vector_norm <= 0
                OR vector_norm = 'Infinity'::double precision
            THEN
                RETURN false;
            END IF;
        END LOOP;
...
CREATE TABLE aura.adaptive_policy_snapshots (
    ...
    owner_id           uuid NOT NULL
        REFERENCES aura.identities(id) ON DELETE CASCADE,
    ...
    decision_point     text NOT NULL CHECK (
        ...
        AND decision_point = domain
    ),
    provider_id        text NOT NULL CHECK (
        aura.adaptive_schema2_ascii_id_valid(provider_id, 64)
    ),
    model_id           text NOT NULL CHECK (
        aura.adaptive_schema2_ascii_id_valid(model_id, 256)
    ),
    policy_version     text NOT NULL CHECK (
        aura.adaptive_schema2_ascii_id_valid(policy_version, 128)
    ),
    training_cutoff    timestamptz NOT NULL,
    feature_schema     jsonb NOT NULL,
    action_catalog     jsonb NOT NULL,
    champion_action_id text NOT NULL CHECK (champion_action_id = 'static'),
    artifact           bytea NOT NULL,
    created_at         timestamptz NOT NULL DEFAULT now(),
    CONSTRAINT adaptive_policy_snapshots_artifact_digest_check CHECK (
        sha256 = pg_catalog.sha256(artifact)
        AND id = encode(
            set_byte(
                set_byte(
                    substring(pg_catalog.sha256(
                        uuid_send('079f0700-7db9-4d37-9d2a-1890d2da90f8'::uuid)
                        || sha256
                    ) FROM 1 FOR 16),
                    6,
                    (
    ...
    CONSTRAINT adaptive_policy_snapshots_artifact_scope_check CHECK (
        aura.adaptive_policy_snapshot_artifact_valid(
            artifact, owner_id, domain, decision_point, provider_id, model_id,
            policy_version, training_cutoff, feature_schema, action_catalog,
            champion_action_id
        )
    )
);

ALTER TABLE aura.adaptive_policy_snapshots ENABLE ROW LEVEL SECURITY;
CREATE POLICY adaptive_policy_snapshots_owner_isolation
    ON aura.adaptive_policy_snapshots
    USING (
        owner_id = NULLIF(
            current_setting('app.current_identity', true),
            ''
        )::uuid
    )
    WITH CHECK (
...
REVOKE ALL ON aura.adaptive_policy_snapshots FROM aura_app;
GRANT SELECT, INSERT ON aura.adaptive_policy_snapshots TO aura_app;
REVOKE ALL ON FUNCTION aura.adaptive_policy_snapshot_artifact_valid(
    bytea, uuid, text, text, text, text, text, timestamptz, jsonb, jsonb, text
) FROM PUBLIC;
GRANT EXECUTE ON FUNCTION aura.adaptive_policy_snapshot_artifact_valid(
    bytea, uuid, text, text, text, text, text, timestamptz, jsonb, jsonb, text
) TO aura_app;

COMMENT ON TABLE aura.adaptive_policy_snapshots IS
