...
# The Aura image carries the complete, tested dependency tree. Clone it without
# contacting a registry; the caller's project remains independently writable.
TEMPLATE_ROOT=/opt/aura-artifacts/template
if [ -d "$TEMPLATE_ROOT/node_modules" ]; then
  PROJECT_NAME="${1:?Usage: init-artifact.sh <project-name>}"
  if [ -e "$PROJECT_NAME" ]; then
    echo "Project already exists: $PROJECT_NAME" >&2
    exit 1
  fi
  cp -a "$TEMPLATE_ROOT" "$PROJECT_NAME"
  (
    cd "$PROJECT_NAME"
    node --input-type=module - "$PROJECT_NAME" <<'JS'
import fs from 'node:fs';
import path from 'node:path';
const name = path.basename(process.argv[2]).toLowerCase().replace(/[^a-z0-9-]/g, '-');
const pkg = JSON.parse(fs.readFileSync('package.json', 'utf8'));
pkg.name = name;
fs.writeFileSync('package.json', JSON.stringify(pkg, null, 2) + '\n');
fs.writeFileSync('index.html', fs.readFileSync('index.html', 'utf8').replace(/<title>.*?<\/title>/, `<title>${name}</title>`));
JS
  )
  echo "Ready: $PROJECT_NAME (preinstalled Aura artifact toolchain, no network needed)"
  exit 0
fi
...
# The bundled shadcn archive uses DayPicker 9 and PanelGroup/PanelResizeHandle.
# Keep its APIs compatible, including DayPicker's React 19 and date-fns peers.
pnpm install sonner cmdk vaul embla-carousel-react react-day-picker@9.14.0 react-resizable-panels@2.1.9 date-fns@4.4.0 react-hook-form @hookform/resolvers zod
