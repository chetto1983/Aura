...
func PromptBlock(loaded []Skill) string {
	...
	sb.WriteString("Before following a skill's detailed guidance, use `file` with action=\"search\" to locate the matching SKILL.md, then `file` with action=\"read\" on that exact file only. ")
