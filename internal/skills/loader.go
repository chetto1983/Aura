...
// Loader reads local skills from a configured skills directory.
type Loader struct {
	dir string
}

// NewLoader creates a loader rooted at dir. The directory is expected to
// contain one subdirectory per skill, each with a SKILL.md file.
...
// LoadAll loads every valid skill. Invalid skill folders are skipped so one
// broken local draft cannot remove all skills from the prompt.
func (l *Loader) LoadAll() ([]Skill, error) {
	...
	for _, entry := range entries {
		...
		skill, err := l.loadPath(filepath.Join(l.dir, entry.Name(), "SKILL.md"))
		if err != nil {
			continue
		}
...
// LoadByName reads one skill by directory name.
func (l *Loader) LoadByName(name string) (Skill, error) {
	name = strings.TrimSpace(name)
	if !skillNameRE.MatchString(name) {
		return Skill{}, fmt.Errorf("invalid skill name %q", name)
	}
	return l.loadPath(filepath.Join(l.dir, name, "SKILL.md"))
}

func (l *Loader) loadPath(path string) (Skill, error) {
