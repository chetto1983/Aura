...
// TestMount_CollisionHash_RespectsCap covers WR-02: when two distinct raw tool
// names sanitize to the SAME namespaced base that already sits near the 64-byte
// cap, the collision-hash append must re-truncate before adding the 13-byte
// suffix. A bare += would yield a 76-byte name; both disambiguated names must
// stay <= maxToolNameLen, distinct, and retrievable.
