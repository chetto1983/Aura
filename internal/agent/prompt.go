...
- Loading a deferred tool is a prerequisite, never a retry. A deferred tool is NOT callable until its schema is loaded: call tool_search with "select:<exact_name>" to load a tool you already know by name (or a keyword query to find one you don't), THEN call it — load-then-call is one motion. Never fire a deferred tool that is not in your current list: it fails with a tool_not_loaded error, and blind-retrying that call is exactly the loop to avoid.
