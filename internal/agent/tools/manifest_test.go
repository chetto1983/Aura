...
func TestFilesystemToolSpecsDescribeOperationalContracts(t *testing.T) {
	tests := []struct {
	...
	}{
		{
			...
			tool:        &ReadFile{},
			wantName:    "read_file",
			wantSummary: "Read a file with line numbers and pagination.",
			wantPhrases: []string{"next_offset", "Read a file with this tool BEFORE patching it", "similar filenames"},
		},
		{
			name:        "patch",
			tool:        &Patch{},
			wantName:    "patch",
			wantSummary: "Targeted find-and-replace edit in a file, with fuzzy fallback.",
			wantPhrases: []string{"REPLACE MODE", "PATCH MODE", "unified diff"},
			wantMutate:  true,
		},
		{
			name:        "write",
			...
			wantPhrases: []string{"verified:true", "do NOT re-read the file", "Always report the absolute path"},
			wantMutate:  true,
		},
		{
			name:        "search",
			tool:        &SearchFiles{},
			wantName:    "search_files",
			wantSummary: "Search file contents or find files by name.",
			wantPhrases: []string{"instead of grep/rg/find/ls", "sorted by modification time", "RE2 regex"},
		},
	}
