...
func ComposeAgentPrompt(cfg *config.Config, loc *time.Location, overlay, pinnedOperational, skillsBlock, toolManifest, wikiTOC string, now time.Time) AgentPromptPlan {
	...
	content += fmt.Sprintf("\n\n## Aura Runtime\n- Prompt Version: %s\n- Tool Discovery: the catalog below lists every tool you have. Invoke any tool by name — the agent loop will load its full schema for this turn.\n\nChoose tools autonomously when they help. For multi-step work, prefer execute_code (Python sandbox) and the file tool to inspect, loop, transform, and verify in one runtime pass instead of asking for many model tool-call rounds. Prefer direct answers when no tool is needed.", version)
