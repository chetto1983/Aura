...
func TestIsAssistantEmpty_TrueForEmptyAssistant(t *testing.T) {
	msg := llm.Message{Role: "assistant", Content: ""}
	if !IsAssistantEmpty(msg) {
		t.Fatal("expected IsAssistantEmpty=true for empty assistant message")
	}
}

func TestIsAssistantEmpty_TrueForWhitespaceOnlyContent(t *testing.T) {
	msg := llm.Message{Role: "assistant", Content: "   \n\t"}
	if !IsAssistantEmpty(msg) {
		t.Fatal("expected IsAssistantEmpty=true for whitespace-only assistant message")
	}
}

func TestIsAssistantEmpty_FalseForAssistantWithContent(t *testing.T) {
	msg := llm.Message{Role: "assistant", Content: "hello"}
	if IsAssistantEmpty(msg) {
		t.Fatal("expected IsAssistantEmpty=false for assistant with content")
	}
}

func TestIsAssistantEmpty_FalseForAssistantWithToolCalls(t *testing.T) {
	msg := llm.Message{
		Role:      "assistant",
		Content:   "",
		ToolCalls: []llm.ToolCall{{ID: "tc1", Name: "file"}},
	}
	if IsAssistantEmpty(msg) {
		t.Fatal("expected IsAssistantEmpty=false for assistant with tool calls")
	}
}

func TestIsAssistantEmpty_FalseForNonAssistantRole(t *testing.T) {
	for _, role := range []string{"user", "tool", "system"} {
		msg := llm.Message{Role: role, Content: ""}
		if IsAssistantEmpty(msg) {
			t.Fatalf("expected IsAssistantEmpty=false for role=%q", role)
		}
	}
}
