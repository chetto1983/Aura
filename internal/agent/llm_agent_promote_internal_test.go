...
// (text_response) and one deferred tool (web_search), enough to exercise the
...
// webSearchSchemaResult mirrors the shape tools.ToolSearch.Execute writes for one
// match: a "## <name>" header, the description, then the Parameters block.
const webSearchSchemaResult = "## web_search\nSearch the web.\n\nParameters:\n{\"type\":\"object\"}\n\n"
...
func TestIsDeferredUnloaded(t *testing.T) {
	...
	if !a.isDeferredUnloaded("web_search") {
		t.Fatal("web_search is deferred and unloaded — gate must report it unloaded")
	}
	...
	a.activated["web_search"] = struct{}{}
	if a.isDeferredUnloaded("web_search") {
		t.Fatal("web_search is promoted — gate must no longer bounce it")
	}
}
...
func TestPromoteFromMeta(t *testing.T) {
	...
	meta := tools.ToolResultMeta{tools.MetaActivatedTools: []string{"web_search", "other_tool"}}
	a.promoteFromMeta(&meta)
	for _, name := range []string{"web_search", "other_tool"} {
...
// not turn-scoped: tool_search loaded web_search's schema in an earlier turn, that schema
...
func TestActivationSurvivesTheTurn(t *testing.T) {
	...
	a := promoteAgentWithHistory(t, []llm.Message{
		...
		{Role: llm.RoleAssistant, ToolCalls: []llm.ToolCall{call(searchID, "tool_search", `{"query":"select:web_search"}`)}},
		{Role: llm.RoleTool, ToolCallID: searchID, Content: webSearchSchemaResult},
		{Role: llm.RoleAssistant, Content: "fatto"},
		{Role: llm.RoleUser, Content: "mandalo di nuovo"},
	})

	if a.isDeferredUnloaded("web_search") {
		t.Fatal("web_search's schema is still in the rehydrated history — a new turn must not force a redundant tool_search")
	}
}
...
// the model could mint its own permissions (`echo "## web_search"`) and then call the tool
...
func TestActivationRequiresARealSearchResult(t *testing.T) {
	...
	a := promoteAgentWithHistory(t, []llm.Message{
		...
		{Role: llm.RoleTool, ToolCallID: shellID, Content: webSearchSchemaResult},
...
func TestActivationDecaysWithTheSchema(t *testing.T) {
	...
	a := promoteAgentWithHistory(t, []llm.Message{
		{Role: llm.RoleUser, Content: "ciao"},
		{Role: llm.RoleAssistant, ToolCalls: []llm.ToolCall{call(searchID, "tool_search", `{"query":"select:text_response,web_search"}`)}},
		{Role: llm.RoleTool, ToolCallID: searchID, Content: truncated},
		{Role: llm.RoleUser, Content: "ora mandalo"},
	})

	if !a.isDeferredUnloaded("web_search") {
		t.Fatal("web_search's schema was truncated out of history — the grant must decay with it, not outlive it")
	}
}
