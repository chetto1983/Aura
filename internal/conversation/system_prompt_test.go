...
func TestDefaultSystemPromptProfessionalCapsuleContract(t *testing.T) {
	...
	for _, want := range []string{
		...
		"Use it for multi-step work when the user asks you to keep track",
		"action=set with content",
		"action=append with line",
		"action=get to read it",
		"action=clear to empty it",
		"what do you know about me?",
		"first action=user_facts",
		"then action=subgraph",
		...
		"write directly with action=write, path, and content",
		"do not discover schemas or call mkdir just to create one file",
		"name: <name>",
