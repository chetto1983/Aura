...
func buildPrecisionTestRegistry(t *testing.T) *Registry {
	...
	precisionSeeds := []struct{ name, description string }{
		...
		{"source", "Manage uploaded sources via action enum: list, read, store, reprocess (ocr|ingest), delete, lint."},
		{"doc", "Generate xlsx/docx/pdf via action enum: xlsx workbook from sheets+rows, docx/pdf from title+blocks (heading/paragraph/bullet/table)."},
		{"execute_code", "Execute Python code in a sandboxed environment. Runs scripts, performs calculations, and processes data."},
		{"file", "Read, write, list, search, or patch files inside the workspace via action enum (list|read|search|write|patch)."},
