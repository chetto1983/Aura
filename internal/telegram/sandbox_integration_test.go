...
func TestSandboxIntegration_ExecuteAndSaveTool(t *testing.T) {
	t.Skip("integration test — requires full Aura stack with process sandbox runtime")
	...
	// 5. LLM optionally calls dev_tool(action=save) to persist the dice simulator
}
...
func TestSandboxIntegration_ToolDiscovery(t *testing.T) {
	...
	// 2. LLM calls dev_tool(action=list), finds csv_summarize from a previous session
	// 3. LLM calls dev_tool(action=read) to get the source
