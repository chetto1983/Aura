...
func setupText(locale string) map[string]string {
	if locale == "it" {
		return map[string]string{
			...
			"ollamaDetected":    "Ollama rilevato su localhost: scegli quel preset per la modalita locale gratuita.",
			...
			"modelHint":         "Scegli un provider. Aura parla con API chat compatibili OpenAI; OpenRouter e una opzione, Ollama e il percorso locale gratuito.",
			"provider":          "Provider",
			"baseURL":           "URL base",
			"model":             "Modello",
			"apiKey":            "Chiave API",
			"apiKeyOptional":    "Chiave API (opzionale per Ollama)",
			"apiKeyPlaceholder": "sk-...",
			"ollamaPlaceholder": "(lascia vuoto per Ollama)",
			...
			"ollamaDesc":        "Gira sulla tua macchina. Nessuna chiave API, nessun costo. Installa da ollama.com.",
	...
	return map[string]string{
		...
		"ollamaDetected":    "Detected Ollama running on localhost - pick that preset for free, local mode.",
		...
		"modelHint":         "Pick a provider. Aura talks to OpenAI-compatible chat APIs; OpenRouter is one option, and Ollama is the free local path.",
		"provider":          "Provider",
		"baseURL":           "Base URL",
		"model":             "Model",
		"apiKey":            "API key",
		"apiKeyOptional":    "API key (optional for Ollama)",
		"apiKeyPlaceholder": "sk-...",
		"ollamaPlaceholder": "(leave blank for Ollama)",
		...
		"ollamaDesc":        "Runs on your machine. No API key, no cost. Install from ollama.com.",
...
func localizedPresets(locale string) []Preset {
	t := setupText(locale)
	out := make([]Preset, len(LLMPresets))
	copy(out, LLMPresets)
	for i := range out {
