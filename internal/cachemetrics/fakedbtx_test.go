...
// receives (so a test can assert the parameterized `since` is bound, never interpolated)
// and replays scripted results / errors for the success and failure branches.
...
// loop. A scanErr fires on the row index it is keyed to; closeErr surfaces from Err().
