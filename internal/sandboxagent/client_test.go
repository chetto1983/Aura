...
func TestClientRunSendsBearerHeader(t *testing.T) {
	var gotAuth string
	srv := httptest.NewServer(http.HandlerFunc(func(w http.ResponseWriter, r *http.Request) {
		gotAuth = r.Header.Get("Authorization")
		_ = json.NewEncoder(w).Encode(RunResponse{Stdout: "ok\n", ExitCode: intPtr(0)})
	}))
	defer srv.Close()

	c := New(Config{BaseURL: srv.URL, TimeoutSec: 5, Token: "spike008-secret"})
	if _, err := c.Run(context.Background(), RunRequest{Command: "true"}); err != nil {
		t.Fatalf("Run: %v", err)
	}
	if gotAuth != "Bearer spike008-secret" {
		t.Fatalf("exec Authorization = %q, want %q", gotAuth, "Bearer spike008-secret")
	}
}

func TestClientHealthSendsBearerHeader(t *testing.T) {
	...
	if gotPath != "/v1/health" {
		t.Fatalf("health path = %q, want /v1/health", gotPath)
	}
	if gotAuth != "Bearer spike008-secret" {
		t.Fatalf("health Authorization = %q, want %q", gotAuth, "Bearer spike008-secret")
	}
}

func TestClientNoTokenOmitsBearer(t *testing.T) {
	var gotAuth string
	srv := httptest.NewServer(http.HandlerFunc(func(w http.ResponseWriter, r *http.Request) {
		gotAuth = r.Header.Get("Authorization")
	...
	c := New(Config{BaseURL: srv.URL, TimeoutSec: 5}) // no Token
	if _, err := c.Run(context.Background(), RunRequest{Command: "true"}); err != nil {
		t.Fatalf("Run: %v", err)
	}
	if gotAuth != "" {
...
func TestClientRun401SurfacesAuthError(t *testing.T) {
	srv := httptest.NewServer(http.HandlerFunc(func(w http.ResponseWriter, _ *http.Request) {
		http.Error(w, "missing or invalid token", http.StatusUnauthorized)
	}))
	defer srv.Close()

	c := New(Config{BaseURL: srv.URL, TimeoutSec: 5, Token: "wrong"})
	_, err := c.Run(context.Background(), RunRequest{Command: "true"})
	if err == nil {
		t.Fatal("want error on 401")
	}
	if !strings.Contains(err.Error(), "auth failed") || !strings.Contains(err.Error(), "AURA_SANDBOX_AGENT_TOKEN") {
		t.Fatalf("401 should surface a clear auth error, got %q", err.Error())
	}
}

func TestClientHealth401SurfacesAuthError(t *testing.T) {
	srv := httptest.NewServer(http.HandlerFunc(func(w http.ResponseWriter, _ *http.Request) {
		http.Error(w, "missing or invalid token", http.StatusUnauthorized)
	}))
	defer srv.Close()
	...
	if !strings.Contains(err.Error(), "auth failed") {
		t.Fatalf("health 401 should surface a clear auth error, got %q", err.Error())
	}
}
